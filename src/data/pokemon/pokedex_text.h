// pixel width limit: 226

const u8 gDummyPokedexText[] = _(
    "This is a newly discovered Pokémon.\n"
    "It is currently under investigation.\n"
    "No detailed information is available\n"
    "at this time.");

const u8 gBulbasaurPokedexText[] = _(
    "Bulbasaur can be seen napping in bright\n"
    "sunlight. There is a seed on its back.\n"
    "By soaking up the sun's rays, the seed\n"
    "grows progressively larger.");

const u8 gIvysaurPokedexText[] = _(
    "To support its bulb, Ivysaur's legs\n"
    "grow sturdy. If it spends more time lying in\n"
    "the sunlight, the bud will soon bloom into\n"
    "a large flower.");

const u8 gVenusaurPokedexText[] = _(
    "Venusaur's flower is said to take on vivid\n"
    "colors if it gets plenty of nutrition and\n"
    "sunlight. The flower's aroma soothes the\n"
    "emotions of people.");

const u8 gCharmanderPokedexText[] = _(
    "The flame that burns at the tip of its\n"
    "tail is an indication of its emotions.\n"
    "The flame wavers when Charmander is\n"
    "happy, and blazes when it is enraged.");

const u8 gCharmeleonPokedexText[] = _(
    "Without pity, its sharp claws destroy foes.\n"
    "If it encounters a strong enemy, it\n"
    "becomes agitated, and the flame on its\n"
    "tail flares with a bluish white color.");

const u8 gCharizardPokedexText[] = _(
    "A Charizard flies about in search of\n"
    "strong opponents. It breathes intense\n"
    "flames that can melt any material. However,\n"
    "it will never torch a weaker foe.");

const u8 gSquirtlePokedexText[] = _(
    "Its shell is not just for protection.\n"
    "Its rounded shape and the grooves on its\n"
    "surface minimize resistance in water,\n"
    "enabling Squirtle to swim at high speeds.");

const u8 gWartortlePokedexText[] = _(
    "Its large tail is covered with rich, thick\n"
    "fur that deepens in color with age.\n"
    "The scratches on its shell are evidence\n"
    "of this Pokémon's toughness in battle.");

const u8 gBlastoisePokedexText[] = _(
    "The waterspouts that protrude from its\n"
    "shell are highly accurate. Their bullets of\n"
    "water can precisely nail tin cans from\n"
    "a distance of over 160 feet.");

const u8 gCaterpiePokedexText[] = _(
    "Its voracious appetite compels it to\n"
    "devour leaves bigger than itself without\n"
    "hesitation. It releases a terribly strong\n"
    "odor from its antennae.");

const u8 gMetapodPokedexText[] = _(
    "Its shell is as hard as an iron slab.\n"
    "A Metapod does not move very much\n"
    "because it is preparing its soft innards\n"
    "for evolution inside the shell.");

const u8 gButterfreePokedexText[] = _(
    "It has a superior ability to search for\n"
    "delicious honey from flowers. It can seek,\n"
    "extract, and carry honey from flowers\n"
    "blooming over six miles away.");

const u8 gWeedlePokedexText[] = _(
    "A Weedle has an extremely acute sense\n"
    "of smell. It distinguishes its favorite\n"
    "kinds of leaves from those it dislikes by\n"
    "sniffing with its big red proboscis (nose).");

const u8 gKakunaPokedexText[] = _(
    "It remains virtually immobile while it\n"
    "clings to a tree. However, on the inside,\n"
    "it busily prepares for evolution. This is\n"
    "evident from how hot its shell becomes.");

const u8 gBeedrillPokedexText[] = _(
    "A Beedrill is extremely territorial.\n"
    "For safety reasons, no one should\n"
    "ever approach its nest. If angered,\n"
    "they will attack in a swarm.");

const u8 gPidgeyPokedexText[] = _(
    "It has an extremely sharp sense of\n"
    "direction. It can unerringly return home to\n"
    "its nest, however far it may be removed\n"
    "from its familiar surroundings.");

const u8 gPidgeottoPokedexText[] = _(
    "This Pokémon flies around, patrolling its\n"
    "large territory. If its living space is\n"
    "violated, it shows no mercy in thoroughly\n"
    "punishing the foe with its sharp claws.");

const u8 gPidgeotPokedexText[] = _(
    "This Pokémon has gorgeous, glossy\n"
    "feathers. Many Trainers are so captivated\n"
    "by the beautiful feathers on its head that\n"
    "they choose Pidgeot as their Pokémon.");

const u8 gRattataPokedexText[] = _(
    "A Rattata is cautious in the extreme.\n"
    "Even while it is asleep, it constantly\n"
    "moves its ears and listens for danger.\n"
    "It will make its nest anywhere.");

const u8 gRaticatePokedexText[] = _(
    "A Raticate's sturdy fangs grow steadily.\n"
    "To keep them ground down, it gnaws on\n"
    "rocks and logs. It may even chew on the\n"
    "walls of houses.");

const u8 gSpearowPokedexText[] = _(
    "Its loud cry can be heard over half a mile\n"
    "away. If its high, keening cry is heard\n"
    "echoing all around, it is a sign that they\n"
    "are warning each other of danger.");

const u8 gFearowPokedexText[] = _(
    "Its long neck and elongated beak are\n"
    "ideal for catching prey in soil or water.\n"
    "It deftly moves this extended and skinny\n"
    "beak to pluck prey.");

const u8 gEkansPokedexText[] = _(
    "An Ekans curls itself up in a spiral while\n"
    "it rests. This position allows it to quickly\n"
    "respond to an enemy from any direction\n"
    "with a threat from its upraised head.");

const u8 gArbokPokedexText[] = _(
    "This Pokémon has a terrifically strong\n"
    "constricting power. It can even flatten\n"
    "steel oil drums. Once it wraps its body\n"
    "around its foe, escaping is impossible.");

const u8 gPikachuPokedexText[] = _(
    "It stores electricity in the electric sacs\n"
    "on its cheeks. When it releases pent-up\n"
    "energy in a burst, the electric power is\n"
    "equal to a lightning bolt.");

const u8 gRaichuPokedexText[] = _(
    "If it stores too much electricity, its\n"
    "behavior turns aggressive. To avoid this,\n"
    "it occasionally discharges excess energy\n"
    "and calms itself down.");

const u8 gSandshrewPokedexText[] = _(
    "When it curls up in a ball, it can make any\n"
    "attack bounce off harmlessly. Its hide has\n"
    "turned tough and solid as a result of\n"
    "living in the desert.");

const u8 gSandslashPokedexText[] = _(
    "It curls up in a ball to protect itself from\n"
    "enemy attacks. It also curls up to prevent\n"
    "heatstroke during the daytime when\n"
    "temperatures rise sharply.");

const u8 gNidoranFPokedexText[] = _(
    "Its highly toxic barbs are thought to have\n"
    "developed as protection for this small-\n"
    "bodied Pokémon. When enraged, it releases\n"
    "a horrible toxin from its horn.");

const u8 gNidorinaPokedexText[] = _(
    "When it is with its friends or\n"
    "family, its barbs are tucked away to\n"
    "prevent injury. It appears to become\n"
    "nervous if separated from the others.");

const u8 gNidoqueenPokedexText[] = _(
    "It is adept at sending foes flying with\n"
    "harsh tackles using its tough, scaly body.\n"
    "This Pokémon is at its strongest when\n"
    "it is defending its young.");

const u8 gNidoranMPokedexText[] = _(
    "The male Nidoran has developed muscles\n"
    "that freely move its ears in any direction.\n"
    "Even the slightest sound does not escape\n"
    "this Pokémon's notice.");

const u8 gNidorinoPokedexText[] = _(
    "Its horn is harder than a diamond.\n"
    "If it senses a hostile presence, all the\n"
    "barbs on its back bristle up at once, and it\n"
    "challenges the foe with all its might.");

const u8 gNidokingPokedexText[] = _(
    "A Nidoking's thick tail packs enormously\n"
    "destructive power capable of toppling\n"
    "a metal transmission tower. Once it goes\n"
    "on a rampage, there is no stopping it.");

const u8 gClefairyPokedexText[] = _(
    "On every night of a full moon, they come\n"
    "out to play. When dawn arrives, the tired\n"
    "Clefairy go to sleep nestled up against\n"
    "each other in deep and quiet mountains.");

const u8 gClefablePokedexText[] = _(
    "A Clefable uses its wings to skip lightly\n"
    "as if it were flying. Its bouncy step\n"
    "lets it even walk on water. On quiet,\n"
    "moonlit nights, it strolls on lakes.");

const u8 gVulpixPokedexText[] = _(
    "It can freely control fire, making fiery\n"
    "orbs fly like will-o'-the-wisps. Just\n"
    "before evolution, its six tails grow hot\n"
    "as if on fire.");

const u8 gNinetalesPokedexText[] = _(
    "It has long been said that each of the\n"
    "nine tails embody an enchanted power.\n"
    "A long-lived Ninetales will have fur that\n"
    "shines like gold.");

const u8 gJigglypuffPokedexText[] = _(
    "Nothing can avoid falling asleep hearing a\n"
    "Jigglypuff's song. The sound waves of its\n"
    "singing voice match the brain waves of\n"
    "someone in a deep sleep.");

const u8 gWigglytuffPokedexText[] = _(
    "Its fur is the ultimate in luxuriousness.\n"
    "Sleeping alongside a Wigglytuff is simply\n"
    "divine. Its body expands seemingly without\n"
    "end when it inhales.");

const u8 gZubatPokedexText[] = _(
    "While living in pitch-black caverns, their\n"
    "eyes gradually grew shut and deprived\n"
    "them of vision. They use ultrasonic waves\n"
    "to detect obstacles.");

const u8 gGolbatPokedexText[] = _(
    "Its fangs easily puncture even thick\n"
    "animal hide. It loves to feast on the blood\n"
    "of people and Pokémon. It flits about in\n"
    "darkness and strikes from behind.");

const u8 gOddishPokedexText[] = _(
    "This Pokémon grows by absorbing moonlight.\n"
    "During the daytime, it buries itself in the\n"
    "ground, leaving only its leaves exposed to\n"
    "avoid detection by its enemies.");

const u8 gGloomPokedexText[] = _(
    "A horribly noxious honey drools from its\n"
    "mouth. One whiff of the honey can result\n"
    "in memory loss. Some fans are said to\n"
    "enjoy this overwhelming stink, however.");

const u8 gVileplumePokedexText[] = _(
    "In seasons when it produces more pollen,\n"
    "the air around a Vileplume turns yellow\n"
    "with the powder as it walks. The pollen is\n"
    "highly toxic and causes paralysis.");

const u8 gParasPokedexText[] = _(
    "A Paras has parasitic tochukaso\n"
    "mushrooms growing on its back. They grow\n"
    "by drawing nutrients from the host. They\n"
    "are valued as a medicine for long life.");

const u8 gParasectPokedexText[] = _(
    "Parasect are known to infest the roots of\n"
    "large trees en masse and drain nutrients.\n"
    "When an infested tree dies, they move\n"
    "onto another tree all at once.");

const u8 gVenonatPokedexText[] = _(
    "Its coat of thin, stiff hair that covers\n"
    "its entire body is said to have evolved\n"
    "for protection. Its large eyes never fail\n"
    "to spot even miniscule prey.");

const u8 gVenomothPokedexText[] = _(
    "Venomoth are nocturnal--they only are\n"
    "active at night. Their favorite prey are\n"
    "insects that gather around streetlights,\n"
    "attracted by the light in the darkness.");

const u8 gDiglettPokedexText[] = _(
    "Diglett are raised in most farms.\n"
    "The reason is simple--wherever they\n"
    "burrow, the soil is left perfectly tilled\n"
    "for growing delicious crops.");

const u8 gDugtrioPokedexText[] = _(
    "Because the triplets originally split from\n"
    "one body, they think exactly alike.\n"
    "They work cooperatively to burrow\n"
    "endlessly through the ground.");

const u8 gMeowthPokedexText[] = _(
    "Meowth withdraw their sharp claws into\n"
    "their paws to silently sneak about.\n"
    "For some reason, this Pokémon loves\n"
    "shiny coins that glitter with light.");

const u8 gPersianPokedexText[] = _(
    "A Persian's six bold whiskers sense air\n"
    "movements to determine what is in its\n"
    "vicinity. It becomes docile if grabbed\n"
    "by the whiskers.");

const u8 gPsyduckPokedexText[] = _(
    "When its headache intensifies, it starts\n"
    "using strange powers. However, it has no\n"
    "recollection of its powers, so it always\n"
    "looks befuddled and bewildered.");

const u8 gGolduckPokedexText[] = _(
    "A Golduck is an adept swimmer.\n"
    "It sometimes joins competitive swimmers\n"
    "in training. It uses psychic powers when\n"
    "its forehead shimmers with light.");

const u8 gMankeyPokedexText[] = _(
    "When it starts shaking and its nasal\n"
    "breathing turns rough, it's a sure sign\n"
    "of anger. However, since this happens\n"
    "instantly, there is no time to flee.");

const u8 gPrimeapePokedexText[] = _(
    "When it becomes furious, its blood\n"
    "circulation becomes more robust, and\n"
    "its muscles are made stronger. But it\n"
    "also becomes much less intelligent.");

const u8 gGrowlithePokedexText[] = _(
    "Its superb sense of smell ensures that\n"
    "this Pokémon won't forget any scent,\n"
    "no matter what. It uses its sense of smell\n"
    "to detect the emotions of others.");

const u8 gArcaninePokedexText[] = _(
    "This fleet-footed Pokémon is said to run\n"
    "over 6,200 miles in a single day and night.\n"
    "The fire that blazes wildly within its body\n"
    "is its source of power.");

const u8 gPoliwagPokedexText[] = _(
    "It is possible to see this Pokémon's spiral\n"
    "innards right through its thin skin.\n"
    "However, the skin is also very flexible.\n"
    "Even sharp fangs bounce right off it.");

const u8 gPoliwhirlPokedexText[] = _(
    "Its body surface is always wet and slick\n"
    "with an oily fluid. Because of this greasy\n"
    "covering, it can easily slip and slide out\n"
    "of the clutches of any enemy in battle.");

const u8 gPoliwrathPokedexText[] = _(
    "Its highly developed muscles never grow\n"
    "fatigued, however much it exercises.\n"
    "This Pokémon can swim back and forth\n"
    "across the Pacific Ocean without effort.");

const u8 gAbraPokedexText[] = _(
    "A Pokémon that sleeps 18 hours a day.\n"
    "Observation revealed that it uses\n"
    "Teleport to change its location once\n"
    "every hour.");

const u8 gKadabraPokedexText[] = _(
    "It is rumored that a boy with psychic\n"
    "abilities suddenly transformed into\n"
    "Kadabra while he was assisting research\n"
    "into extrasensory powers.");

const u8 gAlakazamPokedexText[] = _(
    "While it has strong psychic abilities and\n"
    "high intelligence, an Alakazam's muscles\n"
    "are very weak. It uses psychic power to\n"
    "move its body.");

const u8 gMachopPokedexText[] = _(
    "It continually undertakes strenuous\n"
    "training to master all forms of martial\n"
    "arts. Its strength lets it easily hoist\n"
    "a sumo wrestler onto its shoulders.");

const u8 gMachokePokedexText[] = _(
    "A belt is worn by a Machoke to keep its\n"
    "overwhelming power under control.\n"
    "Because it is so dangerous, no one has\n"
    "ever removed the belt.");

const u8 gMachampPokedexText[] = _(
    "It is impossible to defend against punches\n"
    "and chops doled out by its four arms.\n"
    "Its fighting spirit flares up when it faces\n"
    "a tough opponent.");

const u8 gBellsproutPokedexText[] = _(
    "A Bellsprout's thin and flexible body lets\n"
    "it bend and sway to avoid any attack,\n"
    "however strong it may be. From its mouth,\n"
    "it leaks a fluid that melts even iron.");

const u8 gWeepinbellPokedexText[] = _(
    "At night, a Weepinbell hangs on to a tree\n"
    "branch with its hooked rear and sleeps.\n"
    "If it moves around in its sleep, it may\n"
    "wake up to find itself on the ground.");

const u8 gVictreebelPokedexText[] = _(
    "The long vine extending from its head is\n"
    "waved about as if it were a living thing to\n"
    "attract prey. When an unsuspecting victim\n"
    "approaches, it is swallowed whole.");

const u8 gTentacoolPokedexText[] = _(
    "Its body is almost entirely composed of\n"
    "water. It ensnares its foe with its two\n"
    "long tentacles, then stabs with the poison\n"
    "stingers at their tips.");

const u8 gTentacruelPokedexText[] = _(
    "It lives in complex rock formations on\n"
    "the ocean floor and traps prey using its\n"
    "80 tentacles. Its red orbs glow when it\n"
    "grows excited or agitated.");

const u8 gGeodudePokedexText[] = _(
    "It climbs mountain paths using only the\n"
    "power of its arms. Because they look just\n"
    "like boulders lining paths, hikers may step\n"
    "on them without noticing.");

const u8 gGravelerPokedexText[] = _(
    "They descend from mountains by tumbling\n"
    "down steep slopes. They are so brutal,\n"
    "they smash aside obstructing trees and\n"
    "massive boulders with thunderous tackles.");

const u8 gGolemPokedexText[] = _(
    "It is said to live in volcanic craters\n"
    "on mountain peaks. Once a year, it sheds\n"
    "its hide and grows larger. The shed hide\n"
    "crumbles and returns to the soil.");

const u8 gPonytaPokedexText[] = _(
    "A Ponyta is very weak at birth. It can\n"
    "barely stand up. Its legs become stronger\n"
    "as it stumbles and falls while trying to\n"
    "keep up with its parent.");

const u8 gRapidashPokedexText[] = _(
    "It usually canters casually in the fields\n"
    "and plains. But once a Rapidash turns\n"
    "serious, its fiery manes flare and blaze\n"
    "as it gallops its way up to 150 mph.");

const u8 gSlowpokePokedexText[] = _(
    "It catches prey by dipping its tail in\n"
    "water at the side of a river. But it often\n"
    "forgets what it is doing and spends entire\n"
    "days just loafing at water's edge.");

const u8 gSlowbroPokedexText[] = _(
    "Its tail has a Shellder firmly attached\n"
    "with a bite. As a result, the tail can't be\n"
    "used for fishing anymore. This forces it\n"
    "to reluctantly swim and catch prey.");

const u8 gMagnemitePokedexText[] = _(
    "The units at its sides are extremely\n"
    "powerful magnets. They generate enough\n"
    "magnetism to draw in iron objects from\n"
    "over 300 feet away.");

const u8 gMagnetonPokedexText[] = _(
    "It is actually three Magnemite linked\n"
    "by magnetism. It generates powerful radio\n"
    "waves that raise temperatures by 3.6\n"
    "degrees F within a 3,300-foot radius.");

const u8 gFarfetchdPokedexText[] = _(
    "It is always seen with a stick from a plant.\n"
    "Apparently, there are good sticks and bad\n"
    "sticks. This Pokémon occasionally fights\n"
    "with others over choice sticks.");

const u8 gDoduoPokedexText[] = _(
    "Even while eating or sleeping, one of the\n"
    "heads remains always vigilant for any sign\n"
    "of danger. When threatened, it flees at\n"
    "over 60 miles per hour.");

const u8 gDodrioPokedexText[] = _(
    "A peculiar Pokémon species with three\n"
    "heads. It vigorously races across grassy\n"
    "plains even in arid seasons with little\n"
    "rainfall.");

const u8 gSeelPokedexText[] = _(
    "Seel hunt for prey in frigid, ice-covered\n"
    "seas. When it needs to breathe, it punches\n"
    "a hole through the ice with the sharply\n"
    "protruding section of its head.");

const u8 gDewgongPokedexText[] = _(
    "It loves to snooze on bitterly cold ice.\n"
    "The sight of this Pokémon sleeping on\n"
    "a glacier was mistakenly thought to be\n"
    "a mermaid by a mariner long ago.");

const u8 gGrimerPokedexText[] = _(
    "Born from polluted sludge in the sea,\n"
    "Grimer's favorite food is anything filthy.\n"
    "They feed on wastewater pumped out from\n"
    "factories.");

const u8 gMukPokedexText[] = _(
    "It prefers warm and humid habitats.\n"
    "In the summertime, the toxic substances\n"
    "in its body intensify, making Muk reek like\n"
    "putrid kitchen garbage.");

const u8 gShellderPokedexText[] = _(
    "At night, it burrows a hole in the seafloor\n"
    "with its broad tongue to make a place to\n"
    "sleep. While asleep, it closes its shell,\n"
    "but leaves its tongue hanging out.");

const u8 gCloysterPokedexText[] = _(
    "It swims in the sea by swallowing water,\n"
    "then jetting it out toward the rear.\n"
    "The Cloyster shoots spikes from its\n"
    "shell using the same system.");

const u8 gGastlyPokedexText[] = _(
    "When exposed to a strong wind, a Gastly's\n"
    "gaseous body quickly dwindles away.\n"
    "They cluster under the eaves of houses\n"
    "to escape the ravages of wind.");

const u8 gHaunterPokedexText[] = _(
    "If a Haunter beckons you while it is\n"
    "floating in darkness, don't approach it.\n"
    "This Pokémon will try to lick you with its\n"
    "tongue and steal your life away.");

const u8 gGengarPokedexText[] = _(
    "Deep in the night, your shadow cast by\n"
    "a streetlight may suddenly overtake you.\n"
    "It is actually a Gengar running past\n"
    "you, pretending to be your shadow.");

const u8 gOnixPokedexText[] = _(
    "There is a magnet in its brain that\n"
    "prevents an Onix from losing direction\n"
    "while tunneling. As it grows older, its body\n"
    "becomes steadily rounder and smoother.");

const u8 gDrowzeePokedexText[] = _(
    "If your nose becomes itchy while you are\n"
    "sleeping, it's a sure sign that a Drowzee is\n"
    "standing above your pillow and trying to\n"
    "eat your dream through your nostrils.");

const u8 gHypnoPokedexText[] = _(
    "The arcing movement and glitter of the\n"
    "pendulum in a Hypno's hand lull the foe\n"
    "into deep hypnosis. While searching for\n"
    "prey, it polishes the pendulum.");

const u8 gKrabbyPokedexText[] = _(
    "Krabby live in holes dug into beaches.\n"
    "On sandy shores with little in the way\n"
    "of food, they can be seen squabbling with\n"
    "each other over territory.");

const u8 gKinglerPokedexText[] = _(
    "It waves its huge, oversized claw in the\n"
    "air to communicate with others.\n"
    "But since the claw is so heavy, this\n"
    "Pokémon quickly tires.");

const u8 gVoltorbPokedexText[] = _(
    "It bears an uncanny and unexplained\n"
    "resemblance to a Poké Ball. Because it\n"
    "explodes at the slightest shock, even\n"
    "veteran Trainers treat it with caution.");

const u8 gElectrodePokedexText[] = _(
    "They appear in great numbers at electric\n"
    "power plants. Because they feed on\n"
    "electricity, they cause massive and\n"
    "chaotic blackouts in nearby cities.");

const u8 gExeggcutePokedexText[] = _(
    "It consists of six eggs that care for each\n"
    "other. The eggs attract each other and\n"
    "spin around. When cracks increasingly\n"
    "appear, it is close to evolution.");

const u8 gExeggutorPokedexText[] = _(
    "Originally from the tropics, Exeggutor's\n"
    "heads grow larger from exposure to strong\n"
    "sunlight. It is said that when the heads\n"
    "fall, they group to form an Exeggcute.");

const u8 gCubonePokedexText[] = _(
    "It pines for the mother it will never see\n"
    "again. Seeing a likeness of its mother in\n"
    "the full moon, it cries. The stains on the\n"
    "skull it wears are from its tears.");

const u8 gMarowakPokedexText[] = _(
    "A Marowak is the evolved form of a Cubone\n"
    "that has grown tough by overcoming the\n"
    "grief of losing its mother. Its tempered\n"
    "and hardened spirit is not easily broken.");

const u8 gHitmonleePokedexText[] = _(
    "Its legs freely stretch and contract.\n"
    "Using these springlike limbs, it bowls over\n"
    "foes with devastating kicks. After battle,\n"
    "it rubs down its tired legs.");

const u8 gHitmonchanPokedexText[] = _(
    "A Hitmonchan is said to possess the\n"
    "spirit of a boxer who aimed to become the\n"
    "world champion. Having an indomitable\n"
    "spirit means that it will never give up.");

const u8 gLickitungPokedexText[] = _(
    "Whenever it sees something unfamiliar,\n"
    "it always licks the object because it\n"
    "memorizes things by texture and taste.\n"
    "It is somewhat put off by sour things.");

const u8 gKoffingPokedexText[] = _(
    "Getting up close to a Koffing will give\n"
    "you a chance to observe, through its thin\n"
    "skin, the toxic gases swirling inside. It\n"
    "blows up at the slightest stimulation.");

const u8 gWeezingPokedexText[] = _(
    "By diluting its toxic gases with a special\n"
    "process, the highest grade of perfume can\n"
    "be extracted. To Weezing, gases emanating\n"
    "from garbage are the ultimate feast.");

const u8 gRhyhornPokedexText[] = _(
    "Once it starts running, it doesn't stop.\n"
    "Its tiny brain makes it so stupid that it\n"
    "can't remember why it started running in\n"
    "the first place.");

const u8 gRhydonPokedexText[] = _(
    "Its horn, which rotates like a drill,\n"
    "destroys tall buildings with one strike.\n"
    "It stands on its hind legs, and its brain\n"
    "is well developed.");

const u8 gChanseyPokedexText[] = _(
    "Chansey lay nutritionally excellent eggs\n"
    "every day. The eggs are so delicious, they\n"
    "are eagerly devoured by even those people\n"
    "who have lost their appetite.");

const u8 gTangelaPokedexText[] = _(
    "Its vines snap off easily and painlessly\n"
    "if they are grabbed, allowing it to make a\n"
    "quick getaway. The lost vines are replaced\n"
    "by new growth the very next day.");

const u8 gKangaskhanPokedexText[] = _(
    "If you come across a young Kangaskhan\n"
    "playing by itself, never try to catch it.\n"
    "The baby's parent is sure to be in the area,\n"
    "and it will become violently enraged.");

const u8 gHorseaPokedexText[] = _(
    "By cleverly flicking the fins on its back\n"
    "side to side, it moves in any direction\n"
    "while facing forward. It spits ink to\n"
    "escape if it senses danger.");

const u8 gSeadraPokedexText[] = _(
    "The poisonous barbs all over its body are\n"
    "highly valued as ingredients for making\n"
    "traditional herbal medicine. It shows no\n"
    "mercy to anything approaching its nest.");

const u8 gGoldeenPokedexText[] = _(
    "In the springtime, schools of Goldeen\n"
    "can be seen swimming up falls and rivers.\n"
    "It metes out staggering damage with its\n"
    "single horn.");

const u8 gSeakingPokedexText[] = _(
    "It punches holes in boulders on stream-\n"
    "beds. This is a clever innovation that\n"
    "prevents its eggs from being attacked or\n"
    "washed away by the current.");

const u8 gStaryuPokedexText[] = _(
    "It gathers with others in the night and\n"
    "makes its red core glow on and off with\n"
    "the twinkling stars. It can regenerate\n"
    "limbs if they are severed from its body.");

const u8 gStarmiePokedexText[] = _(
    "People in ancient times imagined that\n"
    "Starmie were transformed from the\n"
    "reflections of stars that twinkled on\n"
    "gentle waves at night.");

const u8 gMrMimePokedexText[] = _(
    "A Mr. Mime is a master of pantomime. It can\n"
    "convince others that something unseeable\n"
    "actually exists. Once believed, the\n"
    "imaginary object does become real.");

const u8 gScytherPokedexText[] = _(
    "Its blindingly fast speed adds to the\n"
    "sharpness of its twin forearm scythes.\n"
    "The scythes can slice through thick logs\n"
    "in one wicked stroke.");

const u8 gJynxPokedexText[] = _(
    "A Jynx sashays rhythmically as if it were\n"
    "dancing. Its motions are so bouncingly\n"
    "alluring, people seeing it are compelled to\n"
    "shake their hips without noticing.");

const u8 gElectabuzzPokedexText[] = _(
    "When a storm approaches, it competes with\n"
    "others to scale heights that are likely to\n"
    "be stricken by lightning. Some towns use\n"
    "Electabuzz in place of lightning rods.");

const u8 gMagmarPokedexText[] = _(
    "In battle, it blows out intense flames from\n"
    "all over its body to intimidate its foe.\n"
    "These fiery bursts create heat waves that\n"
    "ignite grass and trees in the area.");

const u8 gPinsirPokedexText[] = _(
    "Their pincers are strong enough to\n"
    "shatter thick logs. Because they dislike\n"
    "cold, Pinsir burrow and sleep under\n"
    "the ground on chilly nights.");

const u8 gTaurosPokedexText[] = _(
    "It is not satisfied unless it is rampaging\n"
    "at all times. If there is no opponent for\n"
    "Tauros to battle, it will charge at thick\n"
    "trees and knock them down to calm itself.");

const u8 gMagikarpPokedexText[] = _(
    "Its swimming muscles are weak, so it is\n"
    "easily washed away by currents. In places\n"
    "where water pools, you can see many\n"
    "Magikarp deposited there by the flow.");

const u8 gGyaradosPokedexText[] = _(
    "It is an extremely vicious and violent\n"
    "Pokémon. When humans begin to fight,\n"
    "it will appear and burn everything to the\n"
    "ground with intensely hot flames.");

const u8 gLaprasPokedexText[] = _(
    "People have driven Lapras almost to the\n"
    "point of extinction. In the evenings,\n"
    "it is said to sing plaintively as it seeks\n"
    "what few others of its kind still remain.");

const u8 gDittoPokedexText[] = _(
    "A Ditto rearranges its cell structure to\n"
    "transform itself. However, if it tries to\n"
    "change based on its memory, it will get\n"
    "details wrong.");

const u8 gEeveePokedexText[] = _(
    "An Eevee has an unstable genetic makeup\n"
    "that suddenly mutates due to its\n"
    "environment. Radiation from various\n"
    "Stones causes this Pokémon to evolve.");

const u8 gVaporeonPokedexText[] = _(
    "Vaporeon underwent a spontaneous\n"
    "mutation and grew fins and gills that\n"
    "allow them to live underwater. They have\n"
    "the ability to freely control water.");

const u8 gJolteonPokedexText[] = _(
    "Its cells generate weak power that is\n"
    "amplified by its fur's static electricity\n"
    "to drop thunderbolts. The bristling fur is\n"
    "made of electrically charged needles.");

const u8 gFlareonPokedexText[] = _(
    "Flareon's fluffy fur releases heat into\n"
    "the air so that its body does not get\n"
    "excessively hot. Its body temperature can\n"
    "rise to a maximum of 1,650 degrees F.");

const u8 gPorygonPokedexText[] = _(
    "It is capable of reverting itself entirely\n"
    "back to program data in order to enter\n"
    "cyberspace. A Porygon is copy-\n"
    "protected so it cannot be duplicated.");

const u8 gOmanytePokedexText[] = _(
    "One of the ancient and long-since-extinct\n"
    "Pokémon that have been regenerated\n"
    "from fossils by humans. If attacked,\n"
    "it withdraws into its hard shell.");

const u8 gOmastarPokedexText[] = _(
    "An Omastar uses its tentacles to capture\n"
    "its prey. It is believed to have become\n"
    "extinct because its shell grew too large,\n"
    "making its movements slow and ponderous.");

const u8 gKabutoPokedexText[] = _(
    "It is a Pokémon that has been regenerated\n"
    "from a fossil. However, in rare cases, living\n"
    "examples have been discovered. Kabuto\n"
    "have not changed for 300 million years.");

const u8 gKabutopsPokedexText[] = _(
    "Kabutops once swam underwater to hunt\n"
    "for prey. It was apparently evolving from\n"
    "being a water dweller to living on land as\n"
    "evident from changes in its gills and legs.");

const u8 gAerodactylPokedexText[] = _(
    "Aerodactyl is a Pokémon from the age\n"
    "of dinosaurs. It was regenerated from DNA\n"
    "extracted from amber. It is imagined to\n"
    "have been the king of the skies.");

const u8 gSnorlaxPokedexText[] = _(
    "Snorlax's typical day consists of nothing\n"
    "more than eating and sleeping. It is such\n"
    "a docile Pokémon that there are children\n"
    "who use its big belly as a place to play.");

const u8 gArticunoPokedexText[] = _(
    "Articuno is a legendary bird Pokémon that\n"
    "can control ice. The flapping of its wings\n"
    "chills the air. As a result, it is said that\n"
    "when this Pokémon flies, snow will fall.");

const u8 gZapdosPokedexText[] = _(
    "Zapdos is a legendary bird Pokémon that\n"
    "has the ability to control electricity.\n"
    "It usually lives in thunderclouds. It gains\n"
    "power if it is stricken by lightning bolts.");

const u8 gMoltresPokedexText[] = _(
    "Moltres is a legendary bird Pokémon\n"
    "that can control fire. If injured, it is said\n"
    "to dip its body in the molten magma of\n"
    "a volcano to burn and heal itself.");

const u8 gDratiniPokedexText[] = _(
    "A Dratini continually molts and sloughs\n"
    "off its old skin. It does so because the\n"
    "life energy within its body steadily builds\n"
    "to reach uncontrollable levels.");

const u8 gDragonairPokedexText[] = _(
    "A Dragonair stores an enormous amount of\n"
    "energy inside its body. It is said to alter\n"
    "the weather around it by loosing energy\n"
    "from the crystals on its neck and tail.");

const u8 gDragonitePokedexText[] = _(
    "It can circle the globe in just 16 hours.\n"
    "It is a kindhearted Pokémon that leads\n"
    "lost and foundering ships in a storm\n"
    "to the safety of land.");

const u8 gMewtwoPokedexText[] = _(
    "A Pokémon that was created by genetic\n"
    "manipulation. However, even though the\n"
    "scientific power of humans made its body,\n"
    "they failed to give it a warm heart.");

const u8 gMewPokedexText[] = _(
    "A Mew is said to possess the genes of all\n"
    "Pokémon. It is capable of making itself\n"
    "invisible at will, so it entirely avoids\n"
    "notice even if it approaches people.");

const u8 gChikoritaPokedexText[] = _(
    "It waves its leaf around to keep foes\n"
    "at bay. However, a sweet fragrance also\n"
    "wafts from the leaf, creating a friendly\n"
    "atmosphere that becalms the battlers.");

const u8 gBayleefPokedexText[] = _(
    "A Bayleef's neck is ringed by curled-up\n"
    "leaves. Inside each leaf is a small tree\n"
    "shoot. The fragrance of this shoot\n"
    "makes people peppy.");

const u8 gMeganiumPokedexText[] = _(
    "The fragrance of a Meganium's flower\n"
    "soothes and calms emotions. In battle,\n"
    "it gives off more of its becalming scent\n"
    "to blunt the foe's fighting spirit.");

const u8 gCyndaquilPokedexText[] = _(
    "It flares flames from its back to protect\n"
    "itself. The fire burns vigorously if the\n"
    "Pokémon is angry. When it is tired,\n"
    "it sputters with incomplete combustion.");

const u8 gQuilavaPokedexText[] = _(
    "It intimidates foes with intense gusts of\n"
    "flames and superheated air. Its quick\n"
    "nimbleness lets it dodge attacks even\n"
    "while scorching an enemy.");

const u8 gTyphlosionPokedexText[] = _(
    "It can hide behind a shimmering heat haze\n"
    "that it creates using its intense flames.\n"
    "Typhlosion create blazing explosive\n"
    "blasts that burn everything to cinders.");

const u8 gTotodilePokedexText[] = _(
    "Despite its small body, Totodile's jaws\n"
    "are very powerful. While it may think it is\n"
    "just playfully nipping, its bite has enough\n"
    "strength to cause serious injury.");

const u8 gCroconawPokedexText[] = _(
    "Once its jaws clamp down on its foe, it will\n"
    "absolutely not let go. Because the tips of\n"
    "its fangs are forked back like fishhooks,\n"
    "they become irremovably embedded.");

const u8 gFeraligatrPokedexText[] = _(
    "It opens its huge mouth to intimidate\n"
    "enemies. In battle, it runs using its thick\n"
    "and powerful hind legs to charge the\n"
    "foe with incredible speed.");

const u8 gSentretPokedexText[] = _(
    "They take turns standing guard when it\n"
    "is time to sleep. The sentry awakens the\n"
    "others if it senses danger. If one becomes\n"
    "separated, it turns sleepless with fear.");

const u8 gFurretPokedexText[] = _(
    "A Furret has a very slim build. When under\n"
    "attack, it can squirm through narrow\n"
    "spaces and get away. In spite of its short\n"
    "limbs, it is very nimble and fleet.");

const u8 gHoothootPokedexText[] = _(
    "It has an internal organ that senses\n"
    "the earth's rotation. Using this special\n"
    "organ, a Hoothoot begins hooting at\n"
    "precisely the same time every day.");

const u8 gNoctowlPokedexText[] = _(
    "It unfailingly catches prey in darkness.\n"
    "Noctowl owe their success to superior\n"
    "vision that allows them to see in minimal\n"
    "light, and to their supple and silent wings.");

const u8 gLedybaPokedexText[] = _(
    "Ledyba communicate using a fluid that\n"
    "they secrete from where the legs join the\n"
    "body. They are said to convey feelings to\n"
    "others by altering the fluid's scent.");

const u8 gLedianPokedexText[] = _(
    "It is said that in lands with clean air,\n"
    "where the stars fill the sky, there live\n"
    "many Ledian. For good reason, they use\n"
    "the light of the stars as energy.");

const u8 gSpinarakPokedexText[] = _(
    "The web it spins can be considered its\n"
    "second nervous system. It is said that a\n"
    "Spinarak determines its prey by the tiny\n"
    "vibrations it feels through the web.");

const u8 gAriadosPokedexText[] = _(
    "Its feet are tipped with tiny hooked claws\n"
    "that enable it to scuttle on ceilings and\n"
    "vertical walls. It constricts its foe with\n"
    "thin and strong silk webbing.");

const u8 gCrobatPokedexText[] = _(
    "Over the course of evolution, its hind legs\n"
    "turned into wings. By alternately resting\n"
    "its front and rear wings, it can fly all day\n"
    "without having to stop.");

const u8 gChinchouPokedexText[] = _(
    "When it senses danger, it discharges\n"
    "positive and negative electricity from its\n"
    "two antennae. It lives in depths beyond\n"
    "sunlight's reach.");

const u8 gLanturnPokedexText[] = _(
    "The light-emitting orbs on its back are\n"
    "very bright. They are formed from a part of\n"
    "its dorsal fin. This Pokémon illuminates\n"
    "the inky darkness of deep seas.");

const u8 gPichuPokedexText[] = _(
    "It is still inept at retaining electricity.\n"
    "When it is startled, it discharges power\n"
    "accidentally. It gets better at holding\n"
    "power as it grows older.");

const u8 gCleffaPokedexText[] = _(
    "On nights with many shooting stars,\n"
    "Cleffa can be seen dancing in a ring.\n"
    "They dance until daybreak, when they\n"
    "quench their thirst with the morning dew.");

const u8 gIgglybuffPokedexText[] = _(
    "Its soft and pliable body is very bouncy.\n"
    "When it sings continuously with all its\n"
    "might, its body steadily turns a deepening\n"
    "pink color.");

const u8 gTogepiPokedexText[] = _(
    "As its energy, it uses the feelings of\n"
    "compassion and pleasure exuded by\n"
    "people and Pokémon. It stores up happy\n"
    "feelings in its shell, then shares them out.");

const u8 gTogeticPokedexText[] = _(
    "It is said to be a Pokémon that brings good\n"
    "fortune. When it spots someone who is pure\n"
    "of heart, a Togetic appears and shares its\n"
    "happiness with that person.");

const u8 gNatuPokedexText[] = _(
    "It runs up short trees that grow on the\n"
    "savanna to peck at new shoots.\n"
    "A Natu's eyes look as if they are\n"
    "always observing something.");

const u8 gXatuPokedexText[] = _(
    "It has the enigmatic power of foreseeing\n"
    "the future. Some people in different lands\n"
    "have long believed that Xatu are\n"
    "emissaries from another world.");

const u8 gMareepPokedexText[] = _(
    "Its fluffy wool rubs together and builds\n"
    "a static charge. The more energy is\n"
    "charged, the more brightly the lightbulb\n"
    "at the tip of its tail glows.");

const u8 gFlaaffyPokedexText[] = _(
    "Its fleece quality changes to generate\n"
    "strong static electricity with a small\n"
    "amount of wool. The bare, slick parts of its\n"
    "hide are shielded against electricity.");

const u8 gAmpharosPokedexText[] = _(
    "It gives off so much light that it can be\n"
    "seen even from space. People in the old\n"
    "days used its light to send signals back\n"
    "and forth with others far away.");

const u8 gBellossomPokedexText[] = _(
    "Its flower petals deepen in color through\n"
    "exposure to sunlight. When cloudy weather\n"
    "persists, it does a dance that is thought\n"
    "to be a ritual for summoning the sun.");

const u8 gMarillPokedexText[] = _(
    "Its body is covered with water-repellent\n"
    "fur. Because of the fur, it can swim\n"
    "through water at high speed without being\n"
    "slowed by the water's resistance.");

const u8 gAzumarillPokedexText[] = _(
    "It lives in water virtually all day long.\n"
    "Its body color and pattern act as\n"
    "camouflage that makes it tough for\n"
    "enemies to spot in water.");

const u8 gSudowoodoPokedexText[] = _(
    "It mimics a tree to avoid being attacked\n"
    "by enemies. But since its forelegs\n"
    "remain green throughout the year, it is\n"
    "easily identified as a fake in the winter.");

const u8 gPolitoedPokedexText[] = _(
    "The curled hair on its head proves its\n"
    "status as a king. It is said that the\n"
    "longer and curlier the hair, the more\n"
    "respect it earns from its peers.");

const u8 gHoppipPokedexText[] = _(
    "This Pokémon drifts and floats with the\n"
    "wind. If it senses the approach of strong\n"
    "winds, a Hoppip links leaves with others\n"
    "to prepare against being blown away.");

const u8 gSkiploomPokedexText[] = _(
    "It blossoms when the temperature rises\n"
    "above 64 degrees F. Because its flower's\n"
    "blooming changes with the temperature,\n"
    "it is sometimes used as a thermometer.");

const u8 gJumpluffPokedexText[] = _(
    "Jumpluff ride warm southern winds to\n"
    "cross the sea and fly to foreign lands.\n"
    "This Pokémon lands when it encounters\n"
    "cold air while it is floating.");

const u8 gAipomPokedexText[] = _(
    "Its tail ends with a dexterous, handlike\n"
    "appendage. However, because it uses the\n"
    "tail so much, Aipom's real hands have\n"
    "become rather clumsy.");

const u8 gSunkernPokedexText[] = _(
    "Sunkern try to minimize movement to\n"
    "conserve the nutrients they have stored\n"
    "in their bodies for evolution. They will\n"
    "not eat, subsisting only on morning dew.");

const u8 gSunfloraPokedexText[] = _(
    "Sunflora convert solar energy into\n"
    "nutrition. They are highly active in the\n"
    "warm daytime but suddenly stop moving as\n"
    "soon as the sun sets.");

const u8 gYanmaPokedexText[] = _(
    "It can see 360 degrees without moving\n"
    "its eyes. It is a great flier capable of\n"
    "making sudden stops and turning midair to\n"
    "quickly chase down targeted prey.");

const u8 gWooperPokedexText[] = _(
    "Wooper usually live in water but come\n"
    "out onto land seeking food occasionally.\n"
    "On land, they coat their bodies with a\n"
    "gooey, toxic film.");

const u8 gQuagsirePokedexText[] = _(
    "A Quagsire hunts by leaving its mouth wide\n"
    "open in water and waiting for its prey to\n"
    "blunder in. Because it doesn't move, it\n"
    "does not get very hungry.");

const u8 gEspeonPokedexText[] = _(
    "An Espeon is extremely loyal to any\n"
    "Trainer it considers to be worthy. It is\n"
    "said to have developed precognitive\n"
    "powers to protect its Trainer from harm.");

const u8 gUmbreonPokedexText[] = _(
    "Umbreon evolved from exposure to the\n"
    "moon's energy pulses. It lurks in darkness\n"
    "and waits for its foes to move. The rings\n"
    "on its body glow when it leaps to attack.");

const u8 gMurkrowPokedexText[] = _(
    "Murkrow were feared as the alleged\n"
    "bearers of ill fortune. It shows strong\n"
    "interest in anything that sparkles. It will\n"
    "even try to steal rings from women.");

const u8 gSlowkingPokedexText[] = _(
    "It undertakes research every day to\n"
    "solve the mysteries of the world.\n"
    "However, it apparently forgets everything\n"
    "if the Shellder on its head comes off.");

const u8 gMisdreavusPokedexText[] = _(
    "A Misdreavus frightens people with a\n"
    "creepy, sobbing cry. It apparently uses\n"
    "its red spheres to absorb the fear of foes\n"
    "as its nutrition.");

const u8 gUnownPokedexText[] = _(
    "This Pokémon is shaped like ancient text\n"
    "characters. Although research is ongoing,\n"
    "it is a mystery as to which came first,\n"
    "the ancient writings or the various Unown.");

const u8 gWobbuffetPokedexText[] = _(
    "Usually docile, a Wobbuffet strikes back\n"
    "ferociously if its black tail is attacked.\n"
    "It makes its lair in caves where it waits\n"
    "for nightfall.");

const u8 gGirafarigPokedexText[] = _(
    "A Girafarig is an herbivore--it eats\n"
    "grass and tree shoots. While it is eating,\n"
    "its tail makes chewing and swallowing\n"
    "motions as if it were also eating.");

const u8 gPinecoPokedexText[] = _(
    "A Pineco hangs from a tree branch and\n"
    "waits for prey. While eating, if it is\n"
    "disturbed by someone shaking its tree, it\n"
    "falls on the ground and suddenly explodes.");

const u8 gForretressPokedexText[] = _(
    "It keeps itself inside its steel shell.\n"
    "The shell is opened when it is catching\n"
    "prey, but it is so quick that the shell's\n"
    "inside cannot be seen.");

const u8 gDunsparcePokedexText[] = _(
    "Its drill-tipped tail is used to burrow into\n"
    "the ground backwards. This Pokémon is\n"
    "known to make its nest in complex shapes\n"
    "deep under the ground.");

const u8 gGligarPokedexText[] = _(
    "It glides without making a single sound.\n"
    "It grasps the face of its foe using its\n"
    "hind and large front claws, then stabs\n"
    "with its poison barb.");

const u8 gSteelixPokedexText[] = _(
    "Steelix live even further underground\n"
    "than Onix. This Pokémon is known to dig\n"
    "toward the earth's core, reaching a depth\n"
    "of over six-tenths of a mile underground.");

const u8 gSnubbullPokedexText[] = _(
    "By baring its fangs and making a scary\n"
    "face, it sends smaller Pokémon scurrying\n"
    "in terror. The Snubbull does seem a\n"
    "little sad at making its foes flee.");

const u8 gGranbullPokedexText[] = _(
    "It has a particularly well-developed lower\n"
    "jaw. The huge fangs are heavy, causing\n"
    "it to tilt its head. Unless it is startled, it\n"
    "will not try to bite.");

const u8 gQwilfishPokedexText[] = _(
    "A Qwilfish uses the pressure of water\n"
    "it swallows to shoot toxic quills all at\n"
    "once from all over its body. It finds\n"
    "swimming to be somewhat challenging.");

const u8 gScizorPokedexText[] = _(
    "A Scizor has a body with the hardness of\n"
    "steel. It is not easily fazed by ordinary\n"
    "sorts of attacks. It flaps its wings to\n"
    "regulate its body temperature.");

const u8 gShucklePokedexText[] = _(
    "A Shuckle hides under rocks, keeping its\n"
    "body concealed inside its shell while\n"
    "eating stored berries. The berries mix with\n"
    "its body fluids to become a juice.");

const u8 gHeracrossPokedexText[] = _(
    "They gather in forests seeking the sweet\n"
    "sap of trees. It is completely clad in a\n"
    "steel-hard shell. It is proud of its horn,\n"
    "which it uses to fling foes.");

const u8 gSneaselPokedexText[] = _(
    "A Sneasel scales trees by punching its\n"
    "hooked claws into the bark. It seeks out\n"
    "unguarded nests and steals eggs for food\n"
    "while the parents are away.");

const u8 gTeddiursaPokedexText[] = _(
    "It licks its palms that are sweetened by\n"
    "being soaked in honey. A Teddiursa makes\n"
    "its own honey by blending fruits and pollen\n"
    "collected by Beedrill.");

const u8 gUrsaringPokedexText[] = _(
    "In forests, it is said that there are many\n"
    "streams and towering trees where an\n"
    "Ursaring gathers food. It walks through\n"
    "its forest collecting food every day.");

const u8 gSlugmaPokedexText[] = _(
    "It is a species of Pokémon that lives in\n"
    "volcanic areas. If its body cools, its skin\n"
    "hardens and immobilizes it. To avoid that,\n"
    "it sleeps near magma.");

const u8 gMagcargoPokedexText[] = _(
    "The shell on its back is made of hardened\n"
    "magma. Tens of thousands of years spent\n"
    "living in volcanic craters have turned\n"
    "Magcargo's bodies into magma.");

const u8 gSwinubPokedexText[] = _(
    "It roots for food by rubbing its snout\n"
    "against the ground. Its favorite food is a\n"
    "mushroom that grows under dried grass.\n"
    "It occasionally roots out hot springs.");

const u8 gPiloswinePokedexText[] = _(
    "A Piloswine is covered by a thick coat\n"
    "of long hair for enduring freezing cold.\n"
    "It uses its tusks to dig up food that has\n"
    "been buried under ice.");

const u8 gCorsolaPokedexText[] = _(
    "Corsola live in warm southern seas.\n"
    "If the sea becomes polluted, the beautiful\n"
    "coral stalks become discolored and crumble\n"
    "away in tatters.");

const u8 gRemoraidPokedexText[] = _(
    "A Remoraid uses its abdominal muscles\n"
    "to forcefully expel swallowed water, then\n"
    "shoot down flying prey. When evolution\n"
    "approaches, it travels down rivers.");

const u8 gOctilleryPokedexText[] = _(
    "It ensnares its foe with its suction-\n"
    "cupped tentacles before delivering the\n"
    "finishing blow. If the foe turns out to be\n"
    "too strong, it spews ink to escape.");

const u8 gDelibirdPokedexText[] = _(
    "It carries food bundled up in its tail.\n"
    "There was a famous explorer who\n"
    "managed to scale Mt. Everest thanks\n"
    "to a Delibird sharing its food.");

const u8 gMantinePokedexText[] = _(
    "On sunny days, schools of Mantine can be\n"
    "seen elegantly leaping over the waves.\n"
    "It is not bothered by the Remoraid that\n"
    "hitches rides.");

const u8 gSkarmoryPokedexText[] = _(
    "A Pokémon that has a body and wings of\n"
    "steel. People in the past used feathers\n"
    "fallen from Skarmory to make swords and\n"
    "knives.");

const u8 gHoundourPokedexText[] = _(
    "Houndour communicate with each other\n"
    "using a variety of cries to corner their\n"
    "prey. This Pokémon's remarkable teamwork\n"
    "is simply unparalleled.");

const u8 gHoundoomPokedexText[] = _(
    "In a Houndoom pack, the one with its horns\n"
    "raked sharply back serves a leadership\n"
    "role. They choose their leader by fighting\n"
    "among themselves.");

const u8 gKingdraPokedexText[] = _(
    "It sleeps quietly, deep on the seafloor.\n"
    "When it comes up to the surface, it\n"
    "creates a huge whirlpool that can swallow\n"
    "even ships.");

const u8 gPhanpyPokedexText[] = _(
    "Phanpy's big ears serve as broad fans.\n"
    "When it becomes hot, it flaps the ears\n"
    "busily to cool down. Even the young are\n"
    "very strong.");

const u8 gDonphanPokedexText[] = _(
    "A Donphan is so strong it can easily haul\n"
    "a dump truck. Its hide has toughened to a\n"
    "rock-hard state. An ordinary sort of\n"
    "attack won't even leave a scratch.");

const u8 gPorygon2PokedexText[] = _(
    "It was created by humans using the power\n"
    "of science. It has been given artificial\n"
    "intelligence that enables it to learn new\n"
    "gestures and emotions on its own.");

const u8 gStantlerPokedexText[] = _(
    "Stantler's magnificent antlers were\n"
    "once traded at high prices as works of art.\n"
    "As a result, this Pokémon was hunted\n"
    "close to extinction.");

const u8 gSmearglePokedexText[] = _(
    "A Smeargle marks its territory using a\n"
    "fluid that leaks out from the tip of its\n"
    "tail. About 5,000 different marks left by\n"
    "this Pokémon have been found.");

const u8 gTyroguePokedexText[] = _(
    "Tyrogue become stressed out if they do\n"
    "not get to train every day. When raising\n"
    "this Pokémon, the Trainer must establish\n"
    "a regular training schedule.");

const u8 gHitmontopPokedexText[] = _(
    "Its technique of kicking while spinning is\n"
    "a remarkable mix of both offense and\n"
    "defense. Hitmontop travel faster\n"
    "spinning than they do walking.");

const u8 gSmoochumPokedexText[] = _(
    "It actively runs about, but also falls\n"
    "often. Whenever it falls, it will check its\n"
    "reflection on a lake's surface to make\n"
    "sure its face hasn't become dirty.");

const u8 gElekidPokedexText[] = _(
    "If it touches metal and discharges the\n"
    "electricity it has stored in its body, an\n"
    "Elekid begins swinging its arms in circles\n"
    "to recharge itself.");

const u8 gMagbyPokedexText[] = _(
    "If a Magby is spouting yellow flames from\n"
    "its mouth, it is in good health. When it is\n"
    "fatigued, black smoke will be mixed in with\n"
    "the flames.");

const u8 gMiltankPokedexText[] = _(
    "It gives over five gallons of milk daily.\n"
    "Its sweet milk is enjoyed by children and\n"
    "grown-ups alike. People who can't drink\n"
    "milk turn it into yogurt and eat it instead.");

const u8 gBlisseyPokedexText[] = _(
    "If it senses sadness with its fluffy fur,\n"
    "a Blissey will rush over to the sad person,\n"
    "however far away, to share an egg of\n"
    "happiness that brings a smile to any face.");

const u8 gRaikouPokedexText[] = _(
    "Raikou embodies the speed of lightning.\n"
    "Its roars send shock waves shuddering\n"
    "through the air and ground as if\n"
    "lightning bolts were crashing down.");

const u8 gEnteiPokedexText[] = _(
    "Entei embodies the passion of magma.\n"
    "It is thought to have been born in the\n"
    "eruption of a volcano. It blasts fire that\n"
    "consumes all that it touches.");

const u8 gSuicunePokedexText[] = _(
    "Suicune embodies the compassion of\n"
    "a pure spring of water. It runs across\n"
    "the land with gliding elegance. It has the\n"
    "power to purify dirty water.");

const u8 gLarvitarPokedexText[] = _(
    "A Larvitar is born deep under the ground.\n"
    "It must eat its way through the soil above\n"
    "and reach the surface for it to see its\n"
    "parents' faces.");

const u8 gPupitarPokedexText[] = _(
    "A Pupitar creates a gas inside its body\n"
    "that it ejects under compression to propel\n"
    "itself like a jet. Its body can withstand\n"
    "a collision with solid steel.");

const u8 gTyranitarPokedexText[] = _(
    "A Tyranitar is so overwhelmingly powerful,\n"
    "it can bring down a whole mountain to make\n"
    "its nest. It roams in mountains seeking\n"
    "new opponents to fight.");

const u8 gLugiaPokedexText[] = _(
    "Lugia is so powerful even a light\n"
    "fluttering of its wings can blow apart\n"
    "houses. As a result, it chooses to live out\n"
    "of sight deep under the sea.");

const u8 gHoOhPokedexText[] = _(
    "Its feathers--which glow in seven colors\n"
    "depending on the angle at which they are\n"
    "struck by light--are thought to bring joy.\n"
    "It is said to live at the foot of a rainbow.");

const u8 gCelebiPokedexText[] = _(
    "This Pokémon came from the future by\n"
    "crossing over time. It is thought that so\n"
    "long as Celebi appears, a bright and\n"
    "shining future awaits us.");

const u8 gTreeckoPokedexText[] = _(
    "It makes its nest in a giant tree in the\n"
    "forest. It ferociously guards against\n"
    "anything nearing its territory. It is said\n"
    "to be the protector of the forest's trees.");

const u8 gGrovylePokedexText[] = _(
    "Leaves grow out of this Pokémon's body.\n"
    "They help obscure a Grovyle from the eyes\n"
    "of its enemies while it is in a thickly\n"
    "overgrown forest.");

const u8 gSceptilePokedexText[] = _(
    "In the jungle, its power is without equal.\n"
    "This Pokémon carefully grows trees and\n"
    "plants. It regulates its body temperature\n"
    "by basking in sunlight.");

const u8 gTorchicPokedexText[] = _(
    "If attacked, it strikes back by spitting\n"
    "balls of fire it forms in its stomach.\n"
    "A Torchic dislikes darkness because it\n"
    "can't see its surroundings.");

const u8 gCombuskenPokedexText[] = _(
    "It lashes out with 10 kicks per second.\n"
    "Its strong fighting instinct compels it\n"
    "to keep up its offensive until the\n"
    "opponent gives up.");

const u8 gBlazikenPokedexText[] = _(
    "It learns martial arts that use punches\n"
    "and kicks. Every several years, its old\n"
    "feathers burn off, and new, supple\n"
    "feathers grow back in their place.");

const u8 gMudkipPokedexText[] = _(
    "On land, it can powerfully lift large\n"
    "boulders by planting its four feet and\n"
    "heaving. It sleeps by burying itself in soil\n"
    "at the water's edge.");

const u8 gMarshtompPokedexText[] = _(
    "Its toughened hind legs enable it to stand\n"
    "upright. Because it weakens if its skin\n"
    "dries out, it replenishes fluids by playing\n"
    "in mud.");

const u8 gSwampertPokedexText[] = _(
    "If it senses the approach of a storm and\n"
    "a tidal wave, it protects its seaside nest\n"
    "by piling up boulders. It swims as fast as\n"
    "a jet ski.");

const u8 gPoochyenaPokedexText[] = _(
    "It savagely threatens foes with bared\n"
    "fangs. It chases after fleeing targets\n"
    "tenaciously. It turns tail and runs,\n"
    "however, if the foe strikes back.");

const u8 gMightyenaPokedexText[] = _(
    "In the wild, Mightyena live in a pack.\n"
    "They never defy their leader's orders.\n"
    "They defeat foes with perfectly\n"
    "coordinated teamwork.");

const u8 gZigzagoonPokedexText[] = _(
    "Rubbing its nose against the ground, it\n"
    "always wanders about back and forth in\n"
    "search of something. It is distinguished\n"
    "by the zigzag footprints it leaves.");

const u8 gLinoonePokedexText[] = _(
    "It is exceedingly fast if it only has to run\n"
    "in a straight line. When it spots pond-\n"
    "dwelling prey underwater, it quickly leaps\n"
    "in and catches it with its sharp claws.");

const u8 gWurmplePokedexText[] = _(
    "It sticks to tree branches and eats\n"
    "leaves. The thread it spits from its mouth,\n"
    "which becomes gooey when it touches\n"
    "air, slows the movement of its foes.");

const u8 gSilcoonPokedexText[] = _(
    "It prepares for evolution using the\n"
    "energy it stored while it was a Wurmple.\n"
    "It keeps watch over the surroundings with\n"
    "its two eyes.");

const u8 gBeautiflyPokedexText[] = _(
    "Its colorfully patterned wings are its\n"
    "most prominent feature. It flies through\n"
    "flower-covered fields collecting pollen.\n"
    "It attacks ferociously when angered.");

const u8 gCascoonPokedexText[] = _(
    "To avoid detection by its enemies, it hides\n"
    "motionlessly beneath large leaves and in\n"
    "the gaps of branches. It also attaches\n"
    "dead leaves to its body for camouflage.");

const u8 gDustoxPokedexText[] = _(
    "It is a nocturnal Pokémon that flies from\n"
    "fields and mountains to the attraction of\n"
    "streetlights at night. It looses highly\n"
    "toxic powder from its wings.");

const u8 gLotadPokedexText[] = _(
    "This Pokémon lives in ponds with clean\n"
    "water. It is known to ferry small Pokémon\n"
    "across ponds by carrying them on the\n"
    "broad leaf on its head.");

const u8 gLombrePokedexText[] = _(
    "In the evening, it takes great delight in\n"
    "popping out of rivers and startling people.\n"
    "It feeds on aquatic moss that grows on\n"
    "rocks in the riverbed.");

const u8 gLudicoloPokedexText[] = _(
    "When it hears festive music, all the cells\n"
    "in its body become stimulated, and it\n"
    "begins moving in rhythm. It does not\n"
    "quail even when it faces a tough opponent.");

const u8 gSeedotPokedexText[] = _(
    "It hangs off branches and absorbs\n"
    "nutrients. When it finishes eating, its\n"
    "body becomes so heavy that it drops to\n"
    "the ground with a thump.");

const u8 gNuzleafPokedexText[] = _(
    "A forest-dwelling Pokémon that is skilled\n"
    "at climbing trees. Its long and pointed\n"
    "nose is its weak point. It loses power if\n"
    "the nose is gripped.");

const u8 gShiftryPokedexText[] = _(
    "It is said to arrive on chilly, wintry winds.\n"
    "Feared from long ago as the guardian of\n"
    "forests, this Pokémon lives in a deep\n"
    "forest where people do not venture.");

const u8 gTaillowPokedexText[] = _(
    "Although it is small, it is very courageous.\n"
    "It will take on a larger Skarmory on an\n"
    "equal footing. However, its will weakens if\n"
    "it becomes hungry.");

const u8 gSwellowPokedexText[] = _(
    "A Swellow dives upon prey from far above.\n"
    "It never misses its targets. It takes to\n"
    "the skies in search of lands with a warm\n"
    "climate.");

const u8 gWingullPokedexText[] = _(
    "It makes its nest on a sheer cliff at the\n"
    "edge of the sea. It has trouble keeping\n"
    "its wings flapping in flight. Instead, it\n"
    "soars on updrafts.");

const u8 gPelipperPokedexText[] = _(
    "It skims the tops of waves as it flies.\n"
    "When it spots prey, it uses its large beak\n"
    "to scoop up the victim with water.\n"
    "It protects its eggs in its beak.");

const u8 gRaltsPokedexText[] = _(
    "A Ralts has the power to sense the\n"
    "emotions of people and Pokémon with the\n"
    "horns on its head. It takes cover if it\n"
    "senses any hostility.");

const u8 gKirliaPokedexText[] = _(
    "A Kirlia has the psychic power to create\n"
    "a rip in the dimensions and see into the\n"
    "future. It is said to dance with pleasure\n"
    "on sunny mornings.");

const u8 gGardevoirPokedexText[] = _(
    "It apparently does not feel the pull of\n"
    "gravity because it supports itself with\n"
    "psychic power. It will give its life to\n"
    "protect its Trainer.");

const u8 gSurskitPokedexText[] = _(
    "They gather on puddles after evening\n"
    "downpours, gliding across the surface\n"
    "of water as if sliding. It secretes honey\n"
    "with a sweet aroma from its head.");

const u8 gMasquerainPokedexText[] = _(
    "It intimidates foes with the large eyelike\n"
    "patterns on its antennae. Because it can't\n"
    "fly if its wings get wet, it shelters itself\n"
    "from rain under large trees and eaves.");

const u8 gShroomishPokedexText[] = _(
    "It loves to eat damp, composted soil in\n"
    "forests. If you enter a forest after a\n"
    "long rain, you can see many Shroomish\n"
    "feasting on composted soil.");

const u8 gBreloomPokedexText[] = _(
    "It scatters spores from holes in the cap\n"
    "on its head. It loves warm and humid\n"
    "climates. It feeds on trees and plants in\n"
    "fields and forests.");

const u8 gSlakothPokedexText[] = _(
    "It sleeps virtually all day and night long.\n"
    "It doesn't change its nest its entire life,\n"
    "but it sometimes travels great distances\n"
    "by swimming in rivers.");

const u8 gVigorothPokedexText[] = _(
    "It can't keep still because its blood boils\n"
    "with energy. It runs through the fields\n"
    "and mountains all day to calm itself. If it\n"
    "doesn't, it can't sleep at night.");

const u8 gSlakingPokedexText[] = _(
    "Hordes of Slaking gather around trees\n"
    "when fruits come into season. They wait\n"
    "around patiently for ripened fruits to fall\n"
    "out of the trees.");

const u8 gNincadaPokedexText[] = _(
    "It makes its nest at the roots of a mighty\n"
    "tree. Using its whiskerlike antennae, it\n"
    "probes its surroundings in the\n"
    "pitch-black darkness of soil.");

const u8 gNinjaskPokedexText[] = _(
    "Because it darts about vigorously at high\n"
    "speed, it is very difficult to see. Hearing\n"
    "its distinctive cries for too long induces\n"
    "a headache.");

const u8 gShedinjaPokedexText[] = _(
    "A peculiar Pokémon that floats in air even\n"
    "though its wings remain completely still.\n"
    "The inside of its body is hollow and\n"
    "utterly dark.");

const u8 gWhismurPokedexText[] = _(
    "Its cries equal a jet plane in volume.\n"
    "It inhales through its ear canals. Because\n"
    "of this system, it can cry continually\n"
    "without having to catch its breath.");

const u8 gLoudredPokedexText[] = _(
    "It positions the round speakers on its\n"
    "head to assail foes with ultrasonic waves\n"
    "at massive volume. It builds power by\n"
    "stomping the ground.");

const u8 gExploudPokedexText[] = _(
    "It has sound-generating organs all over\n"
    "its body. It communicates with others by\n"
    "adjusting the tone and volume of the cries\n"
    "it emits.");

const u8 gMakuhitaPokedexText[] = _(
    "It loves to toughen up its body above all\n"
    "else. If you hear quaking rumbles in a cave,\n"
    "it is the sound of Makuhita undertaking\n"
    "strenuous training.");

const u8 gHariyamaPokedexText[] = _(
    "It has the habit of challenging others\n"
    "without hesitation to tests of strength.\n"
    "It's been known to stand on train tracks\n"
    "and stop trains using forearm thrusts.");

const u8 gAzurillPokedexText[] = _(
    "Its tail, which is packed with nutrition,\n"
    "is very bouncy like a rubber ball. On sunny\n"
    "days they gather at the edge of water and\n"
    "splash about for fun.");

const u8 gNosepassPokedexText[] = _(
    "Its body emits a powerful magnetism.\n"
    "It feeds on prey that is pulled in by the\n"
    "force. Its magnetism is stronger in cold\n"
    "seasons.");

const u8 gSkittyPokedexText[] = _(
    "A Skitty's adorably cute behavior makes it\n"
    "highly popular. In battle, it makes its tail\n"
    "puff out. It threatens foes with a sharp\n"
    "growl.");

const u8 gDelcattyPokedexText[] = _(
    "Rather than keeping a permanent lair,\n"
    "it habitually seeks comfortable spots and\n"
    "sleeps there. It is nocturnal and becomes\n"
    "active at dusk.");

const u8 gSableyePokedexText[] = _(
    "It digs branching holes in caves using its\n"
    "sharp claws in search of food--raw gems.\n"
    "A Sableye lurks in darkness and is seen\n"
    "only rarely.");

const u8 gMawilePokedexText[] = _(
    "Its giant jaws are actually steel horns\n"
    "that transformed. It fools foes into\n"
    "complacency with its adorable gestures,\n"
    "then chomps them with its huge jaws.");

const u8 gAronPokedexText[] = _(
    "A Pokémon that is clad in steel armor.\n"
    "A new suit of armor is made when it evolves.\n"
    "The old, discarded armor is salvaged as\n"
    "metal for making iron products.");

const u8 gLaironPokedexText[] = _(
    "When two Lairon meet in the wild, they\n"
    "fight for territory by bashing into each\n"
    "other with their steel bodies. The sound\n"
    "of their collision carries for miles.");

const u8 gAggronPokedexText[] = _(
    "Its iron horns grow longer a little at\n"
    "a time. They are used to determine the\n"
    "Aggron's age. The gouges in its armor are\n"
    "worn with pride as mementos from battles.");

const u8 gMedititePokedexText[] = _(
    "It continually meditates for hours every\n"
    "day. As a result of rigorous and dedicated\n"
    "yoga training, it has tempered its\n"
    "spiritual power so much it can fly.");

const u8 gMedichamPokedexText[] = _(
    "Through crushingly harsh yoga training, it\n"
    "gained the power to foretell its foe's\n"
    "actions. It battles with elegant, dance-\n"
    "like movement.");

const u8 gElectrikePokedexText[] = _(
    "It generates electricity using friction\n"
    "from the atmosphere. In seasons with\n"
    "especially arid air, its entire body blazes\n"
    "with violent showers of sparks.");

const u8 gManectricPokedexText[] = _(
    "Because lightning falls in their vicinities,\n"
    "Manectric were thought to have been born\n"
    "from lightning. In battle, they create\n"
    "thunderclouds.");

const u8 gPluslePokedexText[] = _(
    "It has the trait of cheering on its fellow\n"
    "Pokémon. By shorting out the electricity\n"
    "it releases from its paws, it creates\n"
    "pom-poms for cheering.");

const u8 gMinunPokedexText[] = _(
    "At a meeting of Pokémon academics, it was\n"
    "announced that simultaneous exposure to\n"
    "electricity from a Plusle and Minun will\n"
    "promote circulation and boost vitality.");

const u8 gVolbeatPokedexText[] = _(
    "With their taillights lit, Volbeat fly in\n"
    "a swarm, drawing geometric designs in the\n"
    "night sky. They move their nests if their\n"
    "pond water becomes dirty.");

const u8 gIllumisePokedexText[] = _(
    "A nocturnal Pokémon that becomes active\n"
    "upon nightfall. It leads a Volbeat swarm\n"
    "to draw patterns in the night sky. Over 200\n"
    "different patterns have been confirmed.");

const u8 gRoseliaPokedexText[] = _(
    "A Roselia that drinks nutritionally rich\n"
    "springwater blooms with lovely flowers.\n"
    "The fragrance of its flowers has the\n"
    "effect of making its foes careless.");

const u8 gGulpinPokedexText[] = _(
    "This Pokémon's stomach fluid can even\n"
    "digest scrap iron. In one gulp, it can\n"
    "swallow something that is as large as\n"
    "itself.");

const u8 gSwalotPokedexText[] = _(
    "Its powerful stomach acid is capable of\n"
    "digesting almost anything. The one thing\n"
    "in the whole world a Swalot can't digest is\n"
    "its own stomach.");

const u8 gCarvanhaPokedexText[] = _(
    "Carvanha attack ships in swarms, making\n"
    "them sink. Although it is said to be a very\n"
    "vicious Pokémon, it timidly flees as soon\n"
    "as it finds itself alone.");

const u8 gSharpedoPokedexText[] = _(
    "The vicious and sly gangster of the sea.\n"
    "Its skin is specially textured to minimize\n"
    "drag in water. Its speed tops out at over\n"
    "75 miles per hour.");

const u8 gWailmerPokedexText[] = _(
    "While this Pokémon usually lives in the sea,\n"
    "it can survive on land, although not too\n"
    "long. It loses vitality if its body becomes\n"
    "dried out.");

const u8 gWailordPokedexText[] = _(
    "It breathes through nostrils that it\n"
    "raises above the sea. By inhaling to its\n"
    "maximum capacity, a Wailord can dive close\n"
    "to 10,000 feet beneath the waves.");

const u8 gNumelPokedexText[] = _(
    "A Numel stores boiling magma in the hump\n"
    "on its back. It is a hardy Pokémon that can\n"
    "transport a 220-pound load. It has served\n"
    "humans at work since long ago.");

const u8 gCameruptPokedexText[] = _(
    "A Pokémon that lives in the crater of\n"
    "a volcano. Every 10 years, the volcanoes\n"
    "on its back erupt violently. Research is\n"
    "under way on the cause of eruption.");

const u8 gTorkoalPokedexText[] = _(
    "It battles using energy it gets from\n"
    "burning coal. When loosing smoke from its\n"
    "nostrils, it lets off a sound that is\n"
    "similar to a locomotive's horn.");

const u8 gSpoinkPokedexText[] = _(
    "A Pokémon that manipulates psychic power\n"
    "at will. It doesn't stop bouncing even when\n"
    "it is asleep. It loves eating mushrooms\n"
    "that grow underground.");

const u8 gGrumpigPokedexText[] = _(
    "It stores power in the black pearls on its\n"
    "forehead. When it uses psychic power, it\n"
    "performs an odd dance step. Its style of\n"
    "dancing became hugely popular overseas.");

const u8 gSpindaPokedexText[] = _(
    "It is distinguished by a pattern of\n"
    "spots that is always different. Its\n"
    "unsteady, tottering walk has the\n"
    "effect of fouling its foe's aim.");

const u8 gTrapinchPokedexText[] = _(
    "Its big jaws crunch through boulders.\n"
    "Because its head is so big, it has a hard\n"
    "time getting back upright if it tips over\n"
    "onto its back.");

const u8 gVibravaPokedexText[] = _(
    "It looses ultrasonic waves by rubbing its\n"
    "wings together. Since a Vibrava's wings\n"
    "are still in the process of growing, it can\n"
    "only fly short distances.");

const u8 gFlygonPokedexText[] = _(
    "The flapping of its wings sounds like\n"
    "singing. To prevent detection by enemies,\n"
    "it hides itself by flapping up a cloud of\n"
    "desert sand.");

const u8 gCacneaPokedexText[] = _(
    "Cacnea live in deserts with virtually no\n"
    "rainfall. It battles by swinging its thick,\n"
    "spiked arms. Once a year, a yellow flower\n"
    "blooms.");

const u8 gCacturnePokedexText[] = _(
    "After spending thousands of years in\n"
    "harsh deserts, its blood transformed into\n"
    "the same substances as sand. It is\n"
    "nocturnal, so it hunts at night.");

const u8 gSwabluPokedexText[] = _(
    "A Pokémon that has wings like cottony\n"
    "clouds. After enduring winter, in which\n"
    "little food is available, Swablu flocks\n"
    "move closer to towns in the spring.");

const u8 gAltariaPokedexText[] = _(
    "It hums in a beautiful soprano voice.\n"
    "It flies among white clouds in the blue\n"
    "sky. It launches intensely hot fireballs\n"
    "from its mouth.");

const u8 gZangoosePokedexText[] = _(
    "When it battles, it stands on its hind legs\n"
    "and attacks with its sharply clawed\n"
    "forelegs. Its fur bristles if it encounters\n"
    "any Seviper.");

const u8 gSeviperPokedexText[] = _(
    "Seviper and Zangoose are eternal rivals.\n"
    "It counters a Zangoose's dazzling agility\n"
    "with its swordlike tail, which also oozes\n"
    "a horrible poison.");

const u8 gLunatonePokedexText[] = _(
    "It becomes very active on the night of\n"
    "a full moon. This Pokémon was first\n"
    "discovered 40 years ago at the site of\n"
    "a meteor strike.");

const u8 gSolrockPokedexText[] = _(
    "Solar energy is the source of this\n"
    "Pokémon's power. On sunny days, groups of\n"
    "Solrock line up facing the sun and absorb\n"
    "its light.");

const u8 gBarboachPokedexText[] = _(
    "Its body is covered with a slimy film.\n"
    "The film acts as a barrier to prevent germs\n"
    "in muddy water from entering the\n"
    "Barboach's body.");

const u8 gWhiscashPokedexText[] = _(
    "Mysteriously, it can foretell earthquakes.\n"
    "In the daytime, it sleeps in mud at the\n"
    "bottom of a pond. When it awakens, it\n"
    "continually feeds throughout the night.");

const u8 gCorphishPokedexText[] = _(
    "Once it grips prey with its large pincers,\n"
    "it will never let go, no matter what.\n"
    "It is a hardy Pokémon that can thrive\n"
    "in any environment.");

const u8 gCrawdauntPokedexText[] = _(
    "A brutish Pokémon that loves to battle.\n"
    "A veteran Crawdaunt that has prevailed in\n"
    "hundreds of battles has giant pincers\n"
    "marked with countless scars.");

const u8 gBaltoyPokedexText[] = _(
    "A Baltoy moves by spinning on its single\n"
    "foot. It has been depicted in murals\n"
    "adorning the walls of a once-bustling city\n"
    "in an ancient age.");

const u8 gClaydolPokedexText[] = _(
    "A Claydol sleeps while hovering in midair.\n"
    "Its arms are separate from its body.\n"
    "They are kept floating by the Pokémon's\n"
    "manipulation of psychic power.");

const u8 gLileepPokedexText[] = _(
    "It disguises itself as seaweed by making\n"
    "its tentacles sway. Unsuspecting prey\n"
    "that come too close are swallowed whole.\n"
    "It became extinct 100 million years ago.");

const u8 gCradilyPokedexText[] = _(
    "It drags its heavy body along the\n"
    "seafloor. It makes its nest in the shallows\n"
    "of warm seas. Cradily can be seen on\n"
    "beaches when the tide goes out.");

const u8 gAnorithPokedexText[] = _(
    "It was resurrected from a fossil using the\n"
    "power of science. It swims by undulating\n"
    "the eight wings at its sides. They were\n"
    "feet that adapted to life in the sea.");

const u8 gArmaldoPokedexText[] = _(
    "Armaldo usually lives on land. However,\n"
    "when it hunts for prey, it dives beneath\n"
    "the ocean. It swims around using its two\n"
    "large wings.");

const u8 gFeebasPokedexText[] = _(
    "Feebas live in ponds that are heavily\n"
    "infested with weeds. Because of its\n"
    "hopelessly shabby appearance, it\n"
    "seems as if few Trainers raise it.");

const u8 gMiloticPokedexText[] = _(
    "It is said to live at the bottom of\n"
    "large lakes. Considered to be the most\n"
    "beautiful of all Pokémon, it has been\n"
    "depicted in paintings and statues.");

const u8 gCastformPokedexText[] = _(
    "It alters its form depending on the\n"
    "weather. Changes in the climate such as\n"
    "the temperature and humidity appear to\n"
    "affect its cellular structure.");

const u8 gKecleonPokedexText[] = _(
    "A Pokémon that has the ability to alter its\n"
    "body colors to match its surroundings.\n"
    "A Kecleon reverts to its original colors if\n"
    "it is startled.");

const u8 gShuppetPokedexText[] = _(
    "This Pokémon roams about deep in the\n"
    "night seeking such negative emotions as\n"
    "grudges and envy. It retreats to its nest\n"
    "when the sun begins to rise.");

const u8 gBanettePokedexText[] = _(
    "An abandoned plush doll became this\n"
    "Pokémon. They are said to live in garbage\n"
    "dumps and wander about in search of the\n"
    "children that threw them away.");

const u8 gDuskullPokedexText[] = _(
    "A glare from its single scarlet eye makes\n"
    "even burly grown-ups freeze in utter fear.\n"
    "It is a nocturnal Pokémon that roams\n"
    "about under the cloak of darkness.");

const u8 gDusclopsPokedexText[] = _(
    "It is thought that its body is hollow with\n"
    "only a spectral ball of fire burning inside.\n"
    "However, no one has been able to\n"
    "confirm this theory as fact.");

const u8 gTropiusPokedexText[] = _(
    "It flies by flapping its broad leaves.\n"
    "The bunch of fruit that grows around its\n"
    "neck is deliciously sweet. In the spring,\n"
    "it scatters pollen from its neck.");

const u8 gChimechoPokedexText[] = _(
    "They fly about very actively when the hot\n"
    "season arrives. They communicate among\n"
    "themselves using seven different and\n"
    "distinguishing cries.");

const u8 gAbsolPokedexText[] = _(
    "It sharply senses even subtle changes in\n"
    "the sky and the land to predict natural\n"
    "disasters. It is a long-lived Pokémon that\n"
    "has a life-span of 100 years.");

const u8 gWynautPokedexText[] = _(
    "A Wynaut loves to eat sweet fruits.\n"
    "It cleverly picks fruits using its earlike\n"
    "arms. They gather in fruit gardens, drawn\n"
    "by the fragrance.");

const u8 gSnoruntPokedexText[] = _(
    "They tend to move about in groups of\n"
    "around five Snorunt. In snowy regions,\n"
    "it is said that when they are seen late at\n"
    "night, snowfall will arrive by morning.");

const u8 gGlaliePokedexText[] = _(
    "A Glalie has the power to instantaneously\n"
    "freeze moisture in the atmosphere.\n"
    "A dazzling cloud of diamondlike ice\n"
    "crystals forms around its body.");

const u8 gSphealPokedexText[] = _(
    "It is completely covered with plushy fur.\n"
    "As a result, it never feels the cold even\n"
    "when it is rolling about on ice floes or\n"
    "diving in the sea.");

const u8 gSealeoPokedexText[] = _(
    "Sealeo live in herds on ice floes. Using its\n"
    "powerful flippers, it shatters ice.\n"
    "It dives into the sea to hunt prey five\n"
    "times a day.");

const u8 gWalreinPokedexText[] = _(
    "To protect its herd, the leader battles\n"
    "anything that invades its territory, even\n"
    "at the cost of its life. Its tusks may snap\n"
    "off in battle.");

const u8 gClamperlPokedexText[] = _(
    "A Clamperl slams its shell closed on prey\n"
    "to prevent escape. The pearl it creates\n"
    "upon evolution is said to be infused with\n"
    "a mysterious energy.");

const u8 gHuntailPokedexText[] = _(
    "To withstand the crushing pressure of\n"
    "water deep under the sea, its spine is very\n"
    "thick and sturdy. Its tail, which is shaped\n"
    "like a small fish, has eyes that light up.");

const u8 gGorebyssPokedexText[] = _(
    "A Gorebyss siphons the body fluids of prey\n"
    "through its thin, tubular mouth. Its light\n"
    "pink body color turns vivid when it\n"
    "finishes feeding.");

const u8 gRelicanthPokedexText[] = _(
    "A Pokémon that was once believed to have\n"
    "been extinct. The species has not changed\n"
    "its form for 100 million years. It walks on\n"
    "the seafloor using its pectoral fins.");

const u8 gLuvdiscPokedexText[] = _(
    "Luvdisc make the branches of Corsola\n"
    "their nests. There is a custom from long\n"
    "ago of giving a Luvdisc as a gift to\n"
    "express one's feelings of love.");

const u8 gBagonPokedexText[] = _(
    "Although it is small, this Pokémon is very\n"
    "powerful because its body is a bundle of\n"
    "muscles. It launches head-butts with its\n"
    "ironlike skull.");

const u8 gShelgonPokedexText[] = _(
    "It hardly eats while it awaits evolution.\n"
    "It becomes hardier by enduring hunger.\n"
    "Its shell peels off the instant it begins\n"
    "to evolve.");

const u8 gSalamencePokedexText[] = _(
    "After many long years, its cellular\n"
    "structure underwent a sudden mutation to\n"
    "grow wings. When angered, it loses all\n"
    "thought and rampages out of control.");

const u8 gBeldumPokedexText[] = _(
    "When Beldum gather in a swarm, they move\n"
    "in perfect unison as if they were but one\n"
    "Pokémon. They communicate with each other\n"
    "using brain waves.");

const u8 gMetangPokedexText[] = _(
    "The claws tipping its arms pack the\n"
    "destructive power to tear through thick\n"
    "iron sheets as if they were silk. It flies\n"
    "at over 60 miles per hour.");

const u8 gMetagrossPokedexText[] = _(
    "Metagross has four brains that are joined\n"
    "by a complex neural network. As a result of\n"
    "integration, this Pokémon is smarter than\n"
    "a supercomputer.");

const u8 gRegirockPokedexText[] = _(
    "A Pokémon that is made entirely of rocks\n"
    "and boulders. If parts of its body chip off\n"
    "in battle, Regirock repairs itself by\n"
    "adding new rocks.");

const u8 gRegicePokedexText[] = _(
    "Its entire body is made of Antarctic ice.\n"
    "After extensive studies, researchers\n"
    "believe the ice was formed during an\n"
    "ice age.");

const u8 gRegisteelPokedexText[] = _(
    "Its body is harder than any other kind of\n"
    "metal. The body metal is composed of a\n"
    "mysterious substance. Not only is it hard,\n"
    "it shrinks and stretches flexibly.");

const u8 gLatiasPokedexText[] = _(
    "They make a small herd of only several\n"
    "members. They rarely make contact with\n"
    "people or other Pokémon. They disappear\n"
    "if they sense enemies.");

const u8 gLatiosPokedexText[] = _(
    "Even in hiding, it can detect the locations\n"
    "of others and sense their emotions since\n"
    "it has telepathy. Its intelligence allows\n"
    "it to understand human languages.");

const u8 gKyogrePokedexText[] = _(
    "Kyogre has appeared in mythology as the\n"
    "creator of the sea. After long years of\n"
    "feuding with Groudon, it took to sleep at\n"
    "the bottom of the sea.");

const u8 gGroudonPokedexText[] = _(
    "Groudon has appeared in mythology as the\n"
    "creator of the land. It sleeps in magma\n"
    "underground and is said to make volcanoes\n"
    "erupt on awakening.");

const u8 gRayquazaPokedexText[] = _(
    "A Pokémon that flies endlessly in the\n"
    "ozone layer. It is said it would descend\n"
    "to the ground if Kyogre and Groudon\n"
    "were to fight.");

const u8 gJirachiPokedexText[] = _(
    "Jirachi is said to make wishes come true.\n"
    "While it sleeps, a tough crystalline shell\n"
    "envelops the body to protect it from\n"
    "enemies.");

const u8 gDeoxysPokedexText[] = _(
    "Deoxys emerged from a virus that came from\n"
    "space. It is highly intelligent and wields\n"
    "psychokinetic powers. It shoots lasers\n"
    "from the crystalline organ on its chest.");

const u8 gTurtwigPokedexText[] = _(
    "It uses its whole body to photosynthesize\n"
    "when exposed to sunlight. Its shell is made\n"
    "from hardened soil.");

const u8 gGrotlePokedexText[] = _(
    "It knows where pure water wells up. It\n"
    "carries fellow Pokémon there on its back.");

const u8 gTorterraPokedexText[] = _(
    "Some Pokémon are born on a Torterra’s back\n"
    "and spend their entire life there.");

const u8 gChimcharPokedexText[] = _(
    "The gas made in its belly burns from its\n"
    "rear end. The fire burns weakly when it\n"
    "feels sick.");

const u8 gMonfernoPokedexText[] = _(
    "A bigger fire on its tail and a brighter blue\n"
    "pattern on its face means it holds a higher\n"
    "rank in its troop.");

const u8 gInfernapePokedexText[] = _(
    "It uses unique fighting moves with fire on\n"
    "its hands and feet. It will take on any\n"
    "opponent.");

const u8 gPiplupPokedexText[] = _(
    "It doesn’t like to be taken care of. It’s\n"
    "difficult to bond with since it won’t listen\n"
    "to its Trainer.");

const u8 gPrinplupPokedexText[] = _(
    "It lives a solitary life. Its wings deliver\n"
    "wicked blows that can snap even the\n"
    "thickest of trees in half with a single hit.");

const u8 gEmpoleonPokedexText[] = _(
    "It avoids unnecessary disputes, but it will\n"
    "mercilessly decimate anything that\n"
    "threatens the safety of its colony.");

const u8 gStarlyPokedexText[] = _(
    "Because they are weak individually, they\n"
    "form groups. However, they bicker if the\n"
    "group grows too big.");

const u8 gStaraviaPokedexText[] = _(
    "They maintain huge flocks, although fierce\n"
    "scuffles break out between various flocks.");

const u8 gStaraptorPokedexText[] = _(
    "It never stops attacking even if it is\n"
    "injured. It fusses over the shape of its\n"
    "comb.");

const u8 gBidoofPokedexText[] = _(
    "It constantly gnaws on logs and rocks to\n"
    "whittle down its front teeth. It nests\n"
    "alongside water.");

const u8 gBibarelPokedexText[] = _(
    "It makes its nest by damming streams with\n"
    "bark and mud. It is known as an industrious\n"
    "worker.");

const u8 gKricketotPokedexText[] = _(
    "When its antennae hit each other, it sounds\n"
    "like the music of a xylophone.");

const u8 gKricketunePokedexText[] = _(
    "There is a village that hosts a contest\n"
    "based on the amazingly variable cries of\n"
    "this Pokémon.");

const u8 gShinxPokedexText[] = _(
    "The extension and contraction of its\n"
    "muscles generates electricity. Its fur\n"
    "glows when it's in trouble.");

const u8 gLuxioPokedexText[] = _(
    "By gathering their tails together, they\n"
    "collectively generate powerful electricity\n"
    "from their claws.");

const u8 gLuxrayPokedexText[] = _(
    "Luxray's ability to see through objects\n"
    "comes in handy when it's scouting for\n"
    "danger.");

const u8 gBudewPokedexText[] = _(
    "This Pokémon is highly sensitive to\n"
    "temperature changes. When its bud starts\n"
    "to open, that means spring is right around\n"
    "the corner.");

const u8 gRoseradePokedexText[] = _(
    "The poison in its right hand is quick\n"
    "acting. The poison in its left hand is slow\n"
    "acting. Both are life threatening.");

const u8 gCranidosPokedexText[] = _(
    "Cranidos toughen up their already sturdy\n"
    "heads by headbutting one another.");

const u8 gRampardosPokedexText[] = _(
    "If two were to smash their heads together,\n"
    "their foot-thick skulls would keep them\n"
    "from fainting.");

const u8 gShieldonPokedexText[] = _(
    "It is outstandingly armored. As a result, it\n"
    "has no particular need to battle with\n"
    "others and can spend its time feeding on\n"
    "grass and berries.");

const u8 gBastiodonPokedexText[] = _(
    "Bastiodon live in herds. When assaulted by\n"
    "enemies, they line up side by side and use\n"
    "their hard faces to block attacks.");

const u8 gBurmyPokedexText[] = _(
    "If its cloak is broken in battle, it quickly\n"
    "remakes the cloak with materials nearby.");

const u8 gWormadamPokedexText[] = _(
    "When Burmy evolved, its cloak became a part\n"
    "of this Pokémon's body. The cloak is never\n"
    "shed.");

const u8 gMothimPokedexText[] = _(
    "It flutters around at night and steals\n"
    "honey from the Combee hive.");

const u8 gCombeePokedexText[] = _(
    "The trio is together from birth. It\n"
    "constantly gathers nectar from flowers to\n"
    "please Vespiquen.");

const u8 gVespiquenPokedexText[] = _(
    "It raises grubs in the holes in its body. It\n"
    "secretes pheromones to control Combee.");

const u8 gPachirisuPokedexText[] = _(
    "A pair may be seen rubbing their cheek\n"
    "pouches together in an effort to share\n"
    "stored electricity.");

const u8 gBuizelPokedexText[] = _(
    "It inflates its flotation sac, keeping its\n"
    "face above water in order to watch for prey\n"
    "movement.");

const u8 gFloatzelPokedexText[] = _(
    "It is a common sight around fishing ports.\n"
    "It is known to rescue people and help\n"
    "fishers carry what they caught.");

const u8 gCherubiPokedexText[] = _(
    "It nimbly dashes about to avoid getting\n"
    "pecked by bird Pokémon that would love to\n"
    "make off with its small, nutrient-rich\n"
    "storage ball.");

const u8 gCherrimPokedexText[] = _(
    "Its folded petals are pretty tough. Bird\n"
    "Pokémon can peck at them all they want, and\n"
    "Cherrim won't be bothered at all.");

const u8 gShellosPokedexText[] = _(
    "Its shape and coloration vary, depending\n"
    "on its habitat.");

const u8 gGastrodonPokedexText[] = _(
    "It apparently had a huge shell for\n"
    "protection in ancient times. It lives in\n"
    "shallow tidal pools.");

const u8 gAmbipomPokedexText[] = _(
    "Split into two, the tails are so adept at\n"
    "handling and doing things that Ambipom\n"
    "rarely uses its hands.");

const u8 gDrifloonPokedexText[] = _(
    "Because of the way it floats aimlessly, an\n"
    "old folktale calls it a “signpost for\n"
    "wandering spirits.”");

const u8 gDrifblimPokedexText[] = _(
    "They carry people and Pokémon, but the\n"
    "wind can catch them, so there can't be a\n"
    "fixed destination.");

const u8 gBunearyPokedexText[] = _(
    "Buneary can attack by rolling up their ears\n"
    "and then striking with the force created\n"
    "by unrolling them. This attack becomes\n"
    "stronger with training.");

const u8 gLopunnyPokedexText[] = _(
    "Once hot seasons are over, Lopunny's coat\n"
    "will be replaced with fur that holds a lot of\n"
    "insulating air in preparation for colder\n"
    "weather.");

const u8 gMismagiusPokedexText[] = _(
    "Its cries sound like incantations to\n"
    "torment the foe. It appears where you\n"
    "least expect it.");

const u8 gHonchkrowPokedexText[] = _(
    "A single cry from this nocturnal Pokémon,\n"
    "and more than 100 of its Murkrow cronies\n"
    "will assemble.");

const u8 gGlameowPokedexText[] = _(
    "When it's happy, Glameow demonstrates\n"
    "beautiful movements of its tail, like a\n"
    "dancing ribbon.");

const u8 gPuruglyPokedexText[] = _(
    "To make itself appear intimidatingly beefy,\n"
    "it tightly cinches its waist with its twin\n"
    "tails.");

const u8 gChinglingPokedexText[] = _(
    "It emits high-frequency cries that people\n"
    "can't hear. Once it starts, it can cry for an\n"
    "awfully long time.");

const u8 gStunkyPokedexText[] = _(
    "The foul fluid from its rear is so revolting\n"
    "that it can make people feel queasy up to a\n"
    "mile and a quarter away.");

const u8 gSkuntankPokedexText[] = _(
    "It attacks by spraying a repugnant fluid\n"
    "from its tail, but the stench dulls after a\n"
    "few squirts.");

const u8 gBronzorPokedexText[] = _(
    "They are found in ancient tombs. The\n"
    "patterns on their backs are said to be\n"
    "imbued with mysterious power.");

const u8 gBronzongPokedexText[] = _(
    "It brought rains by opening portals to\n"
    "another world. It was revered as a bringer\n"
    "of plentiful harvests.");

const u8 gBonslyPokedexText[] = _(
    "From its eyes, it can expel excess moisture\n"
    "from its body. This liquid is similar in\n"
    "composition to human sweat.");

const u8 gMimeJrPokedexText[] = _(
    "When this gifted mimic surprises an\n"
    "opponent, Mime Jr. feels so happy that it\n"
    "ends up forgetting it was imitating\n"
    "something.");

const u8 gHappinyPokedexText[] = _(
    "It carries a round, white rock in its\n"
    "belly pouch. If it gets along well with\n"
    "someone, it will sometimes give that\n"
    "person the rock.");

const u8 gChatotPokedexText[] = _(
    "It can learn and speak human words. If they\n"
    "gather, they all learn the same saying.");

const u8 gSpiritombPokedexText[] = _(
    "It was formed by uniting 108 spirits. It has\n"
    "been bound to the Odd Keystone to keep it\n"
    "from doing any mischief.");

const u8 gGiblePokedexText[] = _(
    "It skulks in caves, and when prey or an\n"
    "enemy passes by, it leaps out and chomps\n"
    "them. The force of its attack sometimes\n"
    "chips its teeth.");

const u8 gGabitePokedexText[] = _(
    "In rare cases, it molts and sheds its\n"
    "scales. Medicine containing its scales as\n"
    "an ingredient will make a weary body feel\n"
    "invigorated.");

const u8 gGarchompPokedexText[] = _(
    "The protuberances on its head serve as\n"
    "sensors. It can even detect distant prey.");

const u8 gMunchlaxPokedexText[] = _(
    "When it is voraciously gulping down food, it\n"
    "forgets about the food it has hidden under\n"
    "its fur.");

const u8 gRioluPokedexText[] = _(
    "It has the peculiar power of being able to\n"
    "see emotions, such as joy and rage, in the\n"
    "form of waves.");

const u8 gLucarioPokedexText[] = _(
    "It’s said that no foe can remain invisible\n"
    "to Lucario, since it can detect auras--\n"
    "even those of foes it could not otherwise\n"
    "see.");

const u8 gHippopotasPokedexText[] = _(
    "It shrouds itself in sand to ward off\n"
    "germs. It travels easily through the sands\n"
    "of the desert.");

const u8 gHippowdonPokedexText[] = _(
    "It brandishes its gaping mouth in a display\n"
    "of fearsome strength. It raises vast\n"
    "quantities of sand while attacking.");

const u8 gSkorupiPokedexText[] = _(
    "After burrowing into the sand, it waits\n"
    "patiently for prey to come near. This\n"
    "Pokémon and Sizzlipede share common\n"
    "descent.");

const u8 gDrapionPokedexText[] = _(
    "Its poison is potent, but it rarely sees\n"
    "use. This Pokémon prefers to use physical\n"
    "force instead, going on rampages with its\n"
    "car-crushing strength.");

const u8 gCroagunkPokedexText[] = _(
    "It rarely fights fairly, but that is\n"
    "strictly to ensure survival. It is popular\n"
    "as a mascot.");

const u8 gToxicroakPokedexText[] = _(
    "Swaying and dodging the attacks of its\n"
    "foes, it weaves its flexible body in close,\n"
    "then lunges out with its poisonous claws.");

const u8 gCarnivinePokedexText[] = _(
    "It binds itself to trees in marshes. It\n"
    "attracts prey with its sweet-smelling\n"
    "drool and gulps them down.");

const u8 gFinneonPokedexText[] = _(
    "It absorbs sunlight with the pink areas of\n"
    "its skin, which then shine. This appears to\n"
    "be a form of camouflage.");

const u8 gLumineonPokedexText[] = _(
    "With its shining light, it lures its prey\n"
    "close. However, the light also happens to\n"
    "attract ferocious fish Pokémon--its\n"
    "natural predators.");

const u8 gMantykePokedexText[] = _(
    "It's highly friendly and easily tamed. Tours\n"
    "that take people swimming with Mantyke are\n"
    "a super-popular beach activity.");

const u8 gSnoverPokedexText[] = _(
    "Seemingly curious about people, they\n"
    "gather around footsteps they find on\n"
    "snowy mountains.");

const u8 gAbomasnowPokedexText[] = _(
    "They appear when the snow flowers bloom.\n"
    "When the petals fall, they retreat to\n"
    "places unknown again.");

const u8 gWeavilePokedexText[] = _(
    "They travel in groups of four or five,\n"
    "leaving signs for one another on trees and\n"
    "rocks. They bring down their prey with\n"
    "coordinated attacks.");

const u8 gMagnezonePokedexText[] = _(
    "As it zooms through the sky, this Pokémon\n"
    "seems to be receiving signals of unknown\n"
    "origin while transmitting signals of\n"
    "unknown purpose.");

const u8 gLickilickyPokedexText[] = _(
    "Lickilicky can do just about anything with\n"
    "its tongue, which is as dexterous as the\n"
    "human hand. In contrast, Lickilicky's use\n"
    "of its fingers is clumsy.");

const u8 gRhyperiorPokedexText[] = _(
    "A Rhyperior's brain is smaller than a\n"
    "Rhydon's, but the Rhyperior is stronger\n"
    "because it spends more energy on battling\n"
    "and less on brainpower.");

const u8 gTangrowthPokedexText[] = _(
    "Vine growth is accelerated for Tangrowth\n"
    "living in warm climates. If the vines grow\n"
    "long, Tangrowth shortens them by tearing\n"
    "parts of them off.");

const u8 gElectivirePokedexText[] = _(
    "This Pokémon presses the tips of its tails\n"
    "onto an opponent and instantly sends over\n"
    "20,000 volts of high-voltage electricity\n"
    "through them.");

const u8 gMagmortarPokedexText[] = _(
    "They dwell in volcanic craters. It’s said\n"
    "that only a single pair of Magmortar will\n"
    "inhabit any given volcano.");

const u8 gTogekissPokedexText[] = _(
    "Known as a bringer of blessings, it's been\n"
    "depicted on good-luck charms since\n"
    "ancient times.");

const u8 gYanmegaPokedexText[] = _(
    "Yanmega's wingbeats are so powerful that\n"
    "the Pokémon blows down huge trees nearby\n"
    "when it vigorously launches itself into\n"
    "flight.");

const u8 gLeafeonPokedexText[] = _(
    "The younger they are, the more they smell\n"
    "like fresh grass. With age, their fragrance\n"
    "takes on the odor of fallen leaves.");

const u8 gGlaceonPokedexText[] = _(
    "It can control its body temperature at will.\n"
    "This enables it to freeze the moisture in\n"
    "the atmosphere, creating flurries of\n"
    "diamond dust.");

const u8 gGliscorPokedexText[] = _(
    "It flies soundlessly through the sky. It\n"
    "catches prey with its long tail, then lunges\n"
    "at the prey's vulnerable spots with its\n"
    "fangs.");

const u8 gMamoswinePokedexText[] = _(
    "A frozen one was dug up from soil dating\n"
    "back 10,000 years. It woke up, to much\n"
    "amazement.");

const u8 gPorygonZPokedexText[] = _(
    "An additional program was installed to\n"
    "create a more advanced Pokémon, but it was\n"
    "apparently flawed. Porygon-Z moves oddly\n"
    "as a result.");

const u8 gGalladePokedexText[] = _(
    "Because it can sense what its foe is\n"
    "thinking, its attacks burst out first, fast,\n"
    "and fierce.");

const u8 gProbopassPokedexText[] = _(
    "Probopass radiates such a powerful\n"
    "magnetic field that nearby electrical\n"
    "appliances are rendered useless while it's\n"
    "around.");

const u8 gDusknoirPokedexText[] = _(
    "It comes and goes between this world and\n"
    "the afterlife. People fear this Pokémon\n"
    "because it is said to suck in lost spirits\n"
    "and take them along.");

const u8 gFroslassPokedexText[] = _(
    "When it finds humans or Pokémon it likes, it\n"
    "freezes them and takes them to its chilly\n"
    "den, where they become decorations.");

const u8 gRotomPokedexText[] = _(
    "Research continues on this Pokémon, which\n"
    "could be the power source of a unique\n"
    "motor.");

const u8 gUxiePokedexText[] = _(
    "It is said that its emergence gave humans\n"
    "the intelligence to improve their quality\n"
    "of life.");

const u8 gMespritPokedexText[] = _(
    "It sleeps at the bottom of a lake. Its\n"
    "spirit is said to leave its body to fly on\n"
    "the lake's surface.");

const u8 gAzelfPokedexText[] = _(
    "It is thought that Uxie, Mesprit and Azelf\n"
    "all came from the same egg.");

const u8 gDialgaPokedexText[] = _(
    "It has the power to control time. It\n"
    "appears in Sinnoh-region myths as an\n"
    "ancient deity.");

const u8 gPalkiaPokedexText[] = _(
    "It has the ability to distort space. It is\n"
    "described as a deity in Sinnoh-region\n"
    "mythology.");

const u8 gHeatranPokedexText[] = _(
    "Boiling blood, like magma, circulates\n"
    "through its body. It makes its dwelling\n"
    "place in volcanic caves.");

const u8 gRegigigasPokedexText[] = _(
    "It's said that Regigigas created other\n"
    "Pokémon from many different materials\n"
    "before it moved the continents to where\n"
    "they are today.");

const u8 gGiratinaPokedexText[] = _(
    "It was banished for its violence. It\n"
    "silently gazed upon the old world from the\n"
    "Distortion World.");

const u8 gCresseliaPokedexText[] = _(
    "Those who sleep holding Cresselia's\n"
    "feather are assured of joyful dreams. It is\n"
    "said to represent the crescent moon.");

const u8 gPhionePokedexText[] = _(
    "It drifts in warm seas. It always returns to\n"
    "where it was born, no matter how far it may\n"
    "have drifted.");

const u8 gManaphyPokedexText[] = _(
    "It starts its life with a wondrous power\n"
    "that permits it to bond with any kind of\n"
    "Pokémon.");

const u8 gDarkraiPokedexText[] = _(
    "It can lull people to sleep and make them\n"
    "dream. It is active during nights of the new\n"
    "moon.");

const u8 gShayminPokedexText[] = _(
    "The blooming of Gracidea flowers confers\n"
    "the power of flight upon it. Feelings of\n"
    "gratitude are the message it delivers.");

const u8 gArceusPokedexText[] = _(
    "It is told in mythology that this Pokémon\n"
    "was born before the universe even existed.");

const u8 gVictiniPokedexText[] = _(
    "When it shares the infinite energy it\n"
    "creates, that being's entire body will be\n"
    "overflowing with power.");

const u8 gSnivyPokedexText[] = _(
    "It prefers to avoid groups. In its day-to-\n"
    "day life, it dexterously controls its vines\n"
    "to compensate for its short arms.");

const u8 gServinePokedexText[] = _(
    "Servine’s one weakness is its inflated\n"
    "sense of pride. It takes a while for Servine\n"
    "to be able to cooperate with other\n"
    "Pokémon.");

const u8 gSerperiorPokedexText[] = _(
    "An intimidating gaze and majestic\n"
    "appearance have caused this Pokémon to be\n"
    "known as the Lord of the Forest.");

const u8 gTepigPokedexText[] = _(
    "It’s more nimble than it looks, and it uses\n"
    "its speed to confound its enemies. It\n"
    "rapidly launches fireballs from both\n"
    "nostrils.");

const u8 gPignitePokedexText[] = _(
    "When defending its friends, Pignite will\n"
    "stand up to any opponent, no matter how\n"
    "strong. But it’s powerless when faced with\n"
    "an empty stomach.");

const u8 gEmboarPokedexText[] = _(
    "The direr the situation, the more it seems\n"
    "to enjoy itself. The flames around its neck\n"
    "intensify, too, increasing Emboar’s power\n"
    "and speed.");

const u8 gOshawottPokedexText[] = _(
    "It wields the scalchop on its stomach like\n"
    "a knife, blocking the moves of its enemies\n"
    "before slashing back at them in swift\n"
    "retaliation.");

const u8 gDewottPokedexText[] = _(
    "Dewott has a studious and stern\n"
    "personality. It masters the use of its\n"
    "scalchops through dedicated, repeated\n"
    "training.");

const u8 gSamurottPokedexText[] = _(
    "A master of a sword-drawing art called\n"
    "iaijutsu, Samurott settles its battles with\n"
    "a single swing of either large sword\n"
    "sheathed in its foreleg armor.");

const u8 gPatratPokedexText[] = _(
    "Extremely cautious, one of them will always\n"
    "be on the lookout, but it won't notice a foe\n"
    "coming from behind.");

const u8 gWatchogPokedexText[] = _(
    "When they see an enemy, their tails stand\n"
    "high, and they spit the seeds of berries\n"
    "stored in their cheek pouches.");

const u8 gLillipupPokedexText[] = _(
    "This Pokémon is courageous but also\n"
    "cautious. It uses the soft fur covering its\n"
    "face to collect information about its\n"
    "surroundings.");

const u8 gHerdierPokedexText[] = _(
    "Herdier is a very smart and friendly\n"
    "Pokémon. So much so that there's a theory\n"
    "that Herdier was the first Pokémon to\n"
    "partner with people.");

const u8 gStoutlandPokedexText[] = _(
    "Stoutland is immensely proud of its\n"
    "impressive moustache. It's said that\n"
    "moustache length is what determines social\n"
    "standing among this species.");

const u8 gPurrloinPokedexText[] = _(
    "Opponents that get drawn in by its\n"
    "adorable behavior come away with stinging\n"
    "scratches from its claws and stinging pride\n"
    "from its laughter.");

const u8 gLiepardPokedexText[] = _(
    "This stealthy Pokémon sneaks up behind\n"
    "prey without making any sound at all. It\n"
    "competes with Thievul for territory.");

const u8 gPansagePokedexText[] = _(
    "It's good at finding berries and gathers\n"
    "them from all over. It's kind enough to\n"
    "share them with friends.");

const u8 gSimisagePokedexText[] = _(
    "Ill tempered, it fights by swinging its\n"
    "barbed tail around wildly. The leaf growing\n"
    "on its head is very bitter.");

const u8 gPansearPokedexText[] = _(
    "This Pokémon lives in caves in volcanoes.\n"
    "The fire within the tuft on its head can\n"
    "reach 600 degrees Fahrenheit.");

const u8 gSimisearPokedexText[] = _(
    "When it gets excited, embers rise from its\n"
    "head and tail and it gets hot. For some\n"
    "reason, it loves sweets.");

const u8 gPanpourPokedexText[] = _(
    "The water stored inside the tuft on its\n"
    "head is full of nutrients. Plants that\n"
    "receive its water grow large.");

const u8 gSimipourPokedexText[] = _(
    "It prefers places with clean water. When\n"
    "its tuft runs low, it replenishes it by\n"
    "siphoning up water with its tail.");

const u8 gMunnaPokedexText[] = _(
    "Late at night, it appears beside people's\n"
    "pillows. As it feeds on dreams, the patterns\n"
    "on its body give off a faint glow.");

const u8 gMusharnaPokedexText[] = _(
    "It drowses and dreams all the time. It's\n"
    "best to leave it be if it's just woken up, as\n"
    "it's a terrible grump when freshly roused\n"
    "from sleep.");

const u8 gPidovePokedexText[] = _(
    "Where people go, these Pokémon follow. If\n"
    "you're scattering food for them, be careful\n"
    "- several hundred of them can gather at\n"
    "once.");

const u8 gTranquillPokedexText[] = _(
    "These bright Pokémon have acute memories.\n"
    "Apparently delivery workers often choose\n"
    "them as their partners.");

const u8 gUnfezantPokedexText[] = _(
    "Unfezant are exceptional fliers. The\n"
    "females are known for their stamina, while\n"
    "the males outclass them in terms of speed.");

const u8 gBlitzlePokedexText[] = _(
    "This Pokémon prefers places with lots of\n"
    "lightning strikes. It catches lightning\n"
    "with its mane and stores the electricity\n"
    "within its body.");

const u8 gZebstrikaPokedexText[] = _(
    "This Pokémon is aggressive and has a\n"
    "volatile disposition. When agitated, it will\n"
    "illuminate its mane in yellow and fire off\n"
    "bolts of electricity.");

const u8 gRoggenrolaPokedexText[] = _(
    "When it detects a noise, it starts to move.\n"
    "The energy core inside it makes this\n"
    "Pokémon slightly warm to the touch.");

const u8 gBoldorePokedexText[] = _(
    "It relies on sound in order to monitor\n"
    "what's in its vicinity. When angered, it will\n"
    "attack without ever changing the direction\n"
    "it's facing.");

const u8 gGigalithPokedexText[] = _(
    "Although its energy blasts can blow away a\n"
    "dump truck, they have a limitation--they\n"
    "can only be fired when the sun is out.");

const u8 gWoobatPokedexText[] = _(
    "While inside a cave, if you look up and see\n"
    "lots of heart-shaped marks lining the\n"
    "walls, it's evidence that Woobat live there.");

const u8 gSwoobatPokedexText[] = _(
    "The auspicious shape of this Pokémon's\n"
    "nose apparently led some regions to\n"
    "consider Swoobat a symbol of good luck.");

const u8 gDrilburPokedexText[] = _(
    "It makes its way swiftly through the soil\n"
    "by putting both claws together and\n"
    "rotating at high speed.");

const u8 gExcadrillPokedexText[] = _(
    "Its drill, which has evolved into steel, is\n"
    "strong enough to bore through iron plates.\n"
    "This Pokémon is a great help in tunnel\n"
    "construction.");

const u8 gAudinoPokedexText[] = _(
    "This Pokémon has a kind heart. By touching\n"
    "with its feelers, Audino can gauge other\n"
    "creatures' feelings and physical\n"
    "conditions.");

const u8 gTimburrPokedexText[] = _(
    "It fights by swinging a piece of lumber\n"
    "around. It is close to evolving when it can\n"
    "handle heavy lumber without difficulty.");

const u8 gGurdurrPokedexText[] = _(
    "This Pokémon is so muscular and strongly\n"
    "built that even a group of wrestlers could\n"
    "not make it budge an inch.");

const u8 gConkeldurrPokedexText[] = _(
    "Rather than rely on their strength, they\n"
    "master moves that make good use of\n"
    "centrifugal force to swing around\n"
    "concrete.");

const u8 gTympolePokedexText[] = _(
    "It uses sound waves to communicate with\n"
    "others of its kind. People and other\n"
    "Pokémon species can't hear its cries of\n"
    "warning.");

const u8 gPalpitoadPokedexText[] = _(
    "On occasion, their cries are sublimely\n"
    "pleasing to the ear. Palpitoad with larger\n"
    "lumps on their bodies can sing with a wider\n"
    "range of sounds.");

const u8 gSeismitoadPokedexText[] = _(
    "The vibrating of the bumps all over its\n"
    "body causes earthquake-like tremors.\n"
    "Seismitoad and Croagunk are similar\n"
    "species.");

const u8 gThrohPokedexText[] = _(
    "It performs throwing moves with first-rate\n"
    "skill. Over the course of many battles,\n"
    "Throh's belt grows darker as it absorbs its\n"
    "wearer's sweat.");

const u8 gSawkPokedexText[] = _(
    "The karate chops of a Sawk that's trained\n"
    "itself to the limit can cleave the ocean\n"
    "itself.");

const u8 gSewaddlePokedexText[] = _(
    "The bumps on their heads are sensory\n"
    "organs. When Sewaddle meet, they greet\n"
    "each other by rubbing these bumps\n"
    "together.");

const u8 gSwadloonPokedexText[] = _(
    "Swadloon wraps itself in leaves for\n"
    "protection. It uses the two leaves atop its\n"
    "head to detect air movements and feel out\n"
    "its surroundings.");

const u8 gLeavannyPokedexText[] = _(
    "This gentle Pokémon has strong protective\n"
    "instincts. The leaves on its arms are sharp\n"
    "enough to slice a thick tree in half with\n"
    "one stroke.");

const u8 gVenipedePokedexText[] = _(
    "Its fangs are highly venomous. If this\n"
    "Pokémon finds prey it thinks it can eat, it\n"
    "leaps for them without any thought of how\n"
    "things might turn out.");

const u8 gWhirlipedePokedexText[] = _(
    "Whirlipede protects itself with a sturdy\n"
    "shell and poisonous spikes while it stores\n"
    "up the energy it'll need for evolution.");

const u8 gScolipedePokedexText[] = _(
    "Scolipede latches on to its prey with the\n"
    "claws on its neck before slamming them into\n"
    "the ground and jabbing them with its claw's\n"
    "toxic spikes.");

const u8 gCottoneePokedexText[] = _(
    "When attacked, it escapes by shooting\n"
    "cotton from its body. The cotton serves as\n"
    "a decoy to distract the attacker.");

const u8 gWhimsicottPokedexText[] = _(
    "It rides on the wind and slips into people’s\n"
    "homes. After it has turned a room into a\n"
    "cotton- filled mess, it giggles to itself\n"
    "and takes off.");

const u8 gPetililPokedexText[] = _(
    "If the leaves on its head are pruned with\n"
    "regularity, this Pokémon can be grown into\n"
    "a fine plump shape.");

const u8 gLilligantPokedexText[] = _(
    "No matter how much time and money is spent\n"
    "raising it, its flowers are the most\n"
    "beautiful when they bloom in the wild.");

const u8 gBasculinPokedexText[] = _(
    "It's so vicious that it's called the Thug\n"
    "of the River. Yet Basculin is still\n"
    "targeted by predators, such as Dondozo\n"
    "and Bombirdier.");

const u8 gSandilePokedexText[] = _(
    "It submerges itself in sand and moves as if\n"
    "swimming. This wise behavior keeps its\n"
    "enemies from finding it and maintains its\n"
    "temperature.");

const u8 gKrokorokPokedexText[] = _(
    "Thanks to the special membrane covering\n"
    "its eyes, it can see its surroundings\n"
    "clearly, even in the middle of the night.");

const u8 gKrookodilePokedexText[] = _(
    "Its unique faculty of sight can detect\n"
    "small prey more than 30 miles away, even in\n"
    "the midst of a sandstorm.");

const u8 gDarumakaPokedexText[] = _(
    "This popular symbol of good fortune will\n"
    "never fall over in its sleep, no matter how\n"
    "it's pushed or pulled.");

const u8 gDarmanitanPokedexText[] = _(
    "This Pokémon's power level rises along with\n"
    "the temperature of its fire, which can\n"
    "reach 2,500 degrees Fahrenheit.");

const u8 gMaractusPokedexText[] = _(
    "With noises that could be mistaken for the\n"
    "rattles of maracas, it creates an upbeat\n"
    "rhythm, startling bird Pokémon and making\n"
    "them fly off in a hurry.");

const u8 gDwebblePokedexText[] = _(
    "When it finds a stone appealing, it creates\n"
    "a hole inside it and uses it as its home.\n"
    "This Pokémon is the natural enemy of\n"
    "Roggenrola and Rolycoly.");

const u8 gCrustlePokedexText[] = _(
    "Its thick claws are its greatest weapons.\n"
    "They're mighty enough to crack Rhyperior's\n"
    "carapace.");

const u8 gScraggyPokedexText[] = _(
    "It stretches its saggy skin up to its neck\n"
    "to protect itself. Among Scraggy,\n"
    "individuals with saggier skin apparently\n"
    "garner more respect.");

const u8 gScraftyPokedexText[] = _(
    "This Pokémon may be rude, but it takes very\n"
    "good care of its family, its friends, and its\n"
    "turf.");

const u8 gSigilyphPokedexText[] = _(
    "Psychic power allows these Pokémon to fly.\n"
    "Some say they were the guardians of an\n"
    "ancient city. Others say they were the\n"
    "guardians' emissaries.");

const u8 gYamaskPokedexText[] = _(
    "The spirit of a person from a bygone age\n"
    "became this Pokémon. It rambles through\n"
    "ruins, searching for someone who knows its\n"
    "face.");

const u8 gCofagrigusPokedexText[] = _(
    "There are many depictions of Cofagrigus\n"
    "decorating ancient tombs. They're symbols\n"
    "of the wealth that kings of bygone eras\n"
    "had.");

const u8 gTirtougaPokedexText[] = _(
    "Tirtouga is considered to be the ancestor\n"
    "of many turtle Pokémon. It was restored to\n"
    "life from a fossil.");

const u8 gCarracostaPokedexText[] = _(
    "Carracosta completely devours its prey--\n"
    "bones, shells, and all. Because of this,\n"
    "Carracosta's own shell grows thick and\n"
    "sturdy.");

const u8 gArchenPokedexText[] = _(
    "This Pokémon was successfully restored\n"
    "from a fossil. As research suggested,\n"
    "Archen is unable to fly. But it's very good\n"
    "at jumping.");

const u8 gArcheopsPokedexText[] = _(
    "It needs a running start to take off. If\n"
    "Archeops wants to fly, it first needs to\n"
    "run nearly 25 mph, building speed over a\n"
    "course of about 2.5 miles.");

const u8 gTrubbishPokedexText[] = _(
    "Its favorite places are unsanitary ones.\n"
    "If you leave trash lying around, you could\n"
    "even find one of these Pokémon living in\n"
    "your room.");

const u8 gGarbodorPokedexText[] = _(
    "This Pokémon eats trash, which turns into\n"
    "poison inside its body. The main component\n"
    "of the poison depends on what sort of\n"
    "trash was eaten.");

const u8 gZoruaPokedexText[] = _(
    "To protect themselves from danger, they\n"
    "hide their true identities by transforming\n"
    "into people and Pokémon.");

const u8 gZoroarkPokedexText[] = _(
    "Each has the ability to fool a large group\n"
    "of people simultaneously. They protect\n"
    "their lair with illusory scenery.");

const u8 gMinccinoPokedexText[] = _(
    "When its tail has gotten dirty from self-\n"
    "cleaning or from cleaning its nest, Minccino\n"
    "spends a whole day washing its tail in clean\n"
    "springwater.");

const u8 gCinccinoPokedexText[] = _(
    "The oil that seeps from its body is really\n"
    "smooth. For people troubled by bad skin,\n"
    "this oil is an effective treatment.");

const u8 gGothitaPokedexText[] = _(
    "Beware of touching the ribbon-shaped\n"
    "feelers that control its psychic power.\n"
    "Gothita will begin bawling if you do.");

const u8 gGothoritaPokedexText[] = _(
    "Using its psychic power, it arranges\n"
    "pebbles to form the shapes of\n"
    "constellations. Some people believe this\n"
    "Pokémon came from outer space.");

const u8 gGothitellePokedexText[] = _(
    "Gothitelle unleashes psychic energy and\n"
    "shows opponents dreams of the universe’s\n"
    "end. These dreams are apparently ethereal\n"
    "and beautiful.");

const u8 gSolosisPokedexText[] = _(
    "They drive away attackers by unleashing\n"
    "psychic power. They can use telepathy to\n"
    "talk with others.");

const u8 gDuosionPokedexText[] = _(
    "When their brains, now divided in two, are\n"
    "thinking the same thoughts, these Pokémon\n"
    "exhibit their maximum power.");

const u8 gReuniclusPokedexText[] = _(
    "When Reuniclus shake hands, a network\n"
    "forms between their brains, increasing\n"
    "their psychic power.");

const u8 gDucklettPokedexText[] = _(
    "It strengthens its body by diving in to the\n"
    "depths of ponds, swimming around while\n"
    "looking for bog moss to eat.");

const u8 gSwannaPokedexText[] = _(
    "It's said that many artists have taken\n"
    "inspiration from the sight of Swanna\n"
    "performing a beautiful dance in the light\n"
    "of dawn.");

const u8 gVanillitePokedexText[] = _(
    "Unable to survive in hot areas, it makes\n"
    "itself comfortable by breathing out air\n"
    "cold enough to cause snow. It burrows into\n"
    "the snow to sleep.");

const u8 gVanillishPokedexText[] = _(
    "It blasts enemies with cold air reaching\n"
    "-148 degrees Fahrenheit, freezing them\n"
    "solid. But it spares their lives afterward--\n"
    "it's a kind Pokémon.");

const u8 gVanilluxePokedexText[] = _(
    "When its anger reaches a breaking point,\n"
    "this Pokémon unleashes a fierce blizzard\n"
    "that freezes every creature around it, be\n"
    "they friend or foe.");

const u8 gDeerlingPokedexText[] = _(
    "The turning of the seasons changes the\n"
    "color and scent of this Pokémon's fur.\n"
    "People use it to mark the seasons.");

const u8 gSawsbuckPokedexText[] = _(
    "They migrate according to seasons, so some\n"
    "people call Sawsbuck the harbingers of\n"
    "spring.");

const u8 gEmolgaPokedexText[] = _(
    "This Pokémon absolutely loves sweet\n"
    "berries. Sometimes it stuffs its cheeks\n"
    "full of so much food that it can't fly\n"
    "properly.");

const u8 gKarrablastPokedexText[] = _(
    "Its strange physiology reacts to\n"
    "electrical energy in interesting ways. The\n"
    "presence of a Shelmet will cause this\n"
    "Pokémon to evolve.");

const u8 gEscavalierPokedexText[] = _(
    "It charges its enemies, lances at the\n"
    "ready. An image of one of its duels is\n"
    "captured in a famous painting of\n"
    "Escavalier clashing with Sirfetch'd.");

const u8 gFoongusPokedexText[] = _(
    "This Pokémon prefers damp places. It\n"
    "spurts out poison spores to repel\n"
    "approaching enemies.");

const u8 gAmoongussPokedexText[] = _(
    "Be wary of the poisonous spores it\n"
    "releases. Mushrooms resembling\n"
    "Amoonguss’s caps will grow out of anywhere\n"
    "the spores touch.");

const u8 gFrillishPokedexText[] = _(
    "Using the invisible poison spikes on its\n"
    "veillike arms and legs, it paralyzes its\n"
    "enemies and causes them to drown.");

const u8 gJellicentPokedexText[] = _(
    "Fishermen are terrified of Jellicent. It's\n"
    "rumored to drag them into the sea and steal\n"
    "their lives away.");

const u8 gAlomomolaPokedexText[] = _(
    "They float upon the open sea. Many water\n"
    "Pokémon gather in the area around\n"
    "Alomomola.");

const u8 gJoltikPokedexText[] = _(
    "Since it can’t generate its own electricity,\n"
    "it sticks onto large-bodied Pokémon and\n"
    "absorbs static electricity.");

const u8 gGalvantulaPokedexText[] = _(
    "When attacked, they create an electric\n"
    "barrier by spitting out many electrically\n"
    "charged threads.");

const u8 gFerroseedPokedexText[] = _(
    "Mossy caves are their preferred dwellings.\n"
    "Enzymes contained in mosses help\n"
    "Ferroseed's spikes grow big and strong.");

const u8 gFerrothornPokedexText[] = _(
    "This Pokémon scrapes its spikes across\n"
    "rocks, and then uses the tips of its\n"
    "feelers to absorb the nutrients it finds\n"
    "within the stone.");

const u8 gKlinkPokedexText[] = _(
    "It's suspected that Klink were the\n"
    "inspiration behind ancient people's\n"
    "invention of the first gears.");

const u8 gKlangPokedexText[] = _(
    "When Klang goes all out, the minigear links\n"
    "up perfectly with the outer part of the big\n"
    "gear, and this Pokémon's rotation speed\n"
    "increases sharply.");

const u8 gKlinklangPokedexText[] = _(
    "The three gears that compose this Pokémon\n"
    "spin at high speed. Its new spiked gear\n"
    "isn't a living creature.");

const u8 gTynamoPokedexText[] = _(
    "These Pokémon move in schools. They have\n"
    "an electricity-generating organ, so they\n"
    "discharge electricity if in danger.");

const u8 gEelektrikPokedexText[] = _(
    "These Pokémon have a big appetite. When\n"
    "they spot their prey, they attack it and\n"
    "paralyze it with electricity.");

const u8 gEelektrossPokedexText[] = _(
    "It latches on to prey with its sucker\n"
    "mouth, sinking in its fangs and shocking\n"
    "the prey with powerful electricity.");

const u8 gElgyemPokedexText[] = _(
    "This Pokémon was discovered about 50\n"
    "years ago. Its highly developed brain\n"
    "enables it to exert its psychic powers.");

const u8 gBeheeyemPokedexText[] = _(
    "It has strong psychic powers. Using its\n"
    "fingers that flash three different colors,\n"
    "it controls its opponents and rewrites\n"
    "their memories.");

const u8 gLitwickPokedexText[] = _(
    "While shining a light and pretending to be a\n"
    "guide, it leeches off the life-force of any\n"
    "who follow it.");

const u8 gLampentPokedexText[] = _(
    "The spirits it absorbs fuel its baleful fire.\n"
    "It hangs around hospitals waiting for\n"
    "people to pass on.");

const u8 gChandelurePokedexText[] = _(
    "It absorbs spirits, which it then burns. By\n"
    "waving the flames on its arms, it puts its\n"
    "foes into a hypnotic trance.");

const u8 gAxewPokedexText[] = _(
    "If you see peculiar teeth marks on\n"
    "boulders or trees, it means an Axew is\n"
    "likely living nearby.");

const u8 gFraxurePokedexText[] = _(
    "Fraxure uses its thick tusks to neatly cut\n"
    "prey into two portions--one to eat now and\n"
    "one to save for later.");

const u8 gHaxorusPokedexText[] = _(
    "This Pokémon nests in caves and abandoned\n"
    "mines. It is docile, but it will become\n"
    "incredibly angry if its tusks are touched,\n"
    "so beware.");

const u8 gCubchooPokedexText[] = _(
    "Many of this species can be found along\n"
    "the shorelines of cold regions. If a\n"
    "Cubchoo lacks dangling snot, there's a\n"
    "chance it is sick.");

const u8 gBearticPokedexText[] = _(
    "It is a ferocious, carnivorous Pokémon.\n"
    "Once it captures its prey, it will breathe\n"
    "cold air onto the prey to freeze and\n"
    "preserve it.");

const u8 gCryogonalPokedexText[] = _(
    "Cryogonal appear during cold seasons. It is\n"
    "said that people and Pokémon who die on\n"
    "snowy mountains are reborn into these\n"
    "Pokémon.");

const u8 gShelmetPokedexText[] = _(
    "When attacked, it tightly shuts the lid of\n"
    "its shell. This reaction fails to protect it\n"
    "from Karrablast, however, because they can\n"
    "still get into the shell.");

const u8 gAccelgorPokedexText[] = _(
    "It moves with blinding speed and lobs\n"
    "poison at foes. Featuring Accelgor as a\n"
    "main character is a surefire way to make a\n"
    "movie or comic popular.");

const u8 gStunfiskPokedexText[] = _(
    "For some reason, this Pokémon smiles\n"
    "slightly when it emits a strong electric\n"
    "current from the yellow markings on its\n"
    "body.");

const u8 gMienfooPokedexText[] = _(
    "They seclude themselves in the mountains\n"
    "and devote themselves to training. The\n"
    "form of their kicks and chops differs from\n"
    "pack to pack.");

const u8 gMienshaoPokedexText[] = _(
    "When Mienshao lets out a bizarre wail,\n"
    "you're in danger. A flurry of kicks and\n"
    "chops too fast to see is about to be\n"
    "unleashed!");

const u8 gDruddigonPokedexText[] = _(
    "Druddigon are vicious and cunning. They\n"
    "take up residence in nests dug out by\n"
    "other Pokémon, treating the stolen nests\n"
    "as their own lairs.");

const u8 gGolettPokedexText[] = _(
    "Its movements are powered by a mysterious\n"
    "energy. It has continued to move since\n"
    "ancient times, so its power may soon run\n"
    "out.");

const u8 gGolurkPokedexText[] = _(
    "When the seal on its chest is removed, it\n"
    "rages indiscriminately. During such\n"
    "rampages, it can turn a whole town into a\n"
    "mountain of rubble.");

const u8 gPawniardPokedexText[] = _(
    "Any chips in its blades would prove fatal\n"
    "for it. After each battle, it diligently\n"
    "maintains its blades using its favorite\n"
    "sharpening stone.");

const u8 gBisharpPokedexText[] = _(
    "This Pokémon commands a group of several\n"
    "Pawniard. Groups that are defeated in\n"
    "territorial disputes are absorbed by the\n"
    "winning side.");

const u8 gBouffalantPokedexText[] = _(
    "These Pokémon can crush a car with no more\n"
    "than a headbutt. Bouffalant with more hair\n"
    "on their heads hold higher positions within\n"
    "the herd.");

const u8 gRuffletPokedexText[] = _(
    "With its sharp claws, this Pokémon pierces\n"
    "its prey, and then it pecks at them.\n"
    "Although it also consumes berries, it's a\n"
    "carnivore at heart.");

const u8 gBraviaryPokedexText[] = _(
    "For the sake of its friends, this brave\n"
    "warrior of the sky will not stop battling,\n"
    "even if injured.");

const u8 gVullabyPokedexText[] = _(
    "Its healthy appetite leads to visible\n"
    "growth spurts. It often has to replace the\n"
    "bones it wears as its size increases.");

const u8 gMandibuzzPokedexText[] = _(
    "Watching from the sky, they attack\n"
    "weakened prey on the ground. They have a\n"
    "habit of decorating themselves with bones.");

const u8 gHeatmorPokedexText[] = _(
    "There's a hole in its tail that allows it to\n"
    "draw in the air it needs to keep its fire\n"
    "burning. If the hole gets blocked, this\n"
    "Pokémon will fall ill.");

const u8 gDurantPokedexText[] = _(
    "With their large mandibles, these Pokémon\n"
    "can crunch their way through rock. They\n"
    "work together to protect their eggs from\n"
    "Sandaconda.");

const u8 gDeinoPokedexText[] = _(
    "It can’t see, so its first approach to\n"
    "examining things is to bite them. You will be\n"
    "covered in wounds until a Deino warms up to\n"
    "you.");

const u8 gZweilousPokedexText[] = _(
    "The two heads have different likes and\n"
    "dislikes. Because the heads fight with each\n"
    "other, Zweilous gets stronger without\n"
    "needing to rely on others.");

const u8 gHydreigonPokedexText[] = _(
    "It's said that Hydreigon grew ferocious\n"
    "because people in times long past loathed\n"
    "it, considering it to be evil incarnate and\n"
    "attacking it relentlessly.");

const u8 gLarvestaPokedexText[] = _(
    "In ancient times, Larvesta was worshiped\n"
    "as the emissary of the sun. However, it was\n"
    "also viewed as a burden since it often\n"
    "caused forest fires.");

const u8 gVolcaronaPokedexText[] = _(
    "This Pokémon scatters burning scales. Most\n"
    "of the danger of these scales is not in\n"
    "their heat--it's in the way they rob the\n"
    "surrounding air of oxygen.");

const u8 gCobalionPokedexText[] = _(
    "This Pokémon appears in a legend alongside\n"
    "Terrakion and Virizion, fighting against\n"
    "humans in defense of the Unova region's\n"
    "Pokémon.");

const u8 gTerrakionPokedexText[] = _(
    "In Unovan legend, Terrakion battled\n"
    "against humans in an effort to protect\n"
    "other Pokémon.");

const u8 gVirizionPokedexText[] = _(
    "It darts around opponents with a flurry of\n"
    "quick movements, slicing them up with its\n"
    "horns.");

const u8 gTornadusPokedexText[] = _(
    "Tornadus expels massive energy from its\n"
    "tail, causing severe storms. Its power is\n"
    "great enough to blow houses away.");

const u8 gThundurusPokedexText[] = _(
    "As it flies around, it shoots lightning all\n"
    "over the place and causes forest fires. It\n"
    "is therefore disliked.");

const u8 gReshiramPokedexText[] = _(
    "According to myth, if people ignore truth\n"
    "and let themselves become consumed by\n"
    "greed, Reshiram will arrive to burn their\n"
    "kingdoms down.");

const u8 gZekromPokedexText[] = _(
    "Mythology tells us that if people lose the\n"
    "righteousness in their hearts, their\n"
    "kingdoms will be razed by Zekrom's\n"
    "lightning.");

const u8 gLandorusPokedexText[] = _(
    "From the forces of lightning and wind, it\n"
    "creates energy to give nutrients to the\n"
    "soil and make the land abundant.");

const u8 gKyuremPokedexText[] = _(
    "It appears that this Pokémon uses its\n"
    "powers over ice to freeze its own body in\n"
    "order to stabilize its cellular structure.");

const u8 gKeldeoPokedexText[] = _(
    "Cobalion, Terrakion, and Virizion taught\n"
    "this Pokémon how to fight. It dashes\n"
    "across the world, seeking more\n"
    "opportunities to further its training.");

const u8 gMeloettaPokedexText[] = _(
    "Its melodies are sung with a special\n"
    "vocalization method that can control the\n"
    "feelings of those who hear it.");

const u8 gGenesectPokedexText[] = _(
    "This Pokémon existed 300 million years ago.\n"
    "Team Plasma altered it and attached a\n"
    "cannon to its back.");

const u8 gChespinPokedexText[] = _(
    "When Chespin tenses up just before landing\n"
    "a headbutt, the spikes on its head sharpen\n"
    "to points, piercing the enemy’s body on\n"
    "contact.");

const u8 gQuilladinPokedexText[] = _(
    "It can send a trailer flying with a tackle.\n"
    "It’s numb to pain, so even violent\n"
    "collisions don’t faze it.");

const u8 gChesnaughtPokedexText[] = _(
    "Its gentle disposition would have it avoid\n"
    "conflict, but it gives opponents a\n"
    "thrashing with its thorned fists when the\n"
    "situation calls for it.");

const u8 gFennekinPokedexText[] = _(
    "Twigs make for Fennekin’s snacks. When it\n"
    "nibbles on twigs, it finds the courage to\n"
    "face strong foes.");

const u8 gBraixenPokedexText[] = _(
    "It ignites the twig from its tail and waves\n"
    "the twig around to signal its allies. To\n"
    "indicate danger, it draws circles in the air\n"
    "with the twig’s flame.");

const u8 gDelphoxPokedexText[] = _(
    "It controls flames telekinetically,\n"
    "trapping its foes in a fiery vortex\n"
    "surpassing 5,400 degrees Fahrenheit and\n"
    "burning them to a crisp.");

const u8 gFroakiePokedexText[] = _(
    "Its happy-go-lucky look is an act intended\n"
    "to fool its enemies. It will suddenly drop\n"
    "the facade and overwhelm its foes with its\n"
    "blinding speed.");

const u8 gFrogadierPokedexText[] = _(
    "It excels at devising battle strategies\n"
    "that make use of the terrain, and it never\n"
    "misses its mark when throwing bubble-\n"
    "covered pebbles.");

const u8 gGreninjaPokedexText[] = _(
    "The movement of air against the long\n"
    "tongue Greninja keeps wrapped around its\n"
    "neck allows it to sense enemies it can’t\n"
    "see.");

const u8 gBunnelbyPokedexText[] = _(
    "It's very sensitive to danger. The sound of\n"
    "Corviknight's flapping will have Bunnelby\n"
    "digging a hole to hide underground in\n"
    "moments.");

const u8 gDiggersbyPokedexText[] = _(
    "The fur on its belly retains heat\n"
    "exceptionally well. People used to make\n"
    "heavy winter clothing from fur shed by this\n"
    "Pokémon.");

const u8 gFletchlingPokedexText[] = _(
    "Fletchling can be found in both rural and\n"
    "urban areas. They once had a huge\n"
    "territorial dispute with Squawkabilly.\n"
    "Fights broke out all over town.");

const u8 gFlechinderPokedexText[] = _(
    "Fletchinder scatters embers in tall grass\n"
    "where bug Pokémon might be hiding and then\n"
    "catches them as they come leaping out.");

const u8 gTalonflamePokedexText[] = _(
    "When it gets excited, it vents sparks from\n"
    "the gaps between its feathers. These\n"
    "unburnable feathers are used as a\n"
    "fireproof material.");

const u8 gScatterbugPokedexText[] = _(
    "Any poison this Pokémon takes in is\n"
    "converted into a black powder and\n"
    "secreted. That's why Scatterbug can eat\n"
    "poisonous leaves and roots.");

const u8 gSpewpaPokedexText[] = _(
    "Spewpa doesn't live in a fixed location. It\n"
    "roams where it pleases across the fields\n"
    "and mountains, building up the energy it\n"
    "needs to evolve.");

const u8 gVivillonPokedexText[] = _(
    "Vivillon with many different patterns are\n"
    "found all over the world. These patterns\n"
    "are affected by the climate of their\n"
    "habitat.");

const u8 gLitleoPokedexText[] = _(
    "When Litleo are young, female Pyroar will\n"
    "teach them how to hunt. Once the Litleo\n"
    "mature, they will leave the pride and set\n"
    "out on their own.");

const u8 gPyroarPokedexText[] = _(
    "The mane of a male Pyroar heats up to over\n"
    "3,600 degrees Fahrenheit during battle.\n"
    "Merely approaching it will cause severe\n"
    "burns.");

const u8 gFlabebePokedexText[] = _(
    "It's not safe without the power of a\n"
    "flower, but it will keep traveling around\n"
    "until it finds one with the color and shape\n"
    "it wants.");

const u8 gFloettePokedexText[] = _(
    "It gives its own power to flowers, pouring\n"
    "its heart into caring for them. Floette\n"
    "never forgives anyone who messes up a\n"
    "flower bed.");

const u8 gFlorgesPokedexText[] = _(
    "Its life can span several hundred years.\n"
    "It's said to devote its entire life to\n"
    "protecting gardens.");

const u8 gSkiddoPokedexText[] = _(
    "As long as it has sunlight and water, Skiddo\n"
    "can make energy with the leaves on its\n"
    "body, allowing it to live on rocky mountains\n"
    "barren of food.");

const u8 gGogoatPokedexText[] = _(
    "It can sense the feelings of others by\n"
    "touching them with its horns. This species\n"
    "has assisted people with their work since\n"
    "5,000 years ago.");

const u8 gPanchamPokedexText[] = _(
    "Wanting to make sure it's taken seriously,\n"
    "Pancham's always giving others a glare. But\n"
    "if it's not focusing, it ends up smiling.");

const u8 gPangoroPokedexText[] = _(
    "Using its leaf, Pangoro can predict the\n"
    "moves of its opponents. It strikes with\n"
    "punches that can turn a dump truck into\n"
    "scrap with just one hit.");

const u8 gFurfrouPokedexText[] = _(
    "There was an era when aristocrats would\n"
    "compete to see who could trim their\n"
    "Furfrou's fur into the most exquisite\n"
    "style.");

const u8 gEspurrPokedexText[] = _(
    "It keeps its ears covered to prevent the\n"
    "intense psychic power in its body from\n"
    "escaping, but it falls unconscious if this\n"
    "power builds too much.");

const u8 gMeowsticPokedexText[] = _(
    "Though it has enough psychic power to\n"
    "blast away any opponent, Meowstic prefers\n"
    "to support its allies rather than attack.");

const u8 gHonedgePokedexText[] = _(
    "The blue eye on the sword's handguard is\n"
    "the true body of Honedge. With its old\n"
    "cloth, it drains people's lives away.");

const u8 gDoubladePokedexText[] = _(
    "The two swords employ a strategy of\n"
    "rapidly alternating between offense and\n"
    "defense to bring down their prey.");

const u8 gAegislashPokedexText[] = _(
    "Its potent spectral powers allow it to\n"
    "manipulate others. It once used its powers\n"
    "to force people and Pokémon to build a\n"
    "kingdom to its liking.");

const u8 gSpritzeePokedexText[] = _(
    "A scent pouch within this Pokémon's body\n"
    "allows it to create various scents. A\n"
    "change in its diet will alter the fragrance\n"
    "it produces.");

const u8 gAromatissePokedexText[] = _(
    "The scent that constantly emits from its\n"
    "fur is so powerful that this Pokémon's\n"
    "companions will eventually lose their sense\n"
    "of smell.");

const u8 gSwirlixPokedexText[] = _(
    "The sweet smell of cotton candy perfumes\n"
    "Swirlix's fluffy fur. This Pokémon spits out\n"
    "sticky string to tangle up its enemies.");

const u8 gSlurpuffPokedexText[] = _(
    "By taking in a person's scent, it can sniff\n"
    "out their mental and physical condition.\n"
    "It's hoped that this skill will have many\n"
    "medical applications.");

const u8 gInkayPokedexText[] = _(
    "Being upside-down improves its blood flow\n"
    "and clears its mind, causing the power of\n"
    "its psychic moves to increase dramatically.");

const u8 gMalamarPokedexText[] = _(
    "Its hypnotic powers are dangerously\n"
    "strong. Anyone who falls under Malamar’s\n"
    "control loses their memories surrounding\n"
    "the event.");

const u8 gBinaclePokedexText[] = _(
    "After two Binacle find a suitably sized\n"
    "rock, they adhere themselves to it and live\n"
    "together. They cooperate to gather food\n"
    "during high tide.");

const u8 gBarbaraclePokedexText[] = _(
    "Having an eye on each palm allows it to\n"
    "keep watch in all directions. In a pinch, its\n"
    "limbs start to act on their own to ensure\n"
    "the enemy's defeat.");

const u8 gSkrelpPokedexText[] = _(
    "This Pokémon is a poor swimmer. If it's\n"
    "caught in a fierce storm, it will sometimes\n"
    "get washed far away and become unable to\n"
    "return to its home.");

const u8 gDragalgePokedexText[] = _(
    "Using a liquid poison, Dragalge\n"
    "indiscriminately attacks anything that\n"
    "wanders into its territory. This poison can\n"
    "corrode the undersides of boats.");

const u8 gClauncherPokedexText[] = _(
    "Clauncher's claws will regrow if they fall\n"
    "off. The meat inside the claws is edible,\n"
    "but it has a distinct flavor that doesn't\n"
    "appeal to all tastes.");

const u8 gClawitzerPokedexText[] = _(
    "Clawitzer can move through the water at a\n"
    "speed of 60 knots by using gas inside its\n"
    "body to expel water from the back of its\n"
    "claw.");

const u8 gHelioptilePokedexText[] = _(
    "The sun powers this Pokémon's electricity\n"
    "generation. Interruption of that process\n"
    "stresses Helioptile to the point of\n"
    "weakness.");

const u8 gHelioliskPokedexText[] = _(
    "One Heliolisk basking in the sun with its\n"
    "frill outspread is all it would take to\n"
    "produce enough electricity to power a\n"
    "city.");

const u8 gTyruntPokedexText[] = _(
    "This is an ancient Pokémon, revived in\n"
    "modern times. It has a violent disposition,\n"
    "and it'll tear apart anything it gets\n"
    "between its hefty jaws.");

const u8 gTyrantrumPokedexText[] = _(
    "A single bite of Tyrantrum's massive jaws\n"
    "will demolish a car. This Pokémon was the\n"
    "king of the ancient world.");

const u8 gAmauraPokedexText[] = _(
    "This Pokémon was successfully restored\n"
    "from a fossil. In the past, it lived with\n"
    "others of its kind in cold lands where\n"
    "there were fewer predators.");

const u8 gAurorusPokedexText[] = _(
    "When gripped by rage, Aurorus will\n"
    "emanate freezing air, covering\n"
    "everything around it in ice.");

const u8 gSylveonPokedexText[] = _(
    "Sylveon cuts an elegant figure as it\n"
    "dances lightly around, feelers fluttering,\n"
    "but its piercing moves aim straight for its\n"
    "opponents' weak spots.");

const u8 gHawluchaPokedexText[] = _(
    "Its elegant finishing moves--performed\n"
    "by nimbly leaping around using its wings--\n"
    "are polished in the forest where it was\n"
    "born and raised.");

const u8 gDedennePokedexText[] = _(
    "It’s small and its electricity-generating\n"
    "organ is not fully developed, so it uses its\n"
    "tail to absorb electricity from people’s\n"
    "homes and charge itself.");

const u8 gCarbinkPokedexText[] = _(
    "Born from the high temperatures and\n"
    "pressures deep underground, it defends\n"
    "itself by firing beams from the jewel part\n"
    "of its body.");

const u8 gGoomyPokedexText[] = _(
    "Goomy's horns are excellent sensory\n"
    "organs that cover all five of Goomy's\n"
    "senses. Goomy picks up on danger from the\n"
    "movement of the air.");

const u8 gSliggooPokedexText[] = _(
    "When Sliggoo senses danger, the mucus\n"
    "coating its entire body becomes more\n"
    "concentrated--it'll dissolve anything.");

const u8 gGoodraPokedexText[] = _(
    "This Pokémon is uncontrollable when\n"
    "enraged. It rampages on and on, lashing\n"
    "its tail with enough power to send a dump\n"
    "truck flying.");

const u8 gKlefkiPokedexText[] = _(
    "In the past, noble families entrusted\n"
    "their vault keys to a Klefki. They passed\n"
    "the Klefki down through the generations,\n"
    "taking good care of it.");

const u8 gPhantumpPokedexText[] = _(
    "This Pokémon came to be when the spirit of\n"
    "a deceased child possessed a tree stump.\n"
    "It now wanders the forest searching for\n"
    "friends.");

const u8 gTrevenantPokedexText[] = _(
    "Trevenant is very kind to Pokémon living\n"
    "in the forest. It doesn't even care if\n"
    "these Pokémon take up residence in the\n"
    "greenery on its head.");

const u8 gPumpkabooPokedexText[] = _(
    "The light that streams out from the holes\n"
    "in the pumpkin can hypnotize and control\n"
    "the people and Pokémon that see it.");

const u8 gGourgeistPokedexText[] = _(
    "Eerie cries emanate from its body in the\n"
    "dead of night. The sounds are said to be\n"
    "the wails of spirits who are suffering in\n"
    "the afterlife.");

const u8 gBergmitePokedexText[] = _(
    "They live in mountainous regions of frigid\n"
    "cold. On rare occasions, they ride on the\n"
    "back of an Avalugg to cross seas and move\n"
    "to new habitats.");

const u8 gAvaluggPokedexText[] = _(
    "This Pokémon uses its massive icy body to\n"
    "flatten anything that gets in its way.\n"
    "When it's floating out on the ocean, it\n"
    "looks exactly like drift ice.");

const u8 gNoibatPokedexText[] = _(
    "Noibat can change the frequency of its\n"
    "sound waves at will, and it generates\n"
    "ultrasonic waves of up to 200,000 hertz.");

const u8 gNoivernPokedexText[] = _(
    "In the dark, even Hydreigon are terrified\n"
    "of this Pokémon. But in the light, Noivern\n"
    "is the one that avoids battle.");

const u8 gXerneasPokedexText[] = _(
    "When the horns on its head shine in seven\n"
    "different colors, it is said to be sharing\n"
    "everlasting life.");

const u8 gYveltalPokedexText[] = _(
    "When this legendary Pokémon's wings and\n"
    "tail feathers spread wide and glow red, it\n"
    "absorbs the life force of living creatures.");

const u8 gZygardePokedexText[] = _(
    "Some say it can change to an even more\n"
    "powerful form when battling those who\n"
    "threaten the ecosystem.");

const u8 gDianciePokedexText[] = _(
    "It can instantly create many diamonds by\n"
    "compressing the carbon in the air between\n"
    "its hands.");

const u8 gHoopaPokedexText[] = _(
    "It gathers things it likes and passes them\n"
    "through its loop to teleport them to a\n"
    "secret place.");

const u8 gVolcanionPokedexText[] = _(
    "It lets out billows of steam and\n"
    "disappears into the dense fog.\n"
    "It's said to live in mountains\n"
    "where humans do not tread.");

const u8 gRowletPokedexText[] = _(
    "Silently it glides, streaking toward its\n"
    "targets to get close to them. Before they\n"
    "even notice it, it begins to pelt them with\n"
    "vicious kicks.");

const u8 gDartrixPokedexText[] = _(
    "This narcissistic Pokémon is a clean freak.\n"
    "If you don’t groom it diligently, it may\n"
    "stop listening to you.");

const u8 gDecidueyePokedexText[] = _(
    "It cunningly conceals arrow quills in its\n"
    "wings. It launches these quills with such\n"
    "precision that they can pierce a pebble at\n"
    "over a hundred yards.");

const u8 gLittenPokedexText[] = _(
    "While grooming itself, it builds up fur\n"
    "inside its stomach. It sets the fur alight\n"
    "and spews fiery attacks, which change\n"
    "based on how it coughs.");

const u8 gTorracatPokedexText[] = _(
    "Torracat will let its Trainer coddle it once\n"
    "they’ve grown close, but it’s a powerful,\n"
    "sharp-clawed Pokémon, so its Trainer gets\n"
    "covered in scratches.");

const u8 gIncineroarPokedexText[] = _(
    "After hurling ferocious punches and\n"
    "flinging furious kicks, it finishes\n"
    "opponents off by spewing fire from\n"
    "around its navel.");

const u8 gPopplioPokedexText[] = _(
    "The balloons it inflates with its nose\n"
    "grow larger and larger as it practices\n"
    "day by day.");

const u8 gBrionnePokedexText[] = _(
    "It cares deeply for its companions. When\n"
    "its Trainer is feeling down, it performs a\n"
    "cheery dance to try to help.");

const u8 gPrimarinaPokedexText[] = _(
    "Also known as a songstress, it is a sight\n"
    "to behold on moonlit nights when it sings\n"
    "in front of the colony it leads.");

const u8 gPikipekPokedexText[] = _(
    "Pikipek has strong muscles in its neck, so\n"
    "it won’t hurt itself even if it violently\n"
    "shakes its head.");

const u8 gTrumbeakPokedexText[] = _(
    "When it encounters foes, it launches the\n"
    "many seeds stored in its beak in a radial\n"
    "burst.");

const u8 gToucannonPokedexText[] = _(
    "Pairs of Toucannon are considered symbols\n"
    "of companionship, as these Pokémon will\n"
    "raise the temperature of their beaks to\n"
    "warm each other.");

const u8 gYungoosPokedexText[] = _(
    "It spends its waking hours searching for\n"
    "food. When Yungoos is hungry, its ferocity\n"
    "gains a certain edge.");

const u8 gGumshoosPokedexText[] = _(
    "Gumshoos specializes in relentlessly\n"
    "tracking down prey. The simple-minded\n"
    "Skwovet doesn't seem to provide\n"
    "Gumshoos enough challenge.");

const u8 gGrubbinPokedexText[] = _(
    "Grubbin normally live underground, but if\n"
    "they get unearthed by Hoothoot, they\n"
    "may be found gathered around electric\n"
    "Pokémon.");

const u8 gCharjabugPokedexText[] = _(
    "Protected by a sturdy shell, Charjabug\n"
    "eats large quantities of fresh and\n"
    "decomposing leaf litter in order to\n"
    "evolve.");

const u8 gVikavoltPokedexText[] = _(
    "When carrying a Charjabug, Vikavolt can\n"
    "receive electricity from it and then\n"
    "rapidly fire powerful electromagnetic\n"
    "beams from its large jaws.");

const u8 gCrabrawlerPokedexText[] = _(
    "If it loses its pincers, they'll quickly\n"
    "regrow. The pincers are popular\n"
    "ingredients in paella since their shells\n"
    "produce a tasty soup stock.");

const u8 gCrabminblePokedexText[] = _(
    "Though its punches are powerful, this\n"
    "Pokémon's movements are sluggish.\n"
    "It blows icy bubbles from its mouth to\n"
    "immobilize its opponents.");

const u8 gOricorioPokedexText[] = _(
    "This form of Oricorio has sipped red nectar.\n"
    "It whips up blazing flames as it moves to\n"
    "the steps of its passionate dance.");

const u8 gCutieflyPokedexText[] = _(
    "When it senses the auras of people or\n"
    "Pokémon who are having fun, it will draw\n"
    "close and give them a painful poke with its\n"
    "long proboscis.");

const u8 gRibombeePokedexText[] = _(
    "Ribombee can sense when people or Pokémon\n"
    "are feeling down, and it will cheer them up\n"
    "with handmade pollen puffs.");

const u8 gRockruffPokedexText[] = _(
    "This Pokémon is very friendly when it's\n"
    "young. Its disposition becomes vicious\n"
    "once it matures, but it never forgets the\n"
    "kindness of its master.");

const u8 gLycanrocPokedexText[] = _(
    "It has a calm and collected demeanor. It\n"
    "swiftly closes in on its prey, then slices\n"
    "them with the rocks in its mane.");

const u8 gWishiwashiPokedexText[] = _(
    "When it senses danger, its eyes tear up.\n"
    "The sparkle of its tears signals other\n"
    "Wishiwashi to gather.");

const u8 gMareaniePokedexText[] = _(
    "It has excellent regenerative capabilities.\n"
    "Legs that a Mareanie has lost have a very\n"
    "bitter, astringent flavor--they're not\n"
    "suitable as food.");

const u8 gToxapexPokedexText[] = _(
    "This Pokémon lives within a dome made by\n"
    "its own legs. Toxapex monitors its\n"
    "surroundings by sensing the flow of the\n"
    "tide through its spikes.");

const u8 gMudbrayPokedexText[] = _(
    "This Pokémon covers itself in mud that it\n"
    "has regurgitated. The mud won’t dry out\n"
    "even if it’s exposed to the sun for a long\n"
    "time.");

const u8 gMudsdalePokedexText[] = _(
    "This Pokémon has been treasured not just\n"
    "for its physical labor but also because it\n"
    "produces high-quality mud used for making\n"
    "pottery.");

const u8 gDewpiderPokedexText[] = _(
    "It supports the water bubble that\n"
    "envelops its head with three of its legs.\n"
    "Sometimes it secretly swaps which legs are\n"
    "holding up the bubble.");

const u8 gAraquanidPokedexText[] = _(
    "It launches sticky water bubbles. Foes who\n"
    "lose consciousness while trapped inside\n"
    "these bubbles will get torn apart by\n"
    "Araquanid’s gnashing jaws.");

const u8 gFomantisPokedexText[] = _(
    "Fomantis hates having its naps\n"
    "interrupted. It fires off beams using\n"
    "energy it gathers by bathing in the sun.");

const u8 gLurantisPokedexText[] = _(
    "A lot of time and effort is required to\n"
    "maintain the vivid colors of its petals.\n"
    "This Pokémon puts its Trainer's\n"
    "attentiveness to the test.");

const u8 gMorelullPokedexText[] = _(
    "Morelull live in forests that stay dark\n"
    "even during the day. They scatter\n"
    "flickering spores that put enemies to\n"
    "sleep.");

const u8 gShiinoticPokedexText[] = _(
    "Its flickering spores lure in prey and put\n"
    "them to sleep. Once this Pokémon has its\n"
    "prey snoozing, it drains their vitality\n"
    "with its fingertips.");

const u8 gSalanditPokedexText[] = _(
    "It taunts its prey and lures them into\n"
    "narrow, rocky areas where it then sprays\n"
    "them with toxic gas to make them dizzy and\n"
    "take them down.");

const u8 gSalazzlePokedexText[] = _(
    "Salazzle makes its opponents light-headed\n"
    "with poisonous gas, then captivates them\n"
    "with alluring movements to turn them into\n"
    "loyal servants.");

const u8 gStuffulPokedexText[] = _(
    "The way it protects itself by flailing its\n"
    "arms may be an adorable sight, but stay\n"
    "well away. This is flailing that can snap\n"
    "thick tree trunks.");

const u8 gBewearPokedexText[] = _(
    "The moves it uses to take down its prey\n"
    "would make a martial artist jealous. It\n"
    "tucks subdued prey under its arms to\n"
    "carry them to its nest.");

const u8 gBounsweetPokedexText[] = _(
    "Its sweat is sweet, like syrup made from\n"
    "boiled-down fruit. Because of this,\n"
    "Bounsweet was highly valued in the past,\n"
    "when sweeteners were scarce.");

const u8 gSteeneePokedexText[] = _(
    "Steenee bounces energetically through\n"
    "forests. If the rind that peels off its\n"
    "body is pulverized, it can be used to treat\n"
    "stomach pains.");

const u8 gTsareenaPokedexText[] = _(
    "This Pokémon is proud and aggressive.\n"
    "However, it is said that a Tsareena will\n"
    "instantly become calm if someone touches\n"
    "the crown on its calyx.");

const u8 gComfeyPokedexText[] = _(
    "It prefers areas with lots of flowers. A\n"
    "Comfey adorned with flowers is a happy\n"
    "Comfey, and the flowers gain healing\n"
    "effects.");

const u8 gOranguruPokedexText[] = _(
    "People used to mistake Oranguru for a\n"
    "human when they saw it issue command\n"
    "after command to the other Pokémon of\n"
    "the forest.");

const u8 gPassimianPokedexText[] = _(
    "The boss chooses 10 members of the group\n"
    "to go out hunting. The hunting party will\n"
    "evenly split the food they find with the\n"
    "rest of the group.");

const u8 gWimpodPokedexText[] = _(
    "It's nature's cleaner--it eats anything and\n"
    "everything, including garbage and rotten\n"
    "things. The ground near its nest is always\n"
    "clean.");

const u8 gGolisopodPokedexText[] = _(
    "It will do anything to win, taking\n"
    "advantage of every opening and finishing\n"
    "opponents off with the small claws on its\n"
    "front legs.");

const u8 gSandygastPokedexText[] = _(
    "It will do anything to win, taking\n"
    "advantage of every opening and finishing\n"
    "opponents off with the small claws on its\n"
    "front legs.");

const u8 gPalossandPokedexText[] = _(
    "The terrifying Palossand drags smaller\n"
    "Pokémon into its sandy body. Once its\n"
    "victims are trapped, it drains them of\n"
    "their vitality whenever it pleases.");

const u8 gPyukumukuPokedexText[] = _(
    "It's covered in a slime that keeps its skin\n"
    "moist, allowing it to stay on land for days\n"
    "without drying up.");

const u8 gTypeNullPokedexText[] = _(
    "A Pokémon weapon developed for a specific\n"
    "mission, it went berserk during an\n"
    "experiment, so it was cryogenically\n"
    "frozen.");

const u8 gSilvallyPokedexText[] = _(
    "A solid bond of trust between this\n"
    "Pokémon and its Trainer awakened the\n"
    "strength hidden within Silvally. It can\n"
    "change its type at will.");

const u8 gMiniorPokedexText[] = _(
    "It will react when touched or called to,\n"
    "but it’s unknown whether such responses\n"
    "indicate consciousness.");

const u8 gKomalaPokedexText[] = _(
    "A potent anesthetic can be made by\n"
    "diluting Komala's drool. This anesthetic\n"
    "was used for surgeries in the past.");

const u8 gTurtonatorPokedexText[] = _(
    "Eating sulfur in its volcanic habitat is\n"
    "what causes explosive compounds to\n"
    "develop in its shell. Its droppings are also\n"
    "dangerously explosive.");

const u8 gTogedemaruPokedexText[] = _(
    "With the long hairs on its back, this\n"
    "Pokémon takes in electricity from other\n"
    "electric Pokémon. It stores what it\n"
    "absorbs in an electric sac.");

const u8 gMimikyuPokedexText[] = _(
    "This Pokémon lives in dark places\n"
    "untouched by sunlight. When it appears\n"
    "before humans, it hides itself under a\n"
    "cloth that resembles a Pikachu.");

const u8 gBruxishPokedexText[] = _(
    "It grinds its teeth with great force to\n"
    "stimulate its brain. It fires the psychic\n"
    "energy created by this process from the\n"
    "protuberance on its head.");

const u8 gDrampaPokedexText[] = _(
    "The mountains it calls home are nearly two\n"
    "miles in height. On rare occasions, it\n"
    "descends to play with the children living\n"
    "in the towns below.");

const u8 gDhelmisePokedexText[] = _(
    "After lowering its anchor, it waits for its\n"
    "prey. It catches large Wailord and drains\n"
    "their life-force.");

const u8 gJangmoOPokedexText[] = _(
    "The scale atop its head is useful for both\n"
    "offense and defense. It bravely stands\n"
    "and faces its foes rather than turning\n"
    "tail, even if its foes are strong.");

const u8 gHakamoOPokedexText[] = _(
    "It fires itself up by striking its scales\n"
    "with force as it dances. Its roar is a\n"
    "battle cry.");

const u8 gKommoOPokedexText[] = _(
    "It bashes its scales to test its\n"
    "opponents' mettle. The sound of struck\n"
    "Kommo-o scales frightens weaker foes and\n"
    "sends them running.");

const u8 gTapuKokoPokedexText[] = _(
    "The lightning-wielding guardian deity of\n"
    "Melemele, Tapu Koko is brimming with\n"
    "curiosity and appears before people from\n"
    "time to time.");

const u8 gTapuLelePokedexText[] = _(
    "It heals the wounds of people and\n"
    "Pokémon by sprinkling them with its\n"
    "sparkling scales. This guardian deity is\n"
    "worshiped on Akala.");

const u8 gTapuBuluPokedexText[] = _(
    "It makes ringing sounds with its tail to\n"
    "let others know where it is, avoiding\n"
    "unneeded conflicts. This guardian deity\n"
    "of Ula'ula controls plants.");

const u8 gTapuFiniPokedexText[] = _(
    "This guardian deity of Poni Island\n"
    "manipulates water. Because it lives deep\n"
    "within a thick fog, it came to be both\n"
    "feared and revered.");

const u8 gCosmogPokedexText[] = _(
    "Cosmog is very curious but not very\n"
    "cautious, often placing itself in danger.\n"
    "If things start to look dicey, it teleports\n"
    "away.");

const u8 gCosmoemPokedexText[] = _(
    "It sucks in dust from the air at an\n"
    "astounding rate, frantically building up\n"
    "energy within its core as preparation for\n"
    "evolution.");

const u8 gSolgaleoPokedexText[] = _(
    "When light radiates from its body, this\n"
    "Pokémon could almost appear to be the sun.\n"
    "It will dispel any darkness and light up\n"
    "the world.");

const u8 gLunalaPokedexText[] = _(
    "Known as the Beast That Calls the Moon,\n"
    "this Pokémon lives by taking in any and all\n"
    "light and converting it into its own\n"
    "energy.");

const u8 gNihilegoPokedexText[] = _(
    "It appeared in this world from an Ultra\n"
    "Wormhole. Nihilego appears to be a\n"
    "parasite that lives by feeding on people\n"
    "and Pokémon.");

const u8 gBuzzwolePokedexText[] = _(
    "Although it's alien to this world and a\n"
    "danger here, it's apparently a common\n"
    "organism in the world where it normally\n"
    "lives.");

const u8 gPheromosaPokedexText[] = _(
    "Although it's alien to this world and a\n"
    "danger here, it's apparently a common\n"
    "organism in the world where it normally\n"
    "lives.");

const u8 gXurkitreePokedexText[] = _(
    "Although it's alien to this world and a\n"
    "danger here, it's apparently a common\n"
    "organism in the world where it normally\n"
    "lives.");

const u8 gCelesteelaPokedexText[] = _(
    "Although it's alien to this world and a\n"
    "danger here, it's apparently a common\n"
    "organism in the world where it normally\n"
    "lives.");

const u8 gKartanaPokedexText[] = _(
    "Although it's alien to this world and a\n"
    "danger here, it's apparently a common\n"
    "organism in the world where it normally\n"
    "lives.");

const u8 gGuzzlordPokedexText[] = _(
    "Although it's alien to this world and a\n"
    "danger here, it's apparently a common\n"
    "organism in the world where it normally\n"
    "lives.");

const u8 gNecrozmaPokedexText[] = _(
    "It survives by absorbing light. After a\n"
    "long time spent slumbering underground,\n"
    "impurities accumulated within it, causing\n"
    "its body to darken.");

const u8 gMagearnaPokedexText[] = _(
    "It synchronizes its consciousness with\n"
    "others to understand their feelings. This\n"
    "faculty makes it useful for taking care of\n"
    "people.");

const u8 gMarshadowPokedexText[] = _(
    "By slipping into the shadow of a martial\n"
    "arts master and copying their movements,\n"
    "this Pokémon learned the ultimate\n"
    "techniques.");

const u8 gPoipolePokedexText[] = _(
    "An Ultra Beast that lives in a different\n"
    "world, it cackles wildly as it sprays its\n"
    "opponents with poison from the needles on\n"
    "its head.");

const u8 gNaganadelPokedexText[] = _(
    "One kind of Ultra Beast, it fires a glowing,\n"
    "venomous liquid from its needles. This\n"
    "liquid is also immensely adhesive.");

const u8 gStakatakaPokedexText[] = _(
    "When stone walls started moving and\n"
    "attacking, the brute's true identity was\n"
    "this mysterious life-form, which brings to\n"
    "mind an Ultra Beast.");

const u8 gBlacphalonPokedexText[] = _(
    "A UB that appeared from an Ultra Wormhole,\n"
    "it causes explosions, then takes\n"
    "advantage of opponents' surprise to rob\n"
    "them of their vitality.");

const u8 gZeraoraPokedexText[] = _(
    "Electricity sparks from the pads on its\n"
    "limbs. Wherever Zeraora runs, lightning\n"
    "flashes and thunder echoes.");

const u8 gMeltanPokedexText[] = _(
    "They live as a group, but when the time\n"
    "comes, one strong Meltan will absorb all\n"
    "the others and evolve.");

const u8 gMelmetalPokedexText[] = _(
    "Centrifugal force is behind the punches of\n"
    "Melmetal's heavy hex-nut arms. Melmetal is\n"
    "said to deliver the strongest punches of\n"
    "all Pokémon.");

const u8 gGrookeyPokedexText[] = _(
    "The stick Grookey holds has grown harder\n"
    "and more flexible after soaking in the\n"
    "energy that emanates from inside\n"
    "Grookey’s body.");

const u8 gThwackeyPokedexText[] = _(
    "It becomes intensely focused while it\n"
    "drums out vigorous beats--so much so that\n"
    "sometimes it won’t listen to its Trainer’s\n"
    "commands.");

const u8 gRillaboomPokedexText[] = _(
    "Though it has a calm disposition, it won’t\n"
    "tolerate those who drum up discord. It\n"
    "strictly disciplines offenders until they\n"
    "learn their lesson.");

const u8 gScorbunnyPokedexText[] = _(
    "Fire energy gathers in the pads of its\n"
    "feet, raising their temperature. Once hot,\n"
    "Scorbunny’s footpads can deal heavy\n"
    "damage to opponents.");

const u8 gRabootPokedexText[] = _(
    "While it prides itself on its varied kicking\n"
    "moves, it can also deliver powerful\n"
    "headbutts once its flames have heated up\n"
    "its forehead.");

const u8 gCinderacePokedexText[] = _(
    "Using a ball that it made out of flames,\n"
    "Cinderace blinds its opponents and keeps\n"
    "them at bay.");

const u8 gSobblePokedexText[] = _(
    "It gets berries to eat by shooting them\n"
    "down with bullets of water it spurts from\n"
    "its mouth. Its aim is perfect.");

const u8 gDrizzilePokedexText[] = _(
    "It extends its long, slimy tongue at\n"
    "blinding speeds and finishes off its prey\n"
    "with great skill.");

const u8 gInteleonPokedexText[] = _(
    "It may present itself as being well-\n"
    "mannered, but deep down, it still has a\n"
    "lazy side. It will slack off when its\n"
    "Trainer isn’t looking.");

const u8 gSkwovetPokedexText[] = _(
    "No matter how much it stuffs its belly with\n"
    "food, it is always anxious about getting\n"
    "hungry again. So, it stashes berries in its\n"
    "cheeks and tail.");

const u8 gGreedentPokedexText[] = _(
    "This Pokémon makes off with heaps of\n"
    "fallen berries by wrapping them in its tail,\n"
    "which is roughly twice the length of its\n"
    "body.");

const u8 gRookideePokedexText[] = _(
    "The females are fussier than the males. If\n"
    "another creature dirties a female\n"
    "Rookidee's wings, it'll peck the offender\n"
    "relentlessly in a burning rage.");

const u8 gCorvsquirePokedexText[] = _(
    "It’s said that the reason behind\n"
    "Corvisquire’s high level of intelligence is\n"
    "the large size of its brain relative to\n"
    "those of other bird Pokémon.");

const u8 gCorvknightPokedexText[] = _(
    "Corviknight can’t serve as a taxi service\n"
    "in a faraway region because the Pokémon’s\n"
    "natural predators will attack it while it\n"
    "flies, endangering the customer.");

const u8 gBlipbugPokedexText[] = _(
    "Often found in gardens, this Pokémon has\n"
    "hairs on its body that it uses to assess\n"
    "its surroundings.");

const u8 gDottlerPokedexText[] = _(
    "It barely moves, but it's still alive. Hiding\n"
    "in its shell without food or water seems to\n"
    "have awakened its psychic powers.");

const u8 gOrbeetlePokedexText[] = _(
    "It's famous for its high level of\n"
    "intelligence, and the large size of its\n"
    "brain is proof that it also possesses\n"
    "immense psychic power.");

const u8 gNickitPokedexText[] = _(
    "Cunning and cautious, this Pokémon\n"
    "survives by stealing food from others. It\n"
    "erases its tracks with swipes of its tail\n"
    "as it makes off with its plunder.");

const u8 gThievulPokedexText[] = _(
    "It secretly marks potential targets with\n"
    "a scent. By following the scent, it stalks\n"
    "its targets and steals from them when\n"
    "they least expect it.");

const u8 gGossifleurPokedexText[] = _(
    "It anchors itself in the ground with its\n"
    "single leg, then basks in the sun. After\n"
    "absorbing enough sunlight, its petals\n"
    "spread as it blooms brilliantly.");

const u8 gEldegossPokedexText[] = _(
    "The seeds attached to its cotton fluff\n"
    "are full of nutrients. It spreads them on\n"
    "the wind so that plants and other Pokémon\n"
    "can benefit from them.");

const u8 gWoolooPokedexText[] = _(
    "Its curly fleece is such an effective\n"
    "cushion that this Pokémon could fall off a\n"
    "cliff and stand right back up at the\n"
    "bottom, unharmed.");

const u8 gDubwoolPokedexText[] = _(
    "Weave a carpet from its springy wool, and\n"
    "you end up with something closer to a\n"
    "trampoline. You'll start to bounce the\n"
    "moment you set foot on it.");

const u8 gChewtlePokedexText[] = _(
    "Its large front tooth is still growing in.\n"
    "When the tooth itches, this Pokémon will\n"
    "bite another Chewtle’s horn, and the two\n"
    "Pokémon will tussle.");

const u8 gDrednawPokedexText[] = _(
    "Drednaw lurks along the shoreline. When\n"
    "prey come to drink water, Drednaw\n"
    "stretches its neck out and chomps down\n"
    "on them.");

const u8 gYamperPokedexText[] = _(
    "This gluttonous Pokémon only assists\n"
    "people with their work because it wants\n"
    "treats. As it runs, it crackles with\n"
    "electricity.");

const u8 gBoltundPokedexText[] = _(
    "This Pokémon generates electricity and\n"
    "channels it into its legs to keep them\n"
    "going strong. Boltund can run nonstop for\n"
    "three full days.");

const u8 gRolycolyPokedexText[] = _(
    "On sunny days, Rolycoly will come out onto\n"
    "grassy plains and roll around. Then it will\n"
    "eat the grass that gets tangled in its\n"
    "wheel-like leg.");

const u8 gCarkolPokedexText[] = _(
    "Due to the coal tar created inside it, the\n"
    "heap of coal on Carkol's back never falls\n"
    "apart, even when the Pokémon rolls around\n"
    "at high speeds.");

const u8 gCoalossalPokedexText[] = _(
    "It's gentle usually but fearsome when\n"
    "angered. With a body that burns at over\n"
    "2,700 degrees Fahrenheit, it crushes foes\n"
    "and turns them to ash.");

const u8 gApplinPokedexText[] = _(
    "It grows by eating the flesh of an apple.\n"
    "Applin uses its own body fluid to\n"
    "strengthen the apple’s skin and prevent\n"
    "it from rotting.");

const u8 gFlapplePokedexText[] = _(
    "It flutters around while seeking an\n"
    "opening in the opponent’s guard, then\n"
    "attacks by spitting acidic liquid strong\n"
    "enough to melt metal.");

const u8 gAppletunPokedexText[] = _(
    "When some Lechonk come along to lick the\n"
    "sweet nectar on Appletun's back, Appletun\n"
    "will spew sticky nectar at them to drive\n"
    "them away.");

const u8 gSilicobraPokedexText[] = _(
    "Silicobra's large nostrils are specialized\n"
    "for spraying sand, so this Pokémon is not\n"
    "very good at telling apart different\n"
    "smells.");

const u8 gSandacondaPokedexText[] = _(
    "It will expand its body as much as it can\n"
    "and then contract itself, blasting out\n"
    "sand with enough force to wash away a\n"
    "dump truck.");

const u8 gCramorantPokedexText[] = _(
    "Cramorant instinctively swallow\n"
    "everything whole. They're at their most\n"
    "formidable when they're struggling to\n"
    "swallow overly large prey.");

const u8 gArrokudaPokedexText[] = _(
    "Arrokuda can swim quickly only in a\n"
    "straight line. The ones that become\n"
    "sluggish from overeating are the first to\n"
    "be targeted by flocks of Wattrel.");

const u8 gBaraskewdaPokedexText[] = _(
    "It spins its tail fins to leap from the\n"
    "water, then viciously bites down on Wingull\n"
    "flying close to the water’s surface.");

const u8 gToxelPokedexText[] = _(
    "It has no problem drinking dirty water. An\n"
    "organ inside Toxel’s body filters such\n"
    "water into a poisonous liquid that is\n"
    "harmless to Toxel.");

const u8 gToxtricityPokedexText[] = _(
    "As it scatters toxic sweat and emits\n"
    "electricity, a melody that sounds like it\n"
    "came from a guitar reverberates through\n"
    "the surrounding area.");

const u8 gSizzlipedePokedexText[] = _(
    "It wraps prey up with its heated body,\n"
    "cooking them in its coils. Once they're\n"
    "well-done, it will voraciously nibble them\n"
    "down to the last morsel.");

const u8 gCentskorchPokedexText[] = _(
    "When it heats up, its body temperature\n"
    "reaches about 1,500 degrees Fahrenheit.\n"
    "It lashes its body like a whip and launches\n"
    "itself at enemies.");

const u8 gClobbopusPokedexText[] = _(
    "It's very curious, but its means of\n"
    "investigating things is to try to punch\n"
    "them with its tentacles. The search for\n"
    "food is what brings it onto land.");

const u8 gGrapploctPokedexText[] = _(
    "A body made up of nothing but muscle\n"
    "makes the grappling moves this Pokémon\n"
    "performs with its tentacles tremendously\n"
    "powerful.");

const u8 gSinisteaPokedexText[] = _(
    "The soul of someone who died alone\n"
    "possessed some leftover tea. This\n"
    "Pokémon appears in hotels and houses.");

const u8 gPoltegeistPokedexText[] = _(
    "The tea that composes Polteageist’s body\n"
    "has a distinct and enjoyable flavor.\n"
    "Drinking too much, however, can be fatal.");

const u8 gHatennaPokedexText[] = _(
    "It senses the feelings of other living\n"
    "creatures. Be careful not to expose it to\n"
    "strong emotions for too long, or it will end\n"
    "up exhausted.");

const u8 gHattremPokedexText[] = _(
    "The moment this Pokémon finds someone\n"
    "who's emitting strong emotions, it will\n"
    "pummel them senseless with its braids to\n"
    "silence them.");

const u8 gHatterenePokedexText[] = _(
    "Hatterene knocks out those that intrude\n"
    "in its home forest by blasting them with a\n"
    "beam, then slashing with claws enhanced by\n"
    "psychic power.");

const u8 gImpidimpPokedexText[] = _(
    "The reason this Pokémon causes trouble\n"
    "for those it feels close to is because\n"
    "Impidimp itself gets irritable if it can’t\n"
    "absorb negative emotions.");

const u8 gMorgremPokedexText[] = _(
    "This Pokémon absorbs negative emotions\n"
    "and turns them into energy. It's popular\n"
    "with people who tend to think gloomy\n"
    "thoughts.");

const u8 gGrimmsnarlPokedexText[] = _(
    "This Pokémon has complete control over\n"
    "its hair. Grimmsnarl normally keeps its hair\n"
    "wrapped around its body to support its\n"
    "muscles.");

const u8 gObstagoonPokedexText[] = _(
    "Its voice is staggering in volume.\n"
    "Obstagoon has a tendency to take on a\n"
    "threatening posture and shout--this move\n"
    "is known as Obstruct.");

const u8 gPerrserkerPokedexText[] = _(
    "What appears to be an iron helmet is\n"
    "actually hardened hair. This Pokémon lives\n"
    "for the thrill of battle.");

const u8 gCursolaPokedexText[] = _(
    "Its shell is overflowing with its\n"
    "heightened otherworldly energy. The\n"
    "ectoplasm serves as protection for this\n"
    "Pokémon's core spirit.");

const u8 gSirfetchdPokedexText[] = _(
    "Only Farfetch'd that have survived many\n"
    "battles can attain this evolution. When\n"
    "this Pokémon's leek withers, it will retire\n"
    "from combat.");

const u8 gMrRimePokedexText[] = _(
    "Its amusing movements make it very\n"
    "popular. It releases its psychic power\n"
    "from the pattern on its belly.");

const u8 gRunerigusPokedexText[] = _(
    "A powerful curse was woven into an ancient\n"
    "painting. After absorbing the spirit of a\n"
    "Yamask, the painting began to move.");

const u8 gMilceryPokedexText[] = _(
    "When attacked, it tackles its foe and\n"
    "covers them in cream. With its foe either\n"
    "blinded or astonished by deliciousness,\n"
    "Milcery flees.");

const u8 gAlcremiePokedexText[] = _(
    "Desserts that Alcremie have decorated\n"
    "with their cream have a rich, sweet flavor\n"
    "and bring happiness to all who eat them.");

const u8 gFalinksPokedexText[] = _(
    "The leader, known as the brass, uses its\n"
    "extendible horn to issue orders to the\n"
    "others when it's time to change formation.");

const u8 gPincurchinPokedexText[] = _(
    "This Pokémon is so timid that even\n"
    "brushing against seaweed will make it\n"
    "discharge electricity in surprise.\n"
    "Its lips do not conduct electricity.");

const u8 gSnomPokedexText[] = _(
    "It eats snow that has accumulated on the\n"
    "ground. It prefers soft, freshly fallen\n"
    "snow, so it will eat its way up a mountain,\n"
    "aiming for the peak.");

const u8 gFrosmothPokedexText[] = _(
    "It causes blizzards as it flies around with\n"
    "its huge, chill-emanating wings. Clean\n"
    "meltwater is its favorite thing to drink.");

const u8 gStonjornerPokedexText[] = _(
    "This Pokémon spends its life gazing at the\n"
    "setting sun. It strides leisurely across\n"
    "grassy plains on legs of rock that weigh\n"
    "over 400 pounds each.");

const u8 gEiscuePokedexText[] = _(
    "If you lick the ice covering its face,\n"
    "you'll find it has a faintly salty taste.\n"
    "This species rode here on ocean currents\n"
    "from a cold, faraway land.");

const u8 gIndeedeePokedexText[] = _(
    "This Pokémon never leaves its Trainer's\n"
    "side. It predicts their actions with its\n"
    "psychic power and takes care of their\n"
    "day-to-day needs.");

const u8 gMorpekoPokedexText[] = _(
    "It has a small stomach. If it isn't\n"
    "constantly eating the seeds it keeps in\n"
    "its pockets, it will get hungry immediately.");

const u8 gCufantPokedexText[] = _(
    "Cufant can lift loads weighing five tons.\n"
    "In the mornings, it heads into caves with\n"
    "its herd, in search of the ore on which\n"
    "these Pokémon feed.");

const u8 gCopperajahPokedexText[] = _(
    "Copperajah are prideful, cantankerous\n"
    "Pokémon. Specimens with vibrant green\n"
    "skin command the respect of others of\n"
    "their kind.");

const u8 gDracozoltPokedexText[] = _(
    "In ancient times, it was unbeatable thanks\n"
    "to its powerful lower body, but it went\n"
    "extinct anyway after it depleted all its\n"
    "plant-based food sources.");

const u8 gArctozoltPokedexText[] = _(
    "This Pokémon lived on prehistoric\n"
    "seashores and was able to preserve food\n"
    "with the ice on its body. It went extinct\n"
    "because it moved so slowly.");

const u8 gDracovishPokedexText[] = _(
    "Powerful legs and jaws made it the apex\n"
    "predator of its time. Its own overhunting\n"
    "of its prey was what drove it to\n"
    "extinction.");

const u8 gArctovishPokedexText[] = _(
    "Though it's able to capture prey by\n"
    "freezing its surroundings, it has trouble\n"
    "eating the prey afterward because its\n"
    "mouth is on top of its head.");

const u8 gDuraludonPokedexText[] = _(
    "Duraludon’s body is comprised of a special\n"
    "metal that’s lightweight and scratch\n"
    "resistant. It’s also smooth, as though it\n"
    "was given a mirror finish.");

const u8 gDreepyPokedexText[] = _(
    "It has a habit of biting at Clauncher even\n"
    "though it doesn't feed on them. This is\n"
    "said to be vestigial behavior from when\n"
    "Dreepy was alive.");

const u8 gDrakloakPokedexText[] = _(
    "This Pokémon flies around at over 120\n"
    "miles per hour. If a Drakloak is defeated\n"
    "in a battle, its Dreepy will wander off\n"
    "without a second thought.");

const u8 gDragapultPokedexText[] = _(
    "Dragapult can make its whole body\n"
    "transparent by clearing its mind and\n"
    "focusing. Even the Dreepy in Dragapult’s\n"
    "horns become invisible.");

const u8 gZacianPokedexText[] = _(
    "This Pokémon has slumbered for many years.\n"
    "Some say it's Zamazenta's elder sister--\n"
    "others say the two Pokémon are rivals.");

const u8 gZamazentaPokedexText[] = _(
    "In times past, it worked together with a\n"
    "king of the people to save the Galar\n"
    "region. It absorbs metal that it then\n"
    "uses in battle.");

const u8 gEternatusPokedexText[] = _(
    "The core on its chest absorbs energy\n"
    "emanating from the lands of the Galar\n"
    "region. This energy is what allows\n"
    "Eternatus to stay active.");

const u8 gKubfuPokedexText[] = _(
    "If Kubfu pulls the long white hair on its\n"
    "head, its fighting spirit heightens and\n"
    "power wells up from the depths of its\n"
    "belly.");

const u8 gUrshifuPokedexText[] = _(
    "This form of Urshifu is a strong believer in\n"
    "the one-hit KO. Its strategy is to leap in\n"
    "close to foes and land a devastating blow\n"
    "with a hardened fist.");

const u8 gZarudePokedexText[] = _(
    "Within dense forests, this Pokémon lives\n"
    "in a pack with others of its kind. It's\n"
    "incredibly aggressive, and the other\n"
    "Pokémon of the forest fear it.");

const u8 gRegielekiPokedexText[] = _(
    "This Pokémon is a cluster of electrical\n"
    "energy. It's said that removing the rings\n"
    "on Regieleki's body will unleash the\n"
    "Pokémon's latent power.");

const u8 gRegidragoPokedexText[] = _(
    "An academic theory proposes that\n"
    "Regidrago's arms were once the head of an\n"
    "ancient dragon Pokémon. The theory\n"
    "remains unproven.");

const u8 gGlastrierPokedexText[] = _(
    "Glastrier has tremendous physical\n"
    "strength, and the mask of ice covering its\n"
    "face is 100 times harder than diamond.");

const u8 gSpectrierPokedexText[] = _(
    "It probes its surroundings with all its\n"
    "senses save one--it doesn't use its sense\n"
    "of sight. Spectrier's kicks are said to\n"
    "separate soul from body.");

const u8 gCalyrexPokedexText[] = _(
    "Calyrex is known in legend as a king that\n"
    "ruled over Galar in ancient times. It has\n"
    "the power to cause hearts to mend and\n"
    "plants to spring forth.");

const u8 gWyrdeerPokedexText[] = _(
    "The black orbs shine with an uncanny light\n"
    "when it is erecting invisible barriers.\n"
    "The fur shed from its beard retains heat\n"
    "and is useful for winter clothing.");

const u8 gKleavorPokedexText[] = _(
    "This Pokémon is a rough, crude, and violent\n"
    "sort. It swings around its large, heavy\n"
    "stone axes to finish off its prey.");

const u8 gUrsalunaPokedexText[] = _(
    "Swampy terrain gives Ursaluna its burly\n"
    "physique and newfound capacity to\n"
    "manipulate peat at will.");

const u8 gBasclegionPokedexText[] = _(
    "This Pokémon is cloaked in the souls of its\n"
    "comrades who perished during a punishing\n"
    "journey to the river of their birth.");

const u8 gSneaslerPokedexText[] = _(
    "Because of its wicked poison and daunting\n"
    "physical prowess, no other species could\n"
    "hope to best it on frozen highlands. It\n"
    "prefers solitude and doesn't form packs.");

const u8 gOverqwilPokedexText[] = _(
    "It’s ferocious and has a short temper. The\n"
    "ends of its spikes are barbed, and they\n"
    "can’t be easily removed once they’ve\n"
    "punctured something.");

const u8 gEnamorusPokedexText[] = _(
    "Its arrival brings an end to the winter.\n"
    "According to legend, this Pokémon's love\n"
    "gives rise to the budding of fresh life\n"
    "across the land.");

const u8 gSprigatitoPokedexText[] = _(
    "The sweet scent its body gives off\n"
    "mesmerizes those around it. The scent\n"
    "grows stronger when this Pokémon is in the\n"
    "sun.");

const u8 gFloragatoPokedexText[] = _(
    "The hardness of Floragato's fur depends\n"
    "on the Pokémon's mood. When Floragato is\n"
    "prepared to battle, its fur becomes\n"
    "pointed and needle sharp.");

const u8 gMeowscradaPokedexText[] = _(
    "With skillful misdirection, it rigs foes\n"
    "with pollen-packed flower bombs.\n"
    "Meowscarada sets off the bombs before\n"
    "its foes realize what's going on.");

const u8 gFuecocoPokedexText[] = _(
    "Its flame sac is small, so energy is always\n"
    "leaking out. This energy is released from\n"
    "the dent atop Fuecoco's head and flickers\n"
    "to and fro.");

const u8 gCrocalorPokedexText[] = _(
    "The valve in Crocalor's flame sac is closely\n"
    "connected to its vocal cords. This Pokémon\n"
    "utters a guttural cry as it spews flames\n"
    "every which way.");

const u8 gSkeledirgePokedexText[] = _(
    "The fiery bird changes shape when\n"
    "Skeledirge sings. Rumor has it that the\n"
    "bird was born when the fireball on\n"
    "Skeledirge’s head gained a soul.");

const u8 gQuaxlyPokedexText[] = _(
    "Its strong legs let it easily swim around in\n"
    "even fast-flowing rivers. It likes to keep\n"
    "things tidy and is prone to overthinking\n"
    "things.");

const u8 gQuaxwellPokedexText[] = _(
    "These Pokémon constantly run through\n"
    "shallow waters to train their legs, then\n"
    "compete with each other to see which of\n"
    "them kicks most gracefully.");

const u8 gQuaquavalPokedexText[] = _(
    "Dancing in ways that evoke far-away\n"
    "places, this Pokémon mesmerizes all that\n"
    "see it. Flourishes of its decorative water\n"
    "feathers slice into its foes.");

const u8 gLechonkPokedexText[] = _(
    "It searches for food all day. It possesses\n"
    "a keen sense of smell but doesn’t use it\n"
    "for anything other than foraging.");

const u8 gOinkolognePokedexText[] = _(
    "Oinkologne is proud of its fine, glossy\n"
    "skin. It emits a concentrated scent from\n"
    "the tip of its tail.");

const u8 gTarountulaPokedexText[] = _(
    "The thread it secretes from its rear is as\n"
    "strong as wire. The secret behind the\n"
    "thread's strength is the topic of ongoing\n"
    "research.");

const u8 gSpidopsPokedexText[] = _(
    "It clings to branches and ceilings using\n"
    "its threads and moves without a sound. It\n"
    "takes out its prey before the prey even\n"
    "notices it.");

const u8 gNymblePokedexText[] = _(
    "It's highly skilled at a fighting style in\n"
    "which it uses its jumping capabilities to\n"
    "dodge incoming attacks while also dealing\n"
    "damage to opponents.");

const u8 gLokixPokedexText[] = _(
    "It uses its normally folded third set of\n"
    "legs when in Showdown Mode. This places a\n"
    "huge burden on its body, so it can't stay in\n"
    "this mode for long.");

const u8 gPawmiPokedexText[] = _(
    "It has underdeveloped electric sacs on\n"
    "its cheeks. These sacs can produce\n"
    "electricity only if Pawmi rubs them\n"
    "furiously with the pads on its forepaws.");

const u8 gPawmoPokedexText[] = _(
    "When its group is attacked, Pawmo is the\n"
    "first to leap into battle, defeating\n"
    "enemies with a fighting technique that\n"
    "utilizes electric shocks.");

const u8 gPawmotPokedexText[] = _(
    "This Pokémon normally is slow to react,\n"
    "but once it enters battle, it will strike\n"
    "down its enemies with lightning-fast\n"
    "movements.");

const u8 gTandemausPokedexText[] = _(
    "Exhibiting great teamwork, they use their\n"
    "incisors to cut pieces out of any material\n"
    "that might be useful for a nest, then make\n"
    "off with them.");

const u8 gMausholdPokedexText[] = _(
    "The little one(s) just appeared one day.\n"
    "They all live together like a family, but\n"
    "the relationship between the three is\n"
    "still unclear.");

const u8 gFidoughPokedexText[] = _(
    "This Pokémon is smooth and moist to the\n"
    "touch. Yeast in Fidough’s breath induces\n"
    "fermentation in the Pokémon’s vicinity.");

const u8 gDachsbunPokedexText[] = _(
    "The pleasant aroma that emanates from\n"
    "this Pokémon’s body helps wheat grow, so\n"
    "Dachsbun has been treasured by farming\n"
    "villages.");

const u8 gSmolivPokedexText[] = _(
    "It protects itself from enemies by\n"
    "emitting oil from the fruit on its head.\n"
    "This oil is bitter and astringent enough\n"
    "to make someone flinch.");

const u8 gDollivPokedexText[] = _(
    "It basks in the sun to its heart's content\n"
    "until the fruits on its head ripen. After\n"
    "that, Dolliv departs from human\n"
    "settlements and goes on a journey.");

const u8 gArbolivaPokedexText[] = _(
    "This Pokémon drives back enemies by\n"
    "launching its rich, aromatic oil at them\n"
    "with enough force to smash a boulder.");

const u8 gSquawkbilyPokedexText[] = _(
    "The largest of their flocks can contain\n"
    "more than 50 individuals. They fly around\n"
    "towns and forests, searching for food and\n"
    "making a racket.");

const u8 gSquawkbilyYellowPlumagePokedexText[] = _(
    "These Squawkabilly are hotheaded, and\n"
    "their fighting style is vicious.\n"
    "They'll leap within reach of their\n"
    "foes to engage in close combat.");

const u8 gNacliPokedexText[] = _(
    "The ground scrapes its body as it travels,\n"
    "causing it to leave salt behind. Salt is\n"
    "constantly being created and replenished\n"
    "inside Nacli's body.");

const u8 gNaclstackPokedexText[] = _(
    "This Pokémon dry cures its prey by\n"
    "spraying salt over them. The curing\n"
    "process steals away the water in the\n"
    "prey’s body.");

const u8 gGarganaclPokedexText[] = _(
    "Garganacl will rub its fingertips together\n"
    "and sprinkle injured Pokémon with salt.\n"
    "Even severe wounds will promptly heal\n"
    "afterward.");

const u8 gCharcadetPokedexText[] = _(
    "Burnt charcoal came to life and became a\n"
    "Pokémon. Possessing a fiery fighting\n"
    "spirit, Charcadet will battle even tough\n"
    "opponents.");

const u8 gArmarougePokedexText[] = _(
    "Armarouge evolved through the use of a\n"
    "set of armor that belonged to a\n"
    "distinguished warrior. This Pokémon is\n"
    "incredibly loyal.");

const u8 gCeruledgePokedexText[] = _(
    "The fiery blades on its arms burn fiercely\n"
    "with the lingering resentment of a sword\n"
    "wielder who fell before accomplishing their\n"
    "goal.");

const u8 gTadbulbPokedexText[] = _(
    "Tadbulb shakes its tail to generate\n"
    "electricity. If it senses danger, it will\n"
    "make its head blink on and off to alert its\n"
    "allies.");

const u8 gBelliboltPokedexText[] = _(
    "When this Pokémon expands and contracts\n"
    "its wobbly body, the belly-button dynamo\n"
    "in its stomach produces a huge amount of\n"
    "electricity.");

const u8 gWattrelPokedexText[] = _(
    "When its wings catch the wind, the bones\n"
    "within produce electricity. This Pokémon\n"
    "dives into the ocean, catching prey by\n"
    "electrocuting them.");

const u8 gKilowattrlPokedexText[] = _(
    "It uses its throat sac to store\n"
    "electricity generated by its wings.\n"
    "There's hardly any oil in its feathers,\n"
    "so it is a poor swimmer.");

const u8 gMaschiffPokedexText[] = _(
    "It always scowls in an attempt to make\n"
    "opponents take it seriously, but even\n"
    "crying children will burst into laughter\n"
    "when they see Maschiff’s face.");

const u8 gMabosstiffPokedexText[] = _(
    "Mabosstiff loves playing with children.\n"
    "Though usually gentle, it takes on an\n"
    "intimidating look when protecting its\n"
    "family.");

const u8 gShroodlePokedexText[] = _(
    "Though usually a mellow Pokémon, it will\n"
    "sink its sharp, poison-soaked front\n"
    "teeth into any that anger it, causing\n"
    "paralysis in the object of its ire.");

const u8 gGrafaiaiPokedexText[] = _(
    "The color of the poisonous saliva depends\n"
    "on what the Pokémon eats. Grafaiai covers\n"
    "its fingers in its saliva and draws\n"
    "patterns on trees in forests.");

const u8 gBramblinPokedexText[] = _(
    "A soul unable to move on to the afterlife\n"
    "was blown around by the wind until it got\n"
    "tangled up with dried grass and became a\n"
    "Pokémon.");

const u8 gBramblgastPokedexText[] = _(
    "It will open the branches of its head to\n"
    "envelop its prey. Once it absorbs all the\n"
    "life energy it needs, it expels the prey\n"
    "and discards it.");

const u8 gToedscoolPokedexText[] = _(
    "Though it looks like Tentacool, Toedscool\n"
    "is a completely different species.\n"
    "Its legs may be thin, but it can run at a\n"
    "speed of 30 mph.");

const u8 gToedscruelPokedexText[] = _(
    "It coils its 10 tentacles around prey and\n"
    "sucks out their nutrients, causing the\n"
    "prey pain. The folds along the rim of its\n"
    "head are a popular delicacy.");

const u8 gKlawfPokedexText[] = _(
    "Klawf hangs upside-down from cliffs,\n"
    "waiting for prey. But Klawf can’t remain in\n"
    "this position for long because its blood\n"
    "rushes to its head.");

const u8 gCapsakidPokedexText[] = _(
    "The more sunlight this Pokémon bathes in,\n"
    "the more spicy chemicals are produced by\n"
    "its body, and thus the spicier its moves\n"
    "become.");

const u8 gScovillainPokedexText[] = _(
    "The green head has turned vicious due to\n"
    "the spicy chemicals stimulating its brain.\n"
    "Once it goes on a rampage, there is no\n"
    "stopping it.");

const u8 gRellorPokedexText[] = _(
    "This Pokémon creates a mud ball by mixing\n"
    "sand and dirt with psychic energy. It\n"
    "treasures its mud ball more than its own\n"
    "life.");

const u8 gRabscaPokedexText[] = _(
    "The body that supports the ball barely\n"
    "moves. Therefore, it is thought that the\n"
    "true body of this Pokémon is actually\n"
    "inside the ball.");

const u8 gFlittlePokedexText[] = _(
    "Flittle’s toes levitate about half an inch\n"
    "above the ground because of the psychic\n"
    "power emitted from the frills on the\n"
    "Pokémon’s belly.");

const u8 gEspathraPokedexText[] = _(
    "It immobilizes opponents by bathing them\n"
    "in psychic power from its large eyes.\n"
    "Despite its appearance, it has a vicious\n"
    "temperament.");

const u8 gTinkatinkPokedexText[] = _(
    "This Pokémon pounds iron scraps together\n"
    "to make a hammer. It will remake the hammer\n"
    "again and again until it's satisfied with\n"
    "the result.");

const u8 gTinkatuffPokedexText[] = _(
    "These Pokémon make their homes in piles of\n"
    "scrap metal. They test the strength of\n"
    "each other's hammers by smashing them\n"
    "together.");

const u8 gTinkatonPokedexText[] = _(
    "The hammer tops 220 pounds, yet it gets\n"
    "swung around easily by Tinkaton as it\n"
    "steals whatever it pleases and carries its\n"
    "plunder back home.");

const u8 gWiglettPokedexText[] = _(
    "Though it looks like Diglett, Wiglett is an\n"
    "entirely different species. The\n"
    "resemblance seems to be a coincidental\n"
    "result of environmental adaptation.");

const u8 gWugtrioPokedexText[] = _(
    "It has a vicious temperament, contrary\n"
    "to what its appearance may suggest. It\n"
    "wraps its long bodies around prey, then\n"
    "drags the prey into its den.");

const u8 gBombirdierPokedexText[] = _(
    "It gathers things up in an apron made from\n"
    "shed feathers added to the Pokémon’s\n"
    "chest feathers, then drops those things\n"
    "from high places for fun.");

const u8 gFinizenPokedexText[] = _(
    "It likes playing with others of its kind\n"
    "using the water ring on its tail. It uses\n"
    "ultrasonic waves to sense the emotions of\n"
    "other living creatures.");

const u8 gPalafinPokedexText[] = _(
    "This Pokémon changes its appearance if it\n"
    "hears its allies calling for help. Palafin\n"
    "will never show anybody its moment of\n"
    "transformation.");

const u8 gVaroomPokedexText[] = _(
    "It is said that this Pokémon was born when\n"
    "an unknown poison Pokémon entered and\n"
    "inspirited an engine left at a scrap-\n"
    "processing factory.");

const u8 gRevavroomPokedexText[] = _(
    "It creates a gas out of poison and\n"
    "minerals from rocks. It then detonates\n"
    "the gas in its cylinders--now numbering\n"
    "eight--to generate energy.");

const u8 gCyclizarPokedexText[] = _(
    "Apparently Cyclizar has been allowing\n"
    "people to ride on its back since ancient\n"
    "times. Depictions of this have been found\n"
    "in 10,000-year-old murals.");

const u8 gOrthwormPokedexText[] = _(
    "When attacked, this Pokémon will wield\n"
    "the tendrils on its body like fists and\n"
    "pelt the opponent with a storm of\n"
    "punches.");

const u8 gGlimmetPokedexText[] = _(
    "Glimmet's toxic mineral crystals look just\n"
    "like flower petals. This Pokémon scatters\n"
    "poisonous powder like pollen to protect\n"
    "itself.");

const u8 gGlimmoraPokedexText[] = _(
    "Glimmora's petals are made of crystallized\n"
    "poison energy. It has recently become\n"
    "evident that these petals resemble Tera\n"
    "Jewels.");

const u8 gGreavardPokedexText[] = _(
    "It is said that a dog Pokémon that died in\n"
    "the wild without ever interacting with a\n"
    "human was reborn as this Pokémon.");

const u8 gHoundstonePokedexText[] = _(
    "Houndstone spends most of its time\n"
    "sleeping in graveyards. Among all the dog\n"
    "Pokémon, this one is most loyal to its\n"
    "master.");

const u8 gFlamigoPokedexText[] = _(
    "Thanks to a behavior of theirs known as\n"
    "“synchronizing,” an entire flock of these\n"
    "Pokémon can attack simultaneously in\n"
    "perfect harmony.");

const u8 gCetoddlePokedexText[] = _(
    "This species left the ocean and began\n"
    "living on land a very long time ago.\n"
    "It seems to be closely related to Wailmer.");

const u8 gCetitanPokedexText[] = _(
    "Ice energy builds up in the horn on its\n"
    "upper jaw, causing the horn to reach\n"
    "cryogenic temperatures that freeze its\n"
    "surroundings.");

const u8 gVeluzaPokedexText[] = _(
    "When Veluza discards unnecessary flesh,\n"
    "its mind becomes honed and its psychic\n"
    "power increases. The spare flesh has a mild\n"
    "but delicious flavor.");

const u8 gDondozoPokedexText[] = _(
    "It treats Tatsugiri like its boss and\n"
    "follows it loyally. Though powerful,\n"
    "Dondozo is apparently not very smart.");

const u8 gTatsugiriPokedexText[] = _(
    "Tatsugiri is an extremely cunning Pokémon.\n"
    "It feigns weakness to lure in prey, then\n"
    "orders its partner to attack.");

const u8 gAnnihilapePokedexText[] = _(
    "It imbues its fists with the power of the\n"
    "rage that it kept hidden in its heart.\n"
    "Opponents struck by these imbued fists\n"
    "will be shattered to their core.");

const u8 gClodsirePokedexText[] = _(
    "It lives at the bottom of ponds and\n"
    "swamps. It will carry Wooper on its back\n"
    "and ferry them across water from one\n"
    "shore to the other.");

const u8 gFarigirafPokedexText[] = _(
    "Now that the brain waves from the head\n"
    "and tail are synced up, the psychic power\n"
    "of this Pokémon is 10 times stronger than\n"
    "Girafarig’s.");

const u8 gDudunsprcePokedexText[] = _(
    "This Pokémon uses its hard tail to make\n"
    "its nest by boring holes into bedrock\n"
    "deep underground. The nest can reach\n"
    "lengths of over six miles.");

const u8 gKingambitPokedexText[] = _(
    "Though it commands a massive army in\n"
    "battle, it's not skilled at devising\n"
    "complex strategies. It just uses brute\n"
    "strength to keep pushing.");

const u8 gGreatTuskPokedexText[] = _(
    "Sightings of this Pokémon have occurred\n"
    "in recent years. The name Great Tusk was\n"
    "taken from a creature listed in a certain\n"
    "book.");

const u8 gScreamTailPokedexText[] = _(
    "There has been only one reported sighting\n"
    "of this Pokémon. It resembles a mysterious\n"
    "creature depicted in an old expedition\n"
    "journal.");

const u8 gBruteBonetPokedexText[] = _(
    "It bears a slight resemblance to a Pokémon\n"
    "described in a dubious magazine as a cross\n"
    "between a dinosaur and a mushroom.");

const u8 gFluttrManePokedexText[] = _(
    "It has similar features to a ghostly\n"
    "pterosaur that was covered in a\n"
    "paranormal magazine, but the two have\n"
    "little else in common.");

const u8 gSlithrWingPokedexText[] = _(
    "This Pokémon somewhat resembles an\n"
    "ancient form of Volcarona that was\n"
    "introduced in a dubious magazine.");

const u8 gSandyShockPokedexText[] = _(
    "No records exist of this Pokémon being\n"
    "caught. Data is lacking, but the Pokémon’s\n"
    "traits match up with a creature shown in\n"
    "an expedition journal.");

const u8 gIronTreadsPokedexText[] = _(
    "Sightings of this Pokémon have occurred in\n"
    "recent years. It resembles a mysterious\n"
    "object described in an old expedition\n"
    "journal.");

const u8 gIronBundlePokedexText[] = _(
    "Its shape is similar to a robot featured in\n"
    "a paranormal magazine article. The robot\n"
    "was said to have been created by an\n"
    "ancient civilization.");

const u8 gIronHandsPokedexText[] = _(
    "It is very similar to a cyborg covered\n"
    "exclusively by a paranormal magazine. The\n"
    "cyborg was said to be the modified form of\n"
    "a certain athlete.");

const u8 gIronJuglisPokedexText[] = _(
    "It resembles a certain Pokémon introduced\n"
    "in a paranormal magazine, described as the\n"
    "offspring of a Hydreigon that fell in love\n"
    "with a robot.");

const u8 gIronMothPokedexText[] = _(
    "No records exist of this species being\n"
    "caught. Data is lacking, but the Pokémon's\n"
    "traits match up with an object described\n"
    "in an old book.");

const u8 gIronThornsPokedexText[] = _(
    "It has some similarities to a Pokémon\n"
    "introduced in a dubious magazine as a\n"
    "Tyranitar from one billion years into the\n"
    "future.");

const u8 gFrigibaxPokedexText[] = _(
    "Frigibax absorbs heat through its dorsal\n"
    "fin and converts the heat into ice energy.\n"
    "The higher the temperature, the more\n"
    "energy Frigibax stores.");

const u8 gArctibaxPokedexText[] = _(
    "It attacks with the blade of its frozen\n"
    "dorsal fin by doing a front flip in the air.\n"
    "Arctibax's strong back and legs allow it to\n"
    "pull off this technique.");

const u8 gBaxcaliburPokedexText[] = _(
    "It launches itself into battle by flipping\n"
    "upside down and spewing frigid air from its\n"
    "mouth. It finishes opponents off with its\n"
    "dorsal blade.");

const u8 gGimmighoulPokedexText[] = _(
    "This Pokémon was born inside a treasure\n"
    "chest about 1,500 years ago. It sucks the\n"
    "life-force out of scoundrels who try to\n"
    "steal the treasure.");

const u8 gGholdengoPokedexText[] = _(
    "It has a sturdy body made up of stacked\n"
    "coins. Gholdengo overwhelms its enemies\n"
    "by firing coin after coin at them in quick\n"
    "succession.");

const u8 gWoChienPokedexText[] = _(
    "The grudge of a person punished for\n"
    "writing the king’s evil deeds upon wooden\n"
    "tablets has clad itself in dead leaves to\n"
    "become a Pokémon.");

const u8 gChienPaoPokedexText[] = _(
    "This Pokémon can control 100 tons of\n"
    "fallen snow. It plays around innocently by\n"
    "leaping in and out of avalanches it has\n"
    "caused.");

const u8 gTingLuPokedexText[] = _(
    "It slowly brings its exceedingly heavy\n"
    "head down upon the ground, splitting the\n"
    "earth open with huge fissures that run\n"
    "over 160 feet deep.");

const u8 gChiYuPokedexText[] = _(
    "It controls flames burning at over 5,400\n"
    "degrees Fahrenheit. It casually swims\n"
    "through the sea of lava it creates by\n"
    "melting rock and sand.");

const u8 gRoarinMoonPokedexText[] = _(
    "According to an article in a dubious\n"
    "magazine, this Pokémon has some\n"
    "connection to a phenomenon that occurs\n"
    "in a certain region.");

const u8 gIronValianPokedexText[] = _(
    "It's possible that this is the object\n"
    "listed as Iron Valiant in a certain\n"
    "expedition journal.");

const u8 gKoraidonPokedexText[] = _(
    "This Pokémon resembles Cyclizar, but it is\n"
    "far burlier and more ferocious. Nothing is\n"
    "known about its ecology or other features.");

const u8 gMiraidonPokedexText[] = _(
    "This seems to be the Iron Serpent\n"
    "mentioned in an old book. The Iron Serpent\n"
    "is said to have turned the land to ash with\n"
    "its lightning.");

const u8 gWalkinWakePokedexText[] = _(
    "This ferocious creature is shrouded in\n"
    "mystery. It's named after an aquatic\n"
    "monster mentioned in an old expedition\n"
    "journal.");

const u8 gIronLeavesPokedexText[] = _(
    "Many of its physical characteristics\n"
    "match those of a Virizion from the future\n"
    "that was covered in a paranormal magazine.");

const u8 gDipplinPokedexText[] = _(
    "The head sticking out belongs to the\n"
    "fore-wyrm, while the tail belongs to the\n"
    "core-wyrm. The two share one apple and\n"
    "help each other out.");

const u8 gPolchgeistPokedexText[] = _(
    "Supposedly, the regrets of a tea ceremony\n"
    "master who died before perfecting his\n"
    "craft lingered in some matcha and became\n"
    "a Pokémon.");

const u8 gSinistchaPokedexText[] = _(
    "It prefers cool, dark places, such as the\n"
    "back of a shelf or the space beneath a\n"
    "home's floorboards. It wanders in search\n"
    "of prey after sunset.");

const u8 gOkidogiPokedexText[] = _(
    "After all its muscles were stimulated by\n"
    "the toxic chain around its neck, Okidogi\n"
    "transformed and gained a powerful\n"
    "physique.");

const u8 gMunkidoriPokedexText[] = _(
    "The chain is made from toxins that\n"
    "enhance capabilities. It stimulated\n"
    "Munkidori's brain and caused the\n"
    "Pokémon's psychic powers to bloom.");

const u8 gFezandiptiPokedexText[] = _(
    "Fezandipiti owes its beautiful looks and\n"
    "lovely voice to the toxic stimulants\n"
    "emanating from the chain wrapped around\n"
    "its body.");

const u8 gOgerponPokedexText[] = _(
    "This mischief-loving Pokémon is full of\n"
    "curiosity. It battles by drawing out the\n"
    "type-based energy contained within its\n"
    "masks.");

const u8 gArchaludonPokedexText[] = _(
    "It gathers static electricity from its\n"
    "surroundings. The beams it launches when\n"
    "down on all fours are tremendously\n"
    "powerful.");

const u8 gHydrapplePokedexText[] = _(
    "These capricious syrpents have banded\n"
    "together. On the rare occasion that their\n"
    "moods align, their true power is unleashed.");

const u8 gGougingFirePokedexText[] = _(
    "It resembles an eerie Pokémon once shown\n"
    "in a paranormal magazine. That Pokémon was\n"
    "said to be an Entei regenerated from a\n"
    "fossil.");

const u8 gRagingBoltPokedexText[] = _(
    "It bears resemblance to a Pokémon that\n"
    "became a hot topic for a short while after\n"
    "a paranormal magazine touted it as\n"
    "Raikou's ancestor.");

const u8 gIronBoulderPokedexText[] = _(
    "It resembles a Pokémon described in a\n"
    "dubious magazine as a Terrakion that had\n"
    "been modified by an evil organization.");

const u8 gIronCrownPokedexText[] = _(
    "There was supposedly an incident in\n"
    "which it launched shining blades to cut\n"
    "everything around it to pieces.\n"
    "Little else is known about it.");

const u8 gTerapagosPokedexText[] = _(
    "Terapagos protects itself using its\n"
    "power to transform energy into hard\n"
    "crystals. This Pokémon is the source\n"
    "of the Terastal phenomenon.");

const u8 gPecharuntPokedexText[] = _(
    "It feeds others toxic mochi that draw out\n"
    "desires and capabilities. Those who eat\n"
    "the mochi fall under Pecharunt’s control,\n"
    "chained to its will.");

const u8 gVenusaurMPokedexText[] = _(
    "In order to support its flower, which has\n"
    "grown larger due to Mega Evolution, its\n"
    "back and legs have become stronger.");

const u8 gCharizardXPokedexText[] = _(
    "The overwhelming power that fills its\n"
    "entire body causes it to turn black and\n"
    "creates intense blue flames.");

const u8 gCharizardYPokedexText[] = _(
    "Its bond with its Trainer is the source of\n"
    "its power. It boasts speed and\n"
    "maneuverability greater than that of a\n"
    "jet fighter.");

const u8 gBlastoiseMPokedexText[] = _(
    "The cannon on its back is as powerful as a\n"
    "tank gun. Its tough legs and back enable\n"
    "it to withstand the recoil from firing the\n"
    "cannon.");

const u8 gBeedrillMPokedexText[] = _(
    "Its legs have become poison stingers. It\n"
    "stabs its prey repeatedly with the\n"
    "stingers on its limbs, dealing the final\n"
    "blow with the stinger on its rear.");

const u8 gPidgeotMPokedexText[] = _(
    "With its muscular strength now greatly\n"
    "increased, it can fly continuously for two\n"
    "weeks without resting.");

const u8 gAlakazamMPokedexText[] = _(
    "Having traded away its muscles, Alakazam's\n"
    "true power has been unleashed. With its\n"
    "psychic powers, it can foresee all things.");

const u8 gSlowbroMPokedexText[] = _(
    "Having been swallowed whole by Shellder,\n"
    "Slowbro now has an iron defense. It's\n"
    "pretty comfortable in there, too.");

const u8 gGengarMPokedexText[] = _(
    "Mega Evolution has made it possible for\n"
    "Gengar to access other dimensions. Its\n"
    "entire body is brimming with strange\n"
    "power.");

const u8 gKangaskhaMPokedexText[] = _(
    "When the mother sees the back of her\n"
    "Mega-Evolved child, it makes her think of\n"
    "the day when her child will inevitably\n"
    "leave her.");

const u8 gPinsirMPokedexText[] = _(
    "After Mega Evolution, it becomes able to\n"
    "fly. Perhaps because it's so happy, it\n"
    "rarely touches the ground.");

const u8 gGyaradosMPokedexText[] = _(
    "Mega Evolution places a burden on its\n"
    "body. The stress causes it to become all\n"
    "the more ferocious.");

const u8 gAerodactyMPokedexText[] = _(
    "Mega Evolution awakened some dormant\n"
    "genes, bringing back the sharp rocks that\n"
    "once covered Aerodactyl's entire body.");

const u8 gMewtwoXPokedexText[] = _(
    "Psychic power has augmented its muscles.\n"
    "It has a grip strength of one ton and can\n"
    "sprint a hundred meters in two seconds\n"
    "flat!");

const u8 gMewtwoYPokedexText[] = _(
    "Despite its diminished size, its mental\n"
    "power has grown phenomenally. With a mere\n"
    "thought, it can smash a skyscraper to\n"
    "smithereens.");

const u8 gAmpharosMPokedexText[] = _(
    "Massive amounts of energy intensely\n"
    "stimulated Ampharos's cells, apparently\n"
    "awakening its long-sleeping dragon's\n"
    "blood.");

const u8 gSteelixMPokedexText[] = _(
    "Steelix live even further underground\n"
    "than Onix. This Pokémon is known to dig\n"
    "toward the earth's core, reaching a depth\n"
    "of over six-tenths of a mile underground.");

const u8 gScizorMPokedexText[] = _(
    "It's better at beating things than\n"
    "grasping them. When it battles for a long\n"
    "time, the weight of its pincers becomes\n"
    "too much to bear.");

const u8 gHeracrossMPokedexText[] = _(
    "A tremendous influx of energy builds it up,\n"
    "but when Mega Evolution ends, Heracross is\n"
    "bothered by terrible soreness in its\n"
    "muscles.");

const u8 gHoundoomMPokedexText[] = _(
    "Houndoom's entire body generates heat\n"
    "when it Mega Evolves. Its fearsome fiery\n"
    "breath turns its opponents to ash.");

const u8 gTyranitarMPokedexText[] = _(
    "Due to the colossal power poured into it,\n"
    "this Pokémon's back split right open. Its\n"
    "destructive instincts are the only thing\n"
    "keeping it moving.");

const u8 gSceptileMPokedexText[] = _(
    "In the jungle, its power is without equal.\n"
    "This Pokémon carefully grows trees and\n"
    "plants. It regulates its body temperature\n"
    "by basking in sunlight.");

const u8 gBlazikenMPokedexText[] = _(
    "It learns martial arts that use punches\n"
    "and kicks. Every several years, its old\n"
    "feathers burn off, and new, supple\n"
    "feathers grow back in their place.");

const u8 gSwampertMPokedexText[] = _(
    "If it senses the approach of a storm and\n"
    "a tidal wave, it protects its seaside nest\n"
    "by piling up boulders. It swims as fast as\n"
    "a jet ski.");

const u8 gGardevoirMPokedexText[] = _(
    "It apparently does not feel the pull of\n"
    "gravity because it supports itself with\n"
    "psychic power. It will give its life to\n"
    "protect its Trainer.");

const u8 gSableyeMPokedexText[] = _(
    "It blocks any and all attacks with its\n"
    "giant-sized gemstone. However, the\n"
    "stone's a heavy burden, and it limits Mega\n"
    "Sableye's movements.");

const u8 gMawileMPokedexText[] = _(
    "Its two sets of jaws thrash about\n"
    "violently as if they each had a will of\n"
    "their own. One gnash from them can turn a\n"
    "boulder to dust.");

const u8 gAggronMPokedexText[] = _(
    "Its iron horns grow longer a little at\n"
    "a time. They are used to determine the\n"
    "Aggron's age. The gouges in its armor are\n"
    "worn with pride as mementos from battles.");

const u8 gMedichamMPokedexText[] = _(
    "Through crushingly harsh yoga training, it\n"
    "gained the power to foretell its foe's\n"
    "actions. It battles with elegant, dance-\n"
    "like movement.");

const u8 gManectricMPokedexText[] = _(
    "Too much electricity has built up in its\n"
    "body, irritating Manectric. Its explosive\n"
    "speed is equal to that of a lightning bolt.");

const u8 gSharpedoMPokedexText[] = _(
    "The yellow patterns it bears are old scars.\n"
    "The energy from Mega Evolution runs\n"
    "through them, causing it sharp pain and\n"
    "suffering.");

const u8 gCameruptMPokedexText[] = _(
    "A Pokémon that lives in the crater of\n"
    "a volcano. Every 10 years, the volcanoes\n"
    "on its back erupt violently. Research is\n"
    "under way on the cause of eruption.");

const u8 gAltariaMPokedexText[] = _(
    "It hums in a beautiful soprano voice.\n"
    "It flies among white clouds in the blue\n"
    "sky. It launches intensely hot fireballs\n"
    "from its mouth.");

const u8 gBanetteMPokedexText[] = _(
    "Mega Evolution increases its\n"
    "vindictiveness, and the cursing power\n"
    "that was held back by its zipper comes\n"
    "spilling out.");

const u8 gAbsolMPokedexText[] = _(
    "It converts the energy from Mega\n"
    "Evolution into an intimidating aura.\n"
    "Fainthearted people expire from shock at\n"
    "the sight of it.");

const u8 gGlalieMPokedexText[] = _(
    "The power of Mega Evolution was so strong\n"
    "that it smashed Glalie's jaw. Its inability\n"
    "to eat very well leaves Glalie irritated.");

const u8 gSalamenceMPokedexText[] = _(
    "It puts its forelegs inside its shell to\n"
    "streamline itself for flight. Salamence\n"
    "flies at high speeds over all kinds of\n"
    "topographical features.");

const u8 gMetagrossMPokedexText[] = _(
    "When it knows it can't win, it digs the\n"
    "claws on its legs into its opponent and\n"
    "starts the countdown to a big explosion.");

const u8 gLatiasMPokedexText[] = _(
    "They make a small herd of only several\n"
    "members. They rarely make contact with\n"
    "people or other Pokémon. They disappear\n"
    "if they sense enemies.");

const u8 gLatiosMPokedexText[] = _(
    "Even in hiding, it can detect the locations\n"
    "of others and sense their emotions since\n"
    "it has telepathy. Its intelligence allows\n"
    "it to understand human languages.");

const u8 gLopunnyMPokedexText[] = _(
    "It swings its ears like whips and strikes\n"
    "its enemies with them. It has an intensely\n"
    "combative disposition.");

const u8 gGarchompMPokedexText[] = _(
    "Its disposition is more vicious than\n"
    "before its Mega Evolution. Garchomp\n"
    "carves its opponents up with the\n"
    "scythes on both arms.");

const u8 gLucarioMPokedexText[] = _(
    "Its aura has expanded due to Mega\n"
    "Evolution. Governed only by its combative\n"
    "instincts, it strikes enemies without\n"
    "mercy.");

const u8 gAbomasnowMPokedexText[] = _(
    "They appear when the snow flowers bloom.\n"
    "When the petals fall, they retreat to\n"
    "places unknown again.");

const u8 gGalladeMPokedexText[] = _(
    "Because it can sense what its foe is\n"
    "thinking, its attacks burst out first, fast,\n"
    "and fierce.");

const u8 gAudinoMPokedexText[] = _(
    "This Pokémon has a kind heart. By touching\n"
    "with its feelers, Audino can gauge other\n"
    "creatures' feelings and physical\n"
    "conditions.");

const u8 gDiancieMPokedexText[] = _(
    "It can instantly create many diamonds by\n"
    "compressing the carbon in the air between\n"
    "its hands.");

const u8 gRayquazaMPokedexText[] = _(
    "A Pokémon that flies endlessly in the\n"
    "ozone layer. It is said it would descend\n"
    "to the ground if Kyogre and Groudon\n"
    "were to fight.");

const u8 gKyogrePPokedexText[] = _(
    "Kyogre has appeared in mythology as the\n"
    "creator of the sea. After long years of\n"
    "feuding with Groudon, it took to sleep at\n"
    "the bottom of the sea.");

const u8 gGroudonPPokedexText[] = _(
    "Groudon has appeared in mythology as the\n"
    "creator of the land. It sleeps in magma\n"
    "underground and is said to make volcanoes\n"
    "erupt on awakening.");

const u8 gRattataAPokedexText[] = _(
    "Night after night, they sneak into\n"
    "people's homes seeking food. A massive\n"
    "outbreak of them has become an issue of\n"
    "public concern.");

const u8 gRaticateAPokedexText[] = _(
    "It has an incredibly greedy personality.\n"
    "Its nest is filled with so much food\n"
    "gathered by Rattata at its direction, it\n"
    "can't possibly eat it all.");

const u8 gRaichuAPokedexText[] = _(
    "It's believed that the weather, climate,\n"
    "and food of the Alola region all play a\n"
    "part in causing Pikachu to evolve into\n"
    "this form of Raichu.");

const u8 gSandshrewAPokedexText[] = _(
    "An ancient tradition of Alolan festivals,\n"
    "still carried on to this day, is a\n"
    "competition to slide Sandshrew across ice\n"
    "as far as one can.");

const u8 gSandslashAPokedexText[] = _(
    "A long, long time ago, it lived in the desert.\n"
    "With its sharp claws fully extended, it can\n"
    "run right up an iceberg without slipping.");

const u8 gVulpixAPokedexText[] = _(
    "They live together in a skulk, helping one\n"
    "another. Before eating their prey, they\n"
    "freeze it solid with their -58 degree\n"
    "Fahrenheit breath.");

const u8 gNinetalesAPokedexText[] = _(
    "Possessing a calm demeanor, this Pokémon\n"
    "was revered as a deity incarnate before it\n"
    "was identified as a regional variant of\n"
    "Ninetales.");

const u8 gDiglettAPokedexText[] = _(
    "The things growing from its head are\n"
    "whiskers that have become metallic.\n"
    "Diglett communicates with its comrades by\n"
    "waving them.");

const u8 gDugtrioAPokedexText[] = _(
    "Its metallic whiskers are heavy, so it’s\n"
    "not very fast, but it has the power to dig\n"
    "through even the hardest bedrock.");

const u8 gMeowthAPokedexText[] = _(
    "Deeply proud and keenly smart, this\n"
    "Pokémon moves with cunning during battle\n"
    "and relentlessly attacks enemies' weak\n"
    "points.");

const u8 gPersianAPokedexText[] = _(
    "The round face of Alolan Persian is\n"
    "considered to be a symbol of prosperity in\n"
    "the Alola region, so these Pokémon are\n"
    "very well cared for.");

const u8 gGeodudeAPokedexText[] = _(
    "If you step on a Geodude sleeping on the\n"
    "ground, you’ll hear a zapping sound and\n"
    "feel a tingling shock throughout your\n"
    "entire body.");

const u8 gGravelerAPokedexText[] = _(
    "They eat rocks and often get into a scrap\n"
    "over them. The impact of Graveler smashing\n"
    "together causes a flash of light and a\n"
    "booming noise.");

const u8 gGolemAPokedexText[] = _(
    "It fires rocks charged with electricity.\n"
    "Even if the rock isn’t fired that\n"
    "accurately, just grazing an opponent will\n"
    "cause numbness and fainting.");

const u8 gGrimerAPokedexText[] = _(
    "Alolan Grimer will gladly gobble up any kind\n"
    "of trash, making it a widely used and\n"
    "popular Pokémon at garbage disposal\n"
    "facilities.");

const u8 gMukAPokedexText[] = _(
    "There are over a hundred kinds of poison\n"
    "inside its body. Chemical reactions\n"
    "between different poisons are the source\n"
    "of its vitality.");

const u8 gExeggutorAPokedexText[] = _(
    "As it grew taller and taller, it outgrew its\n"
    "reliance on psychic powers, while within it\n"
    "awakened the power of the sleeping\n"
    "dragon.");

const u8 gMarowakAPokedexText[] = _(
    "The cursed flames that light up the bone\n"
    "carried by this Pokémon are said to cause\n"
    "both mental and physical pain that will\n"
    "never fade.");

const u8 gMeowthGPokedexText[] = _(
    "These daring Pokémon have coins on their\n"
    "foreheads. Darker coins are harder, and\n"
    "harder coins garner more respect among\n"
    "Meowth.");

const u8 gPonytaGPokedexText[] = _(
    "Its small horn hides a healing power. With a\n"
    "few rubs from this Pokémon's horn, any\n"
    "slight wound you have will be healed.");

const u8 gRapidashGPokedexText[] = _(
    "Brave and prideful, this Pokémon dashes\n"
    "airily through the forest, its steps aided\n"
    "by the psychic power stored in the fur on\n"
    "its fetlocks.");

const u8 gSlowpokeGPokedexText[] = _(
    "It gives off a stimulating scent because\n"
    "of chemicals that have seeped into its\n"
    "body from the spice that it eats as its\n"
    "staple food.");

const u8 gSlowbroGPokedexText[] = _(
    "Shellder bites down hard on Slowbro’s arm\n"
    "to suck out more poison. This seems to\n"
    "cause itching, so Slowbro occasionally\n"
    "lashes out.");

const u8 gFarfetchdGPokedexText[] = _(
    "The stalks of leeks are thicker and longer\n"
    "in the Galar region. Farfetch'd that\n"
    "adapted to these stalks took on a unique\n"
    "form.");

const u8 gWeezingGPokedexText[] = _(
    "Long ago, during a time when droves of\n"
    "factories fouled the air with pollution,\n"
    "Weezing changed into this form for some\n"
    "reason.");

const u8 gMrMimeGPokedexText[] = _(
    "Its talent is tap-dancing. It can also\n"
    "manipulate temperatures to create a floor\n"
    "of ice, which this Pokémon can kick up to\n"
    "use as a barrier.");

const u8 gArticunoGPokedexText[] = _(
    "Its feather-like blades are composed of\n"
    "psychic energy and can shear through\n"
    "thick iron sheets as if they were paper.");

const u8 gZapdosGPokedexText[] = _(
    "One kick from its powerful legs will\n"
    "pulverize a dump truck. Supposedly, this\n"
    "Pokémon runs through the mountains at\n"
    "over 180 mph.");

const u8 gMoltresGPokedexText[] = _(
    "This Pokémon's sinister, flame-like aura\n"
    "will consume the spirit of any creature it\n"
    "hits. Victims become burned-out shadows\n"
    "of themselves.");

const u8 gSlowkingGPokedexText[] = _(
    "It administers its potions to weakened\n"
    "Pokémon it sees. These potions are\n"
    "derived from poison and secreted from\n"
    "holes in Slowking’s horns.");

const u8 gCorsolaGPokedexText[] = _(
    "Watch your step when wandering areas\n"
    "oceans once covered. What looks like a\n"
    "stone could be this Pokémon, and it will\n"
    "curse you if you kick it.");

const u8 gZigzagoonGPokedexText[] = _(
    "Its restlessness has it constantly\n"
    "running around. If it sees another\n"
    "Pokémon, it will purposely run into them in\n"
    "order to start a fight.");

const u8 gLinooneGPokedexText[] = _(
    "It uses its long tongue to taunt\n"
    "opponents. Once the opposition is\n"
    "enraged, this Pokémon hurls itself at the\n"
    "opponent, tackling them forcefully.");

const u8 gDarumakaGPokedexText[] = _(
    "The colder they get, the more energetic\n"
    "they are. They freeze their breath to make\n"
    "snowballs, using them as ammo for playful\n"
    "snowball fights.");

const u8 gDarmanitaGPokedexText[] = _(
    "On days when blizzards blow through, it\n"
    "comes down to where people live. It stashes\n"
    "food in the snowball on its head, taking it\n"
    "home for later.");

const u8 gYamaskGPokedexText[] = _(
    "It's said that this Pokémon was formed\n"
    "when an ancient clay tablet was drawn to a\n"
    "vengeful spirit.");

const u8 gStunfiskGPokedexText[] = _(
    "Its conspicuous lips lure prey in as it lies\n"
    "in wait in the mud. When prey gets close,\n"
    "Stunfisk clamps its jagged steel fins down\n"
    "on them.");

const u8 gGrowlitheHPokedexText[] = _(
    "They patrol their territory in pairs.\n"
    "The igneous rock components in its fur\n"
    "are the result of volcanic activity in its\n"
    "habitat.");

const u8 gArcanineHPokedexText[] = _(
    "Despite its bulk, it deftly feints every\n"
    "which way, leading opponents on a\n"
    "deceptively merry chase as it all but\n"
    "dances around them.");

const u8 gVoltorbHPokedexText[] = _(
    "An enigmatic Pokémon that resembles a\n"
    "Poké Ball. When excited, it discharges the\n"
    "electric current it has stored in its belly,\n"
    "then lets out a great, uproarious laugh.");

const u8 gElectrodeHPokedexText[] = _(
    "When irritated, this Pokémon lets loose\n"
    "an electric current equal to 20 lightning\n"
    "bolts.");

const u8 gTyphlosioHPokedexText[] = _(
    "This form of Typhlosion is said to purify\n"
    "lost, forsaken souls with its flames and\n"
    "guide them to the afterlife.");

const u8 gQwilfishHPokedexText[] = _(
    "It lives just offshore in cold parts of\n"
    "the world. Its poisonous spikes are\n"
    "thicker and shorter than those of other\n"
    "regions’ Qwilfish.");

const u8 gSneaselHPokedexText[] = _(
    "From the tips of its claws drips a venom\n"
    "that infiltrates the nerves of any prey\n"
    "caught in its grasp.");

const u8 gSamurottHPokedexText[] = _(
    "Hard of heart and deft of blade, this\n"
    "rare form of Samurott is a product of the\n"
    "Pokémon's evolution in the region of Hisui.");

const u8 gLilligantHPokedexText[] = _(
    "Its well-developed legs are the result of\n"
    "a life spent on mountains covered in deep\n"
    "snow. The scent it exudes from its flower\n"
    "crown heartens those in proximity.");

const u8 gZoruaHPokedexText[] = _(
    "Derives power from resentment, which rises\n"
    "as energy atop its head and takes on the\n"
    "forms of foes. In this way, Zorua vents\n"
    "lingering malice.");

const u8 gZoroarkHPokedexText[] = _(
    "Heedless of its own safety, Zoroark\n"
    "attacks its nemeses with a bitter energy\n"
    "so intense, it lacerates Zoroark's own\n"
    "body.");

const u8 gBraviaryHPokedexText[] = _(
    "Screaming a bloodcurdling battle cry, this\n"
    "huge and ferocious bird Pokémon goes out\n"
    "on the hunt.");

const u8 gSliggooHPokedexText[] = _(
    "A creature given to melancholy. Its\n"
    "metallic shell developed as a result of\n"
    "the mucus on its skin reacting with the\n"
    "iron in Hisui's water.");

const u8 gGoodraHPokedexText[] = _(
    "Able to freely control the hardness of its\n"
    "metallic shell. It loathes solitude and is\n"
    "extremely clingy--it will fume and run riot\n"
    "if those dearest to it ever leave its side.");

const u8 gAvaluggHPokedexText[] = _(
    "This Pokémon barrels along steep\n"
    "mountain paths, cleaving through the\n"
    "deep snow.");

const u8 gDecidueyeHPokedexText[] = _(
    "The air stored inside the rachises of\n"
    "Decidueye's feathers insulates the\n"
    "Pokémon against Hisui's extreme cold.");

const u8 gTaurosPPokedexText[] = _(
    "This Pokémon has a muscular body and\n"
    "excels at close-quarters combat. It uses\n"
    "its short horns to strike the opponent’s\n"
    "weak spots.");

const u8 gWooperPPokedexText[] = _(
    "After losing a territorial struggle, Wooper\n"
    "began living on land. The Pokémon changed\n"
    "over time, developing a poisonous film to\n"
    "protect its body.");

const u8 gTaurosPFPokedexText[] = _(
    "When heated by fire energy, its horns can\n"
    "get hotter than 1,800 degrees Fahrenheit.\n"
    "Those gored by them will suffer both\n"
    "wounds and burns.");

const u8 gTaurosPWPokedexText[] = _(
    "It swims by jetting water from its horns.\n"
    "The most notable characteristic of the\n"
    "Aqua Breed is its high body fat, which\n"
    "allows it to float easily.");

const u8 gCastformFPokedexText[] = _(
    "This is the form Castform takes on the\n"
    "brightest of days. Its skin is\n"
    "unexpectedly hot to the touch, so\n"
    "approach with care.");

const u8 gCastformWPokedexText[] = _(
    "This is the form Castform takes when\n"
    "soaked with rain. When its body is\n"
    "compressed, water will seep out as if from a\n"
    "sponge.");

const u8 gCastformIPokedexText[] = _(
    "This is the form Castform takes when\n"
    "covered in snow. Its body becomes an ice-\n"
    "like material, with a temperature near 23\n"
    "degrees Fahrenheit.");

const u8 gDeoxysAPokedexText[] = _(
    "This Deoxys has transformed into its\n"
    "aggressive guise. It can fool enemies by\n"
    "altering its appearance.");

const u8 gDeoxysDPokedexText[] = _(
    "When it changes form, an aurora appears.\n"
    "It absorbs attacks by altering its\n"
    "cellular structure.");

const u8 gDeoxysSPokedexText[] = _(
    "A Pokémon that mutated from an\n"
    "extraterrestrial virus exposed to a laser\n"
    "beam. Its body is configured for superior\n"
    "agility and speed.");

const u8 gBurmySPokedexText[] = _(
    "It covers itself with a cloak to shelter\n"
    "from the cold. When it's hot, its cloak is\n"
    "thinner.");

const u8 gBurmyTPokedexText[] = _(
    "Even if it is born where there are no\n"
    "cocooning materials, it somehow always\n"
    "ends up with a cloak.");

const u8 gWormadamSPokedexText[] = _(
    "It is said that a Wormadam that evolves on\n"
    "a cold day will have a thicker cloak.");

const u8 gWormadamTPokedexText[] = _(
    "When evolving, its body takes in\n"
    "surrounding materials. As a result, there\n"
    "are many body variations.");

const u8 gCherrimSunshinePokedexText[] = _(
    "After absorbing plenty of sunlight,\n"
    "Cherrim takes this form. It's full of\n"
    "energy while it's like this, and its\n"
    "liveliness will go on until sundown.");

const u8 gRotomHPokedexText[] = _(
    "This Rotom has possessed a convection\n"
    "microwave oven. It will sear the\n"
    "surrounding area with its burning heat,\n"
    "then smile in delight.");

const u8 gRotomWPokedexText[] = _(
    "The model of washing machine that Rotom\n"
    "can inspirit has been discontinued, so\n"
    "these appliances are now traded at high\n"
    "prices.");

const u8 gRotomFrPokedexText[] = _(
    "The special motor within the refrigerator\n"
    "is the key to bringing about Rotom's change\n"
    "in form.");

const u8 gRotomFaPokedexText[] = _(
    "This Rotom has entered an electric fan. It\n"
    "smirks with satisfaction over a prank well\n"
    "pulled after it blows away everything\n"
    "around it.");

const u8 gRotomMPokedexText[] = _(
    "This Rotom has possessed a lawn mower. It\n"
    "will mow down grass and flowers, then\n"
    "swagger around with pride at its\n"
    "accomplishments.");

const u8 gDialgaOPokedexText[] = _(
    "Radiant light caused it to take on a form\n"
    "bearing a striking resemblance to Arceus.\n"
    "It now wields such colossal strength that\n"
    "one must conclude this is its true form.");

const u8 gPalkiaOPokedexText[] = _(
    "It soars across the sky in a form that\n"
    "greatly resembles Arceus of all things.\n"
    "Perhaps this imitation of appearance is\n"
    "its strategy for gaining Arceus's powers.");

const u8 gGiratinaOPokedexText[] = _(
    "Giratina loses its legs upon changing into\n"
    "this form. It's believed it hails from a\n"
    "world where the heavens and the earth\n"
    "are as one.");

const u8 gShayminSPokedexText[] = _(
    "Upon taking in the scent of a particular\n"
    "rare flower, Shaymin is enveloped in\n"
    "light, and its tiny body transforms.");

const u8 gBasculinBluePokedexText[] = _(
    "These Pokémon form schools with others of\n"
    "the same color. The greatest foe of a\n"
    "school of blue Basculin is a school of red\n"
    "Basculin.");

const u8 gBasculinHPokedexText[] = _(
    "Its ecology is starkly different from that\n"
    "of other Basculin, so theories that it's a\n"
    "totally different species have gained\n"
    "traction in recent years.");

const u8 gDarmanitaZPokedexText[] = _(
    "When wounded, it stops moving. It goes as\n"
    "still as stone to meditate, sharpening its\n"
    "mind and spirit.");

const u8 gDarmanitZGPokedexText[] = _(
    "Darmanitan takes this form when enraged.\n"
    "It won't stop spewing flames until its rage\n"
    "has settled, even if its body starts to\n"
    "melt.");

const u8 gTornadusTPokedexText[] = _(
    "This strange avian guise is believed to be\n"
    "Tornadus’s true form. Tornadus has been\n"
    "sighted crossing the ocean while in this\n"
    "form.");

const u8 gThundurusTPokedexText[] = _(
    "It pulverizes foes into nothingness with\n"
    "showers of devastatingly powerful\n"
    "lightning bolts launched from the string of\n"
    "orbs on its tail.");

const u8 gLandorusTPokedexText[] = _(
    "Landorus soars through the sky in this\n"
    "form, bestowing plentiful harvests upon\n"
    "the land and earning the people's\n"
    "reverence.");

const u8 gKyuremBPokedexText[] = _(
    "The sameness of Reshiram's and Kyurem's\n"
    "genes allowed Kyurem to absorb Reshiram.\n"
    "Kyurem can now use the power of both fire\n"
    "and ice.");

const u8 gKyuremWPokedexText[] = _(
    "The sameness of Zekrom's and Kyurem's\n"
    "genes allowed Kyurem to absorb Zekrom.\n"
    "Kyurem can now use the power of both\n"
    "electricity and ice.");

const u8 gMeloettaPPokedexText[] = _(
    "The melodies sung by Meloetta have the\n"
    "power to make Pokémon that hear them\n"
    "happy or sad.");

const u8 gGreninjaAPokedexText[] = _(
    "It appears and vanishes with a ninja's\n"
    "grace. It toys with its enemies using swift\n"
    "movements, while slicing them with throwing\n"
    "stars of sharpest water.");

const u8 gFloetteEPokedexText[] = _(
    "Terrifying energy is concealed within its\n"
    "ominous flower, but Floette still swings it\n"
    "about innocently.");

const u8 gMeowsticFPokedexText[] = _(
    "Female Meowstic are more aggressive than\n"
    "males. Their expression remains blank even\n"
    "while they're wielding immense psychic\n"
    "power.");

const u8 gAegislashBladePokedexText[] = _(
    "Once upon a time, a king with an Aegislash\n"
    "reigned over the land. His Pokémon\n"
    "eventually drained him of life, and his\n"
    "kingdom fell with him.");

const u8 gPumpkabooSmallPokedexText[] = _(
    "When taking spirits to the afterlife, small\n"
    "Pumpkaboo prefer the spirits of children\n"
    "to those of adults.");

const u8 gPumpkabooLargePokedexText[] = _(
    "When taking spirits to the afterlife, large\n"
    "Pumpkaboo prefer the spirits of adults to\n"
    "those of children.");

const u8 gPumpkabooSuperPokedexText[] = _(
    "Supersized Pumpkaboo are very partial to\n"
    "the spirits of people who were of similarly\n"
    "superior proportions.");

const u8 gGourgeistSmallPokedexText[] = _(
    "A small-sized Pumpkaboo evolves into a\n"
    "small-sized Gourgeist. Its bodily\n"
    "proportions also get passed on to its\n"
    "descendants.");

const u8 gGourgeistLargePokedexText[] = _(
    "A large-sized Pumpkaboo evolves into a\n"
    "large-sized Gourgeist. Its bodily\n"
    "proportions also get passed on to its\n"
    "descendants.");

const u8 gGourgeistSuperPokedexText[] = _(
    "A supersized Pumpkaboo evolves into a\n"
    "supersized Gourgeist. Its bodily\n"
    "proportions also get passed on to its\n"
    "descendants.");

const u8 gZygarde10PokedexText[] = _(
    "This is Zygarde when about 10% of its\n"
    "pieces have been assembled. It leaps at\n"
    "its opponent's chest and sinks its sharp\n"
    "fangs into them.");

const u8 gZygardeCPokedexText[] = _(
    "This is Zygarde's perfected form. From the\n"
    "orifice on its chest, it radiates high-\n"
    "powered energy that eliminates\n"
    "everything.");

const u8 gHoopaUPokedexText[] = _(
    "In its true form, it possess a huge amount\n"
    "of power. Legends of its avarice tell how it\n"
    "once carried off an entire castle to gain\n"
    "the treasure hidden within.");

const u8 gOricorioEPokedexText[] = _(
    "This form of Oricorio has sipped yellow\n"
    "nectar. It uses nimble steps to approach\n"
    "opponents, then knocks them out with\n"
    "electric punches.");

const u8 gOricorioPPokedexText[] = _(
    "This form of Oricorio has sipped pink\n"
    "nectar. It elevates its mind with the\n"
    "gentle steps of its dance, then unleashes\n"
    "its psychic energy.");

const u8 gOricorioGPokedexText[] = _(
    "This form of Oricorio has sipped purple\n"
    "nectar. It uses ethereal dance steps to\n"
    "call forth the spirits of the dead.");

const u8 gLycanrocMidnightPokedexText[] = _(
    "This Pokémon uses its rocky mane to slash\n"
    "any who approach. It will even disobey its\n"
    "Trainer if it dislikes the orders it was\n"
    "given.");

const u8 gLycanrocDuskPokedexText[] = _(
    "These Pokémon have both calm and\n"
    "ferocious qualities. It's said that this\n"
    "form of Lycanroc is the most troublesome\n"
    "to raise.");

const u8 gWishiwashSPokedexText[] = _(
    "When facing tough opponents, they get\n"
    "into formation. But if they get wounded in\n"
    "battle, they'll scatter and become\n"
    "solitary again.");

const u8 gMiniorCPokedexText[] = _(
    "It feeds on dust in the atmosphere. The\n"
    "color of its core is said to be determined\n"
    "by the composition of the dust it eats.");

const u8 gNecrozmaDMPokedexText[] = _(
    "When it dominates Solgaleo, it takes on\n"
    "this form. It's a vicious Pokémon, mangling\n"
    "prey with its many claws--including those\n"
    "on its back.");

const u8 gNecrozmaDWPokedexText[] = _(
    "When Necrozma latches on to Lunala, it\n"
    "becomes vicious, seeing enemies\n"
    "everywhere it looks. It will burn the world\n"
    "with lasers.");

const u8 gNecrozmaUPokedexText[] = _(
    "The light pouring out from all over its\n"
    "body affects living things and nature,\n"
    "impacting them in various ways.");

const u8 gToxtricityLowKeyPokedexText[] = _(
    "As it gulps down stagnant water and\n"
    "generates electricity in its body, a sound\n"
    "like a rhythm played by a bass guitar\n"
    "reverberates all around.");

const u8 gEiscueNoiceFacePokedexText[] = _(
    "The ice that was covering this Eiscue’s\n"
    "face has shattered. Eiscue emits cold air\n"
    "from the tip of its hair to protect its\n"
    "head, which is sensitive to heat.");

const u8 gIndeedeeFPokedexText[] = _(
    "It dislikes conflict. If a fight occurs in\n"
    "its territory, it uses psychic power to\n"
    "throw things at the combatants in an\n"
    "effort to drive them away.");

const u8 gMorpekoHangryPokedexText[] = _(
    "Hunger has disrupted this Pokémon's\n"
    "hormonal balance. The Electric-type\n"
    "energy stored in its cheek sacs has\n"
    "turned into Dark-type energy.");

const u8 gZacianCPokedexText[] = _(
    "Able to cut down anything with a single\n"
    "strike, it became known as the Fairy King's\n"
    "Sword, and it inspired awe in friend and\n"
    "foe alike.");

const u8 gZamazentaCPokedexText[] = _(
    "Its ability to deflect any attack led to\n"
    "it being known as the Fighting Master's\n"
    "Shield. It was feared and respected by\n"
    "all.");

const u8 gEternatusEternamaxPokedexText[] = _(
    "As a result of Rose's meddling,\n"
    "Eternatus absorbed all the energy\n"
    "in the Galar region. It's now in a\n"
    "state of power overload.");

const u8 gUrshifuRPokedexText[] = _(
    "This form of Urshifu is a strong believer in\n"
    "defeating foes by raining many blows down\n"
    "on them. Its strikes are nonstop, flowing\n"
    "like a river.");

const u8 gCalyrexIPokedexText[] = _(
    "According to lore, this Pokémon showed no\n"
    "mercy to those who got in its way, yet it\n"
    "would heal its opponents' wounds after\n"
    "battle.");

const u8 gCalyrexSPokedexText[] = _(
    "Legend says that by using its power to\n"
    "see all events from past to future, this\n"
    "Pokémon saved the creatures of a forest\n"
    "from a meteorite strike.");

const u8 gUrsalunaBPokedexText[] = _(
    "It crossed the sea and drifted ashore in a\n"
    "new land. Surviving in this place led it to\n"
    "take on a unique appearance and gain\n"
    "special powers.");

const u8 gBasculegionFPokedexText[] = _(
    "It can afflict a target with terrifying\n"
    "illusions that are under its control. The\n"
    "deeper the sadness in its friends' souls,\n"
    "the paler Basculegion becomes.");

const u8 gEnamorusTPokedexText[] = _(
    "From the clouds, it descends upon\n"
    "those who treat any form of life\n"
    "with disrespect and metes out\n"
    "wrathful, ruthless punishment.");

const u8 gOinkologneFPokedexText[] = _(
    "This Pokémon sends a flowerlike scent\n"
    "wafting about. Well-developed muscles in\n"
    "its legs allow it to leap more than 16 feet\n"
    "with no trouble at all.");

const u8 gPalafinHPokedexText[] = _(
    "This Pokémon’s ancient genes have\n"
    "awakened. It is now so extraordinarily\n"
    "strong that it can easily lift a cruise\n"
    "ship with one fin.");

const u8 gGimmighoulRoamingPokedexText[] = _(
    "This Pokémon was born from passion that\n"
    "seeped into a coin. It wanders, apparently\n"
    "seeking to return to the treasure chest it\n"
    "once inhabited.");

const u8 gOgerponWPokedexText[] = _(
    "This form excels in both attack and\n"
    "defense. It ceaselessly unleashes moves\n"
    "like a spring gushes water.");

const u8 gOgerponFPokedexText[] = _(
    "This form is the most aggressive,\n"
    "bombarding enemies with the intensity of\n"
    "flames blazing within a hearth.");

const u8 gOgerponRPokedexText[] = _(
    "This form has excellent defenses,\n"
    "absorbing impacts solidly like the\n"
    "cornerstones that support houses.");

const u8 gTerapagosTerastalPokedexText[] = _(
    "The shell is made of crystallized Terastal\n"
    "energy. When struck by a move, this shell\n"
    "absorbs the move’s energy and transfers\n"
    "it to Terapagos.");

const u8 gTerapagosStellarPokedexText[] = _(
    "An old expedition journal describes the\n"
    "sight of this Pokémon buried in the depths\n"
    "of the earth as resembling a planet\n"
    "floating in space.");
